...
function docSach() {
    return JSON.parse(readFileSync(join(process.cwd(), 'public/lorebooks/than-thoai-an-do.json'), 'utf8'));
}
function theGioi() {
...
describe('Lorebook dựng sẵn — Thần thoại Ấn Độ', () => {
    ...
    it('mở theo giai đoạn, cho gọi đích danh sớm và không dồn nhiều entry cùng nhóm', () => {
        ...
        const called = dungChiMuc(state, 'Ta muốn hành hương tới Kailash').filter((c) => c.id.startsWith('ck_lore_'));
