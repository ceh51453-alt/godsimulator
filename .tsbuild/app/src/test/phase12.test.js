...
function chuoiRac(rng, dai) {
    let s = '';
    for (let i = 0; i < dai; i++) {
        if (rng.d100() <= 12) {
            s += CHU_DOC[rng.nguyen(CHU_DOC.length)];
            continue;
        }
        s += CHU[rng.nguyen(CHU.length)];
    }
    return s;
}
/** Một giá trị JSON bất kỳ, sâu tối đa `sau` tầng. */
function giaTriRac(rng, sau = 3) {
    ...
    switch (loai) {
        ...
        default: {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const khoa = rng.d100() <= 15
                    ? ['__proto__', 'constructor', 'prototype'][rng.nguyen(3)]
...
describe('[BB] Phase 12 — file độc hại không có side effect', () => {
    ...
    it('fuzz 500 lorebook: nhapLorebook luôn trả issue thay vì nổ', () => {
        ...
        for (let i = 0; i < 500; i++) {
            ...
            if (!kq.ok)
                expect(kq.issues.length).toBeGreaterThan(0);
            else
                expect(kq.lorebook).not.toBeNull();
        }
    });
    ...
    it('[BB] 64.4 — khóa nguy hiểm KHÔNG làm bẩn Object.prototype', () => {
        const doc = JSON.parse('{"__proto__":{"biNhiem":true},"a":1}');
        locKhoaNguyHiem(doc);
        quetAnToan(doc);
        expect({}['biNhiem']).toBeUndefined();
    });
    it('fuzz 400 cấu hình rerank: luôn về heuristic an toàn, không throw', () => {
        const rng = taoRng('fuzz#rerank');
        for (let i = 0; i < 400; i++) {
            const kq = doCauHinhRerank(giaTriRac(rng, 3));
...
describe('[BB] Phase 12 — bản ghi model tạo phải ĐỦ trước khi vào thế giới', () => {
    ...
    it('aspect thiếu trường được điền theo `.prefault()`, không được để undefined', () => {
        const r = chuanHoaBanGhiMoi('entities', {
            id: 'mortal_x',
            kind: 'mortal',
            ten: 'Người Thiếu',
            tickSinh: 0,
    ...
    it('domain thiếu `doiThuIds`/`neoTaiChiem` cũng được điền — bất biến tầng Thần chạm vào chúng', () => {
        ...
        const d = r.value.aspects['domain'];
    ...
    it('aspect engine không biết tên bị BỎ kèm lý do, không giữ làm dữ liệu tự do', () => {
        const r = chuanHoaBanGhiMoi('entities', { id: 'x', kind: 'concept', ten: 'X', tickSinh: 0, aspects: { mat_troi_bi_an: { x: 1 } } }, 'br_test');
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
        expect(r.value.aspects['mat_troi_bi_an']).toBeUndefined();
        ...
        for (let i = 0; i < 600; i++) {
            for (const bang of ['entities', 'links', 'gaps', 'prayers']) {
                ...
                if (r.ok) {
                    ...
                    expect(r.value.branchId).toBe('br_test');
                }
                else {
...
<CapNhat>{"patches":[{"op":"link","target":{"table":"entities","id":"e1","path":""},"value":{
  "id":"e1","branchId":"br_cua_ke_khac","kind":"concept","ten":"Lạ","tickSinh":0,"aspects":{}}}]}</CapNhat>`;
        ...
        expect((kq.patches[0]?.value).branchId).toBe('br_that');
    });
});
...
describe('[BB] Phase 12 — gói save không được bỏ sót bảng nào của WorldState', () => {
    ...
    it('mọi Map trong WorldState đều có một trường tương ứng trong gói export', () => {
        const s = taoState(moTheGioiTrong(KhoiTaoWorldSchema.parse({ seed: 'x', worldId: 'w', branchId: 'b' })).world);
...
describe('[BB] Phase 12 — audit riêng tư', () => {
    it('mật khẩu proxy không bao giờ ra khỏi máy: strip theo TÊN KHÓA, mọi độ sâu', () => {
        ...
        for (let i = 0; i < 300; i++) {
            ...
            for (const k of KHOA_SECRET)
                expect(sach.toLowerCase()).not.toContain(`"${k.toLowerCase()}"`);
        }
    });
    it('cheMatKhau che cả bốn điểm cuối, kể cả endpoint rerank', () => {
        ...
        for (const m of ['a1', 'b2', 'c3', 'd4'])
            expect(che).not.toContain(m);
    });
    it('prompt gửi model không chứa trường riêng tư nào', () => {
        ...
        for (const k of KHOA_CAM_RA_NGOAI)
            expect(toanBo).not.toContain(k);
...
describe('[BB] Phase 12 — soak ngắt mạch', () => {
    it('ba lần hỏng liên tiếp đóng cổng chơi, và một cấu hình đủ vẫn không cứu được', () => {
        ...
        let mach = MACH_MOI;
        for (let i = 0; i < 3; i++)
            mach = machSauKhiHong(mach, 'TIMEOUT', 'quá hạn');
    ...
    it('soak 200 lần hỏng: trạng thái mạch không tràn số và không tự mở lại', () => {
        let mach = MACH_MOI;
        for (let i = 0; i < 200; i++)
            mach = machSauKhiHong(mach, 'HTTP_500', 'proxy chết');
...
describe('[BB] Phase 12 — cổng quét mã nguồn', () => {
    ...
    it('[BB] ADR-0055 — store và ui KHÔNG được nhập hạt giống fixture', () => {
        ...
        for (const p of TAT_CA_NGUON) {
            const t = ten(p);
            if (!t.startsWith('src/store/') && !t.startsWith('src/ui/'))
                continue;
            const src = readFileSync(p, 'utf8');
            if (/\b(moThuGioi|eventGieoTheGioi)\b/.test(src))
                pham.push(t);
        }
        expect(pham).toEqual([]);
    });
    ...
    it('không file nào ghi log mật khẩu bằng console', () => {
        const pham = TAT_CA_NGUON.filter((p) => /console\.\w+\([^)]*(proxyPassword|apiKey|password)/i.test(readFileSync(p, 'utf8')));
        expect(pham.map(ten)).toEqual([]);
    });
});
