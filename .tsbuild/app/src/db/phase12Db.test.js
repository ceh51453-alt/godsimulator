 ...
 */
function vanCoNoiDung(branchId, soTick) {
    ...
    if (nen)
        expect(apDungEvent(s, nen, log).ok).toBe(true);
...
describe('[BB] Phase 12 — quản lý ván', () => {
    ...
    it('đổi tên rồi liệt kê lại thấy tên mới; tên rỗng về lại nhãn mặc định', async () => {
        ...
        expect((await danhSachSave(db))[0]?.ten).toBe(nhanMacDinh(state.world.tick, state.world.playerState.mode));
    });
...
describe('[BB] Phase 12 — backup và phục hồi', () => {
    it('xuất rồi nhập lại giữ nguyên state hash, đi qua JSON thật', async () => {
        ...
        if (!x.ok)
            return;
        const qua = JSON.parse(JSON.stringify(x.value));
...
describe('[BB] Phase 12 — migration mọi version được hỗ trợ', () => {
    ...
    it('chạy mọi migration trên cơ sở dữ liệu trắng là an toàn và idempotent', async () => {
        ...
        if (lan2.ok)
            expect(lan2.value.daBoQua).toBe(true);
    });
    /**
...
describe('[BB] Phase 12 — soak hiệu năng và bộ nhớ', () => {
    it('save 10.000 nhịp: ghi được, mở lại đúng hash, invariant toàn cục sạch', async () => {
    ...
    it('mười ngàn nhịp không làm event log hay entity nở vô hạn', () => {
