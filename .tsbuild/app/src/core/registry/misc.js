...
export const GAPS_DUNG_SAN = gapNguon.map((g) => ({
    id: g.id,
    ten: g.ten,
    moTa: g.moTa,
...
export const ACTIONS_DUNG_SAN = actionNguon.map((a) => ({
    id: a.id,
    ten: a.ten,
    moTa: a.moTa,
    tangApDung: a.tang,
...
const endingNguon = [
    ...
    },
    {
        id: 'tu_van_hanh',
        ten: 'Tự Vận Hành',
        vanBan: 'tuSinhSuKien gần chạm trần suốt nhiều kỷ nguyên. Thế giới không cần bạn nữa — và đó là kết cục tốt nhất.',
    },
];
export const ENDINGS_DUNG_SAN = endingNguon.map((e) => ({
    id: e.id,
    ten: e.ten,
    vanBan: e.vanBan,
...
export const METRICS_DUNG_SAN = metricNguon.map((m) => ({
    id: m.id,
    ten: m.ten,
    moTa: m.moTa,
...
export const PROFILES_DUNG_SAN = profileNguon.map((p) => ({
    id: p.id,
    ten: p.ten,
    profile: p,
...
export const STORY_KINDS_DUNG_SAN = storyNguon.map((s) => ({
    id: s.id,
    ten: s.ten,
    moTa: s.moTa,
    nhipMacDinh: s.nhip,
...
export const MECHANISMS_DUNG_SAN = mechNguon.map((m) => ({
    id: m.id,
    ten: m.ten,
    moTa: m.moTa,
 ...
 */
const sp = (table, path) => ({ table, path });
const A = 'aspects';
const wpNguon = [
    {
    ...
    },
    {
        ...
        moTa: 'Thần không do người chơi nhập vẫn đáp áp lực, trả lời cầu nguyện, theo đuổi việc dài hơi ' +
            'và giữ domain.',
        phamVi: 'entity',
        nhip: { unit: 'tick', every: 2 },
        buoc: 3,
...
export const WORLD_PROCESSES_DUNG_SAN = wpNguon.map((w) => {
...
export const WORLD_PROCESS_IDS_THAN = ['divine_alienation', 'prayer_flow', 'divine_agency'];
/** Tiến trình tầng Phàm Nhân — Phase 7 (Phần 70). */
export const WORLD_PROCESS_IDS_PHAM = ['mortal_daily', 'household_lifecycle'];
/** Tiến trình tự sự — Phase 8 (Phần 28, 30). */
export const WORLD_PROCESS_IDS_TRUYEN = ['storyline_beat'];
