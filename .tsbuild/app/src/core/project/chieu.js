...
function mucTheoNguong(x, nguong, macDinh) {
    for (const [n, nhan] of nguong)
        if (x >= n)
            return nhan;
    return macDinh;
}
 ...
 */
function dinhTinhHoa(ten, o) {
    const soCua = (k, trong) => {
        const goc = trong ? o[trong] : o;
        const v = goc?.[k];
        return typeof v === 'number' ? v : 0;
    };
    if (ten === 'kinh_te') {
        return {
            doiSong: mucTheoNguong(soCua('thieuHut'), [
                [0.6, 'doi_kem'],
                [0.3, 'thieu_an'],
                [0.05, 'vua_du'],
            ], 'sung_tuc'),
            nghe: mucTheoNguong(soCua('kyThuat'), [
                [60, 'tinh_xao'],
                [25, 'thanh_thao'],
            ], 'tho_so'),
        };
    ...
    if (ten === 'y_te') {
        return {
            benhTat: mucTheoNguong(soCua('tyLeMac'), [
    ...
    if (ten === 'an_ninh') {
        const dangDanh = Array.isArray(o['xungDot'])
            ? o['xungDot'].some((x) => x.tickKetThuc === null || x.tickKetThuc === undefined)
            : false;
        return {
            anToan: dangDanh
                ? 'dang_co_chien_su'
                : mucTheoNguong(soCua('deDoa'), [
                    [60, 'ai_cung_thu_the'],
                    [30, 'bat_an'],
                ], 'yen'),
        };
    ...
    if (ten === 'sinh_thai') {
        return {
            dat: mucTheoNguong(soCua('suyThoai'), [
                [0.6, 'kiet'],
                [0.3, 'bac_mau'],
            ], 'con_tot'),
        };
    ...
    if (ten === 'van_hoa') {
        ...
        const tt = Array.isArray(o['tapTuc']) ? o['tapTuc'] : [];
        return { tapTuc: tt.map((t) => t.ten ?? '').filter((x) => x !== '') };
    }
    if (ten === 'duong') {
        return {
            thongSuot: o['thongSuot'] === true,
            loi: mucTheoNguong(soCua('chatLuong'), [
                [60, 'de_di'],
                [25, 'di_duoc'],
            ], 'kho_di'),
        };
 ...
 */
function hoaThanDangNgu(e) {
    const av = e ? docAspect(e, 'avatar') : undefined;
    if (!av || av.daThucTinh === true)
        return null;
    return typeof av.thanTheId === 'string' && av.thanTheId !== '' ? { thanTheId: av.thanTheId } : null;
}
function dungNgocCanh(state, modeNguoiChoi, chuTheId) {
    ...
    const mode = hoaThan ? 'pham_nhan' : modeNguoiChoi;
    const neoViTri = hoaThan?.thanTheId ?? chuTheId;
    const vungChuTheId = neoViTri ? vungCua(state, neoViTri) : null;
    const lienHe = new Set();
    if (chuTheId) {
        for (const lk of state.links.values()) {
            if (lk.tickDut !== null || lk.trongSo <= 0)
                continue;
            if (lk.tuId === chuTheId)
                lienHe.add(lk.denId);
            if (lk.denId === chuTheId)
                lienHe.add(lk.tuId);
        }
    }
    ...
    if (hoaThan) {
        for (const lk of state.links.values()) {
            if (lk.tickDut !== null || lk.trongSo <= 0)
                continue;
            if (lk.tuId === hoaThan.thanTheId)
                lienHe.add(lk.denId);
            if (lk.denId === hoaThan.thanTheId)
                lienHe.add(lk.tuId);
        }
    ...
    const domainVung = new Set();
    if (mode === 'than' && chuThe) {
        const ven = docAspect(chuThe, 'venerable');
        for (const [vung, mat] of Object.entries(ven?.matDoDen ?? {})) {
            if (mat > 0)
                domainVung.add(vung);
        }
    }
    const soul = chuThe ? docAspect(chuThe, 'soul') : undefined;
    const mortal = chuThe ? docAspect(chuThe, 'mortal') : undefined;
...
function suongMuCua(nc, e) {
    ...
    if (nc.mode === 'than') {
        ...
        if (nc.lienHeTrucTiep.has(e.id))
            return 'ro';
        if (nc.domainVungIds.has(e.id))
            return 'ro';
        const vung = vungCua(nc.state, e.id);
        if (vung && nc.domainVungIds.has(vung))
            return 'mo';
        // Được nhắc bởi tín đồ nhưng ngoài lãnh địa → tin đồn.
        if (nc.domainVungIds.size > 0)
            return 'tin_don';
        return 'mu';
    }
    // pham_nhan
    ...
    if (nc.vungChuTheId !== null) {
        if (e.id === nc.vungChuTheId)
            return 'ro';
        const vung = vungCua(nc.state, e.id);
        if (vung !== null && vung === nc.vungChuTheId)
            return 'ro';
    }
...
function locAspect(nc, e, muc) {
    const ra = {};
    for (const [ten, duLieu] of Object.entries(e.aspects)) {
        if (duLieu === null || duLieu === undefined)
            continue;
        // ── lawful ──
        if (ten === 'lawful') {
            const l = duLieu;
        ...
        if (ten === 'provenance') {
            ...
            const p = duLieu;
            if (nc.chuTheId !== null && p['actorId'] === nc.chuTheId)
                ra[ten] = p;
            continue;
        }
        ...
        if (ASPECT_NEN.has(ten)) {
            ...
            const dinhTinh = dinhTinhHoa(ten, duLieu);
            // Thần nhìn từ trên: thấy quy mô, không thấy sổ sách của từng nhà.
            if (nc.mode === 'than' && muc === 'ro') {
                ra[ten] = { ...dinhTinh, ...quyMoChoThan(ten, duLieu) };
                continue;
            }
            if (Object.keys(dinhTinh).length > 0)
                ra[ten] = dinhTinh;
            continue;
        }
...
function dienGiaiCuaVung(lawful, vungId) {
    const ds = (lawful['dienGiai'] ?? []);
    if (ds.length === 0)
        return { noiDung: '', doLech: 0 };
...
function chieuConcept(nc, e, muc) {
    const c = (docAspect(e, 'conceptual') ?? {});
    const day = nc.mode === 'sang_the' || (nc.mode === 'than' && muc === 'ro');
    return Object.freeze({
        id: e.id,
        ten: e.ten,
        // [BB] Phàm nhân KHÔNG thấy trọng số.
        trongSo: day ? (c['trongSo'] ?? 0) : null,
        giaiDoan: day ? (c['giaiDoan'] ?? null) : (c['giaiDoan'] ?? null),
        sacThai: day ? (c['sacThai'] ?? {}) : null,
        mucRo: muc,
    });
}
/**
 ...
 */
function phamNhanBietKhaiNiem(e) {
    const c = docAspect(e, 'conceptual');
    return c?.giaiDoan !== 'hu_danh';
}
function dongTuKhaDung(mode) {
    napDungSan();
    ...
    if (mode !== 'sang_the')
        return [];
    return R.verb.tatCa().map((v) => Object.freeze({
        id: v.id,
        ten: v.ten,
        moTa: v.moTa ?? '',
 ...
 */
function chieuLinks(state, thay) {
    const ra = [];
    ...
    for (const id of ids) {
        ...
        if (!lk)
            continue;
        if (!thay.has(lk.tuId) || !thay.has(lk.denId))
            continue;
 ...
 */
function chieuMachTruyen(nc, thay) {
    const ra = [];
    ...
    for (const id of ids) {
        ...
        if (nc.mode !== 'sang_the' && !m.nguoiChoiBiet && !trongCuoc)
            continue;
        ra.push(Object.freeze({
            id: m.id,
            ten: m.ten,
            ...
            phucButChuaTra: Object.freeze(m.phucBut
...
function chieuLuatNen(nc, thay) {
    ...
    for (const truc of TRUC_NEN) {
        ...
        if (nc.mode === 'than' && (!coTen || nenId === null || !thay.has(nenId)))
            continue;
        ra.push(Object.freeze({
            truc,
            ten: NHAN_TRUC_NEN[truc],
            trangThai: coTen ? 'co_ten' : 'vo_danh',
...
function chieuDiBiet(nc) {
    ...
    for (const kv of nc.state.loreExpectations.values()) {
        ...
        if (kv.trangThai === 'da_thoa')
            o.thoa++;
        else if (kv.trangThai === 'da_lech')
            o.lech++;
        else if (kv.trangThai === 'bat_kha')
            o.batKha++;
        else
            o.cho++;
        dem.set(nguon, o);
    }
    return Object.freeze([...dem.entries()]
 ...
 */
function chieuPhucBut(nc, machBiet) {
...
function chieuLoiCau(nc) {
    ...
    for (const p of nc.state.prayers.values()) {
        ...
        if (nc.mode === 'than' && p.thanNhanId !== null && p.thanNhanId !== nc.chuTheId)
            continue;
...
function chieuLoHong(nc) {
    if (nc.mode !== 'sang_the')
        return Object.freeze([]);
    const tick = nc.state.world.tick;
    const ra = [];
    for (const g of nc.state.gaps.values()) {
        if (g.trangThai === 'da_giai')
            continue;
        ra.push(Object.freeze({
            id: g.id,
            moTa: g.moTa,
...
export function chieu(state, mode, chuTheId) {
    napDungSan();
    ...
    for (const id of ids) {
        ...
        if (nc.mode === 'pham_nhan' && e.kind === 'concept' && !phamNhanBietKhaiNiem(e))
            muc = 'mu';
        if (muc === 'mu') {
            mu.push(id);
            continue;
        ...
        if (muc === 'ro')
            ...
            tinDon.push(id);
        if (e.kind === 'law')
            laws.push(chieuLaw(nc, e, muc));
        if (e.kind === 'concept')
            concepts.push(chieuConcept(nc, e, muc));
    }
    /**
    ...
    const thay = new Set(entities.keys());
    const mach = chieuMachTruyen(nc, thay);
    const machBiet = new Set(mach.map((m) => m.id));
    return Object.freeze({
        mode,
        mucChieu: nc.mode,
