...
function theGioiDaGieo() {
    ...
    if (!r.ok)
        throw new Error(`gieo thế giới thất bại: ${r.errors[0]?.message}`);
    return { state, log };
}
let state;
let log;
...
describe('ba cửa vào — Phần 17.4', () => {
    ...
    it('[BB] 6.3 — không thực thể mồ côi sau khi gieo', () => {
        ...
        for (const [id, n] of bac)
            expect(n, `'${id}' mồ côi`).toBeGreaterThan(0);
    });
    ...
    it('hai vùng diễn giải cùng một luật LỆCH nhau — nền của Tầng 2 bắt buộc sai', () => {
        const l = state.entities.get('law_thuong')?.aspects['lawful'];
        const ds = l.dienGiai ?? [];
...
describe('[BB] Phần 18.2 — ba quy tắc cứng chống rò rỉ', () => {
    ...
    it('[BB] kẽ hở engine đã biết KHÔNG lọt xuống tầng dưới', () => {
        for (const mode of ['than', 'pham_nhan']) {
    ...
    it('[BB] phàm nhân CHỈ thấy banTinhTinDoTin — và bản đó NGƯỢC với sự thật', () => {
        const v = chieu(state, 'pham_nhan', 'mortal_1');
        const ven = v.entities.get('deity_1')?.aspects['venerable'];
...
describe('[BB] cùng một Event được chiếu KHÁC NHAU ở ba tầng', () => {
    ...
    it('visibilityHash ổn định giữa hai lần chiếu cùng state', () => {
        expect(chieu(state, 'pham_nhan', 'mortal_1').visibilityHash).toBe(chieu(state, 'pham_nhan', 'mortal_1').visibilityHash);
    });
...
describe('bopMeo — sai CÓ CẤU TRÚC, không phải mơ hồ', () => {
    const ts = { chang: 3, triThuc: 30, thienVi: 'phong_dai', seed: 's' };
    it('deterministic', () => {
        expect(bopMeo('Đấng Tẩy Uế', 'Ngài hiền lành.', ts)).toEqual(bopMeo('Đấng Tẩy Uế', 'Ngài hiền lành.', ts));
    });
    ...
    it('số bị phóng đại theo hướng có lợi cho phe kể', () => {
        const nhieuLan = Array.from({ length: 40 }, (_, i) => meoSo(1000, { ...ts, seed: `s${i}`, thienVi: 'phong_dai' }));
...
describe('[BB] hiện diện ban đầu đi qua Event, không ghi World trực tiếp', () => {
    ...
    it('[BB] 78.7 — suc của domain do ENGINE quyết, không do người chơi khai', () => {
        ...
        const d = state.entities.get(r.value.chuTheId)?.aspects['domain'];
    ...
    it('[BB] 78.7 — thần mới chưa ai thờ; không có "sức mạnh khởi đầu"', () => {
        ...
        if (!r.ok)
            return;
        apDungEvent(state, r.value.events[0], log);
        const ven = state.entities.get(r.value.chuTheId)?.aspects['venerable'];
    ...
    it('[BB] 78.8 — kỹ năng vượt ngân sách thành mục tiêu, không được cấp sẵn', () => {
        ...
        if (!r.ok)
            return;
        apDungEvent(state, r.value.events[0], log);
        const m = state.entities.get(r.value.chuTheId)?.aspects['mortal'];
        expect(Object.keys(m.kyNang ?? {})).toHaveLength(3);
        for (const v2 of Object.values(m.kyNang ?? {}))
            expect(v2).toBeLessThan(40);
    ...
    it('[BB] 78.8 — vật chưa tồn tại không vào tay ngay', () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.value.diff.khongCapThang.join(' ')).toContain('phải đi tìm');
    });
...
describe('tick engine — Phần 24', () => {
    ...
    it('[BB] 10.1 — doLech TĂNG theo thế hệ; diễn giải đúng 100% là bug', () => {
        ...
        const sau = doLechCua();
        expect(sau[0]).toBeGreaterThan(truoc[0]);
        expect(sau[1]).toBeGreaterThan(truoc[1]);
    });
    it('trọng số khái niệm tích theo áp lực từ diễn giải lệch', () => {
        const trongSo = () => {
            const c = state.entities.get('concept_o_ue')?.aspects['conceptual'];
            return c.trongSo ?? 0;
        };
        const truoc = trongSo();
        for (let i = 0; i < 5; i++) {
    ...
    it('toàn bộ lát dọc replay được từ event log', () => {
        ...
        if (lai.ok)
            expect(lai.value.hashCuoi).toBe(hashTruoc);
    });
});
