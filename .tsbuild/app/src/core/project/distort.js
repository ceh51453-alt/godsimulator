...
export function mucMeo(chang, triThuc) {
    if (chang <= 0)
        return 0;
...
export function meoTen(ten, ts) {
    const m = mucMeo(ts.chang, ts.triThuc);
    if (m < 0.2 || ten.length === 0)
        return ten;
    ...
    return `${tienTo[rng.nguyen(tienTo.length)]}${ten}`;
}
/** Số bị phóng đại hoặc giảm nhẹ theo hướng có lợi cho phe kể. */
export function meoSo(so, ts) {
    const m = mucMeo(ts.chang, ts.triThuc);
    if (m === 0)
        return so;
...
export function meoThoiGian(tickCu, tickHienTai, ts) {
    const m = mucMeo(ts.chang, ts.triThuc);
    const cach = Math.max(0, tickHienTai - tickCu);
    if (m < 0.25)
        return `${cach} nhịp trước`;
    if (m < 0.5)
        return cach > 200 ? 'nhiều đời trước' : 'không lâu trước đây';
    if (m < 0.75)
        return 'thuở ông bà còn kể lại';
    return 'từ thời chưa ai nhớ nổi';
}
/** Động cơ bị gán nhầm — nguồn kịch tính chính của tầng Thần. */
export function meoDongCo(dongCoThat, ts) {
    const m = mucMeo(ts.chang, ts.triThuc);
    if (m < 0.35)
        return dongCoThat;
    ...
    return gan[rng.nguyen(gan.length)];
}
/**
 ...
 */
export function meoMoTa(moTa, ts) {
    const m = mucMeo(ts.chang, ts.triThuc);
    if (m < 0.25 || moTa.length === 0)
        return moTa;
    ...
    if (m >= 0.7) {
        ...
        const giu = cau[rng.nguyen(cau.length)];
        const them = [
        ...
        return `${giu} ${them[rng.nguyen(them.length)]}`;
    }
    if (m >= 0.45) {
...
export function bopMeo(ten, moTa, ts) {
    return {
        ten: meoTen(ten, ts),
