...
const CHI_THI = /(?:@entity|thực\s*thể|entity|nhân\s*vật|character|npc|thần|deity|địa\s*danh|place|location|khái\s*niệm|concept)\s*(?:\(([^)]+)\)|\[([^\]]+)\])?\s*[:：]\s*([^\r\n;|]{2,80})/iu;
const NHAN_VAT = /\b(?:nhân\s*vật|character|npc|tuổi|age|gender|giới\s*tính|cô ấy|anh ấy|nàng|hắn|ông ấy|bà ấy|protagonist|antagonist)\b/iu;
const THAN = /\b(?:thần|thánh|deity|god|goddess|divine|神|女神)\b/iu;
const NOI = /\b(?:địa\s*danh|nơi\s*chốn|thành\s*phố|vương\s*quốc|núi|sông|đảo|place|location|city|realm|kingdom|城|国)\b/iu;
...
function kindTuNhan(nhan, noiDung) {
    const ca = `${nhan}\n${noiDung}`;
    if (THAN.test(ca))
        return 'deity';
    if (NOI.test(ca))
        return 'place';
    if (KHAI_NIEM.test(ca))
        return 'concept';
    return 'mortal';
}
/** Đọc một neo rõ ràng; không đoán mọi entry thành nhân vật. */
export function docNeoLore(entry) {
    if (entry.triHoanHienThuc)
        return null;
    ...
    if (m !== null) {
        ...
        if (!coHinhDang || t.length < 2 || TEN_CHUNG.test(t))
            return null;
        ten = t;
        nhan = entry.noiDung;
    }
    if (ten.length < 2 || TEN_CHUNG.test(ten))
        return null;
...
function aspects(kind, tick, eventId) {
    ...
    if (kind === 'place')
        return { provenance, spatial: SpatialSchema.parse({}) };
 ...
 */
export function vatChatHoaLorebook(lorebook, state, eventId) {
    if (!lorebook.bat)
        return [];
    const daCo = new Set();
    for (const e of state.entities.values()) {
        ...
        for (const a of e.aliases)
            daCo.add(a.toLocaleLowerCase());
    }
    const ra = [];
    for (const entry of [...lorebook.entries].sort((a, b) => a.order - b.order || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))) {
        if (entry.trangThai !== 'hoat_dong')
            continue;
        const neo = docNeoLore(entry);
        if (neo === null || daCo.has(neo.ten.toLocaleLowerCase()))
            continue;
        const id = `lore.${bam(`${lorebook.id}|${entry.id}|${neo.kind}|${neo.ten}`)}`;
        const tags = ['lorebook', `lorebook_${tag(lorebook.id)}`, `entry_${tag(entry.id)}`, tag(neo.ten)].filter((x) => x !== '');
        ra.push(EntitySchema.parse({
            id,
