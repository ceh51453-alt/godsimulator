...
export const CACH_DAP_DI_HOA = ['chap_nhan', 'chong_lai', 'mac_ca', 'phan_than'];
...
export const NHAN_TRANG_THAI_DOMAIN = Object.freeze({
    held: 'đang giữ',
 ...
 */
export function khoangCachBanTinh(a, b) {
    let max = 0;
 ...
 */
export function hinhHienTai(coreSelf, followerImage, hienThanh) {
 ...
 */
export function trangThaiSuyRa(d) {
    if (d.trangThai === 'merged' || d.trangThai === 'transformed' || d.trangThai === 'fragmented') {
        return d.trangThai;
    }
    if (d.suc <= 0)
        return d.neoTaiChiem.length > 0 ? 'reclaimable' : 'lost';
    if (d.doiThuIds.length > 0)
        return 'contested';
    if (d.suc < 8)
        return 'dormant';
    return 'held';
