 ...
 */
export const BAC_NGHE = ['hoc_viec', 'tho_ban', 'tho_ca', 'bac_thay'];
export const NHAN_BAC_NGHE = Object.freeze({
    hoc_viec: 'học việc',
    tho_ban: 'thợ bạn',
    tho_ca: 'thợ cả',
    bac_thay: 'bậc thầy',
 ...
 */
export const NHAN_NGHE = Object.freeze({
...
export const SinhKeSchema = z
    ...
    .prefault({});
// ─────────────────────────────────────────── hộ (70.2)
export const VAI_TRONG_HO = ['chu_ho', 'ban_doi', 'con', 'nguoi_gia', 'nguoi_o', 'khach_tru'];
export const NHAN_VAI_HO = Object.freeze({
    chu_ho: 'chủ nhà',
    ban_doi: 'bạn đời',
    con: 'con',
...
export const NHAN_VI_TRI = Object.freeze({
    dau: 'đầu',
    than: 'mình',
    tay_trai: 'tay trái',
    tay_phai: 'tay phải',
...
export function bacTheoKyNang(kyNang) {
    if (kyNang >= NGUONG_BAC.bac_thay)
        return 'bac_thay';
    if (kyNang >= NGUONG_BAC.tho_ca)
        return 'tho_ca';
    if (kyNang >= NGUONG_BAC.tho_ban)
        return 'tho_ban';
    return 'hoc_viec';
}
/**
...
export function viecBiChan(t) {
    ...
    if (t.viTri === 'chan_trai' || t.viTri === 'chan_phai') {
        ra.push('di_xa');
        if (nang >= 0.5)
            ra.push('chay', 'dung_lau');
    }
    if (t.viTri === 'tay_trai' || t.viTri === 'tay_phai') {
        ra.push('mang_vac');
        if (nang >= 0.5)
            ra.push('che_tac', 'danh_nhau');
    }
    if (t.viTri === 'dau') {
        ra.push('hoc');
        if (nang >= 0.5)
            ra.push('thuong_luong');
    }
    if (t.loai === 'benh' || t.loai === 'nhiem_doc')
        ra.push('gan_nguoi_khac');
    if (t.loai === 'kiet_suc')
        ra.push('lam_viec_nang');
    return ra;
}
/** Câu người ta nói về vết thương này — [BB] 56.2, không con số. */
export function keVeThuongTich(t) {
