...
import { RerankConfigSchema, RerankQuerySchema, RerankCandidateSchema, RerankResultSchema, RerankCacheEntrySchema, RetrievalRunSchema, RetrievalEvalCaseSchema, doCauHinhRerank, CAU_HINH_HEURISTIC, RERANK_MODES, RERANK_TASKS, } from './rerank.js';
...
describe('[BB] 77.3 — candidate mang projectedText, không mang nội dung gốc', () => {
    ...
    it('initialRank bắt đầu từ 1, không phải 0', () => {
        expect(RerankCandidateSchema.safeParse({
            chunkId: 'c',
            sourceType: 's',
            projectedText: '',
            initialRank: 0,
...
describe('fixture retrieval-eval đủ bốn loại chunk', () => {
    it('có chunk đúng, nhiễu, trùng nguồn và chunk cấm', () => {
        ...
        for (const c of CHUNKS_EVAL)
            theoNguon.set(c.nguonId, (theoNguon.get(c.nguonId) ?? 0) + 1);
