import { cong, dat, docAspect, kep, laoDong, lam, langGieng, moiNoiChon, taoBanGhi, tongCohort, PHAN_KHO, } from './tienIch.js';
...
export function chaySanXuat(nc) {
    const patches = [];
    const suKien = [];
    for (const { id, e } of moiNoiChon(nc.state)) {
        ...
        if (dan <= 0) {
            ...
            if (hao > 0) {
                ...
                patches.push(dat(nc, id, 'aspects.kinh_te.soCai', { sanXuat: 0, tieuThu: 0, nhap: 0, xuat: 0, hao, thue: 0 }));
            }
            continue;
        }
        ...
        if (dKho !== 0)
            patches.push(cong(nc, id, 'aspects.kinh_te.kho.luongThuc', dKho));
        if (layGo > 0) {
            patches.push(cong(nc, id, 'aspects.kinh_te.kho.vatLieu', layGo));
            patches.push(cong(nc, id, 'aspects.sinh_thai.taiNguyen.rung', -layGo));
        }
        if (layThu > 0)
            patches.push(cong(nc, id, 'aspects.sinh_thai.taiNguyen.thu', -lam(layThu)));
        if (layCa > 0)
            patches.push(cong(nc, id, 'aspects.sinh_thai.taiNguyen.ca', -lam(layCa)));
        ...
        patches.push(dat(nc, id, 'aspects.kinh_te.soCai', {
            sanXuat,
            tieuThu: an,
            nhap: 0,
            xuat: 0,
...
const NGUONG_CHO = 20;
export function chayTraoDoi(nc) {
    const patches = [];
    const suKien = [];
    /** Tổng thay đổi kho theo vùng — phát một patch mỗi vùng để tổng luôn khớp 0. */
    const dLuongThuc = new Map();
    const dVatLieu = new Map();
    const themVao = (m, id, x) => {
            ...
            for (const [hang, ban] of [
                ...
                if (hang === 'luongThuc' && traDuoc < luong * 0.5) {
                    ...
                    if (!nc.state.debts.has(noId)) {
                        patches.push(taoBanGhi(nc, 'debts', noId, DebtRowSchema.parse({
    ...
    for (const noId of [...nc.state.debts.keys()].sort((a, b) => (a < b ? -1 : 1))) {
        ...
        if (!no || no.status !== 'open')
            continue;
        if (no.dueTick === null || nc.tick < no.dueTick)
            continue;
        const eNo = nc.state.entities.get(no.debtorId);
        const ktNo = docAspect(eNo, 'kinh_te');
        ...
        if (coTra >= no.amount && nc.state.entities.has(no.creditorId)) {
            ...
            suKien.push({
                loai: 'vo_no',
                mucDo: 'lon',
                moTa: `${nc.state.entities.get(no.debtorId)?.ten ?? no.debtorId} không trả nổi món nợ với ${nc.state.entities.get(no.creditorId)?.ten ?? no.creditorId}.`,
...
export function chayDinhCu(nc) {
    ...
    for (const { id, e } of moiNoiChon(nc.state)) {
        ...
        if (mucNha > 0)
            patches.push(cong(nc, id, 'aspects.kinh_te.haTang.nha', -mucNha));
        if (dungVatLieu > 0) {
    ...
    for (const id of [...nc.state.entities.keys()].sort((a, b) => (a < b ? -1 : 1))) {
        ...
        if (lam(moi) !== lam(d.chatLuong))
            patches.push(dat(nc, id, 'aspects.duong.chatLuong', lam(moi)));
        ...
        if (d.luuLuong > 0)
            patches.push(cong(nc, id, 'aspects.duong.luuLuong', -lam(d.luuLuong * 0.5)));
    }
    return { patches, suKien };
}
