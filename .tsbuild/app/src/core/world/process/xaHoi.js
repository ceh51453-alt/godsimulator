...
export function chayThietChe(nc) {
    const patches = [];
    const suKien = [];
    for (const { id, ins, ten } of moiThietChe(nc)) {
        ...
        for (const vungId of [...ins.vungCaiTriIds].sort((a, b) => (a < b ? -1 : 1))) {
            ...
            const kt = docAspect(eV, 'kinh_te');
            if (!kt || kt.kho.luongThuc <= 0)
                continue;
            ...
            const lay = lam(Math.max(0, Math.min(co * ins.thueSuat * nc.soBuocGop, kt.kho.luongThuc * PHAN_KHO.thue)));
            if (lay <= 0)
                continue;
        ...
        if (thu > 0)
            patches.push(cong(nc, id, 'aspects.institutional.khoCong.luongThuc', lam(thu)));
        // ── ổn định: đói và thuế nặng kéo xuống, kho công đầy kéo lên ──
        let apLuc = 0;
        let soVung = 0;
        for (const vungId of ins.vungCaiTriIds) {
            const kt = docAspect(nc.state.entities.get(vungId), 'kinh_te');
            if (!kt)
                continue;
        ...
        if (lam(onMoi) !== lam(ins.onDinh))
            patches.push(dat(nc, id, 'aspects.institutional.onDinh', lam(onMoi)));
        ...
        if (ins.keVi === 'khong_co' || ins.chucVu.length === 0)
            continue;
        ...
        if (doiNguoi)
            patches.push(dat(nc, id, 'aspects.institutional.chucVu', chucMoi));
    }
...
export function chayVanHoa(nc) {
    ...
    for (const { id, e } of moiNoiChon(nc.state)) {
        ...
        const tapTuc = vh.tapTuc.map((t) => ({ ...t }));
        let doi = false;
        ...
        if (doi)
            patches.push(dat(nc, id, 'aspects.van_hoa.tapTuc', conLai));
    }
    return { patches, suKien };
}
// ─────────────────────────────────────────── conflict_security
...
export function chayXungDot(nc) {
    const patches = [];
    const suKien = [];
    const n = nc.soBuocGop;
    for (const { id, e } of moiNoiChon(nc.state)) {
        ...
        for (const lg of hangXom) {
            const ktb = docAspect(nc.state.entities.get(lg.noiId), 'kinh_te');
        ...
        if (lam(deMoi) !== lam(an.deDoa))
            patches.push(dat(nc, id, 'aspects.an_ninh.deDoa', lam(deMoi)));
        // ── nổ ra ──
        if (deMoi >= 60 && dangDanh.length === 0 && !coHoaUoc && dan > 30) {
            const dich = hangXom
                .map((lg) => {
                const ktb = docAspect(nc.state.entities.get(lg.noiId), 'kinh_te');
                const dcb = docAspect(nc.state.entities.get(lg.noiId), 'dan_cu');
        ...
        if (dangDanh.length === 0)
            continue;
        const xungDotMoi = an.xungDot.map((x) => ({ ...x }));
        let thuongVong = 0;
        for (const x of xungDotMoi) {
            if (x.tickKetThuc !== null)
                continue;
            const mat = Math.min(Math.floor((dan * x.cuongDo * 0.0015 + 1) * n), Math.floor(dan * TRAN_THUONG_VONG));
        ...
        if (thuongVong > 0) {
            ...
            if (that > 0) {
                if (tuAdult > 0)
                    patches.push(cong(nc, id, 'aspects.dan_cu.cohort.adult', -tuAdult));
                if (tuYouth > 0)
                    patches.push(cong(nc, id, 'aspects.dan_cu.cohort.youth', -tuYouth));
