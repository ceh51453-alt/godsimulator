...
export function chayDiHoa(nc) {
    ...
    for (const { id, e } of moiThan(nc)) {
        ...
        for (const [vungId, mat] of Object.entries(ven?.matDoDen ?? {})) {
            ...
            const kt = docAspect(v, 'kinh_te');
            const an = docAspect(v, 'an_ninh');
            const yt = docAspect(v, 'y_te');
            khoKhan +=
        ...
        const ap = doApLuc({ ...bn, followerImage: anhMoi });
        ...
        if (ap.distortion <= nc.tuning.than.nguongDiHoa || ap.trucNang === null || dangMo.length > 0)
            continue;
 ...
 */
export function chayCauNguyen(nc) {
    const patches = [];
    const suKien = [];
    // ── sinh ──
    let daSinh = 0;
    ...
    for (const p of loiCauQuaHan(nc.state, nc.tick)) {
        ...
        if (p.thanNhanId && nc.state.entities.has(p.thanNhanId) && p.soNguoi >= 20) {
            ...
            suKien.push({
                ...
                mucDo: 'lon',
                moTa: `${p.soNguoi} người ở ${nc.state.entities.get(p.nguoiCauId)?.ten ?? p.nguoiCauId} ` +
 ...
 */
export function chayThanNpc(nc) {
    const patches = [];
    const suKien = [];
    ...
    for (const { id, e } of moiThan(nc)) {
        ...
        if (dsCau.length > 0) {
            ...
            if (cau) {
                ...
                const cach = ['ban_phuoc', 'lam_ngo', 'trung_phat', 'dau_hieu', 'tra_gia'][rng.softmax(diem, nc.tuning.npc.nhietDoSoftmax * 100)];
                if (cach) {
                    patches.push({
                        op: 'set',
                    ...
                    suKien.push({
                        ...
                        moTa: `${e.ten} ${HAU_QUA_TRA_LOI[cach].nhan.toLowerCase()} lời cầu từ ${nc.state.entities.get(cau.nguoiCauId)?.ten ?? cau.nguoiCauId}.`,
                        tienTrinhId: 'divine_agency',
        ...
        for (const d of rs.doi) {
            if (d.den !== 'lost' && d.den !== 'reclaimable' && d.den !== 'dormant')
                continue;
            suKien.push({
                loai: `domain_${d.den}`,
 ...
 */
function duAnCuaThan(nc, id, e, suKien) {
    const patches = [];
    ...
    if (conChay.length < 2) {
        const ung = ungVienDuAn(nc.state, id).filter((u) => !sau.some((p) => p.status !== 'completed' && p.goal === u.goal));
        if (ung.length > 0) {
...
function chonCachDapNpc(bn, rng, nhietDo) {
