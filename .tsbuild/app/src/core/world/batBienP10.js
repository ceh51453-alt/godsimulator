...
let daNap = false;
export function napBatBienPhase10() {
    if (daNap)
        return;
    daNap = true;
...
function moiEntry(s) {
    ...
    for (const id of [...s.lorebooks.keys()].sort((a, b) => (a < b ? -1 : 1))) {
        ...
        for (const e of lb.entries)
            ra.push({ lbId: lb.id, nguon: lb.nguon, e });
    }
    return ra;
}
function dangKyTatCa() {
        ...
        kiem: (s) => {
            ...
            for (const { lbId, nguon, e } of moiEntry(s)) {
                ...
                for (const h of e.lichSu) {
                    if (h.boiAi === 'nguoi_choi')
                        continue;
                    if (h.op === 'che' || h.op === 'bo_che')
                        continue;
                    vp.push(`Entry "${e.ten}" (${lbId}) bị ${h.boiAi} thực hiện op "${h.op}" ở nhịp ${h.tick}. ` +
    ...
    dangKyInvariant({
        ...
        kiem: (s) => moiEntry(s)
            ...
            .map(({ lbId, e }) => `Entry "${e.ten}" (${lbId}) đã khóa canon nhưng đang bị che bởi ${e.biCheBoiId ?? '?'}.`),
    });
    /**
    ...
    dangKyInvariant({
        ...
        kiem: (s) => {
            ...
            for (const { lbId, e } of moiEntry(s)) {
                if (e.trangThai !== 'bi_che')
                    continue;
                if (e.biCheBoiId === null)
                    vp.push(`Entry "${e.ten}" (${lbId}) bị che mà không khai bị che bởi ai.`);
                if (e.lyDoChe.trim() === '')
                    vp.push(`Entry "${e.ten}" (${lbId}) bị che mà không có lý do.`);
                if (e.tickChe === null)
                    vp.push(`Entry "${e.ten}" (${lbId}) bị che mà không ghi nhịp che.`);
            }
            return vp;
        },
    });
    ...
    dangKyInvariant({
        ...
        canToanCuc: true,
        kiem: (s) => moiEntry(s)
            .filter(({ nguon, e }) => nguon !== 'nguoi_dung' && e.doTinCay >= NGUONG_TIN_CAY_NAP && e.suKienChongLung.length === 0)
            .map(({ lbId, e }) => `Entry "${e.ten}" (${lbId}) có doTinCay ${e.doTinCay} nhưng không trích dẫn sự kiện nào. ` +
    ...
    dangKyInvariant({
        ...
        canToanCuc: true,
        kiem: (s) => {
            const coTen = new Set([...s.substrateLaws.values()].filter((x) => x.trangThai === 'co_ten').map((x) => x.truc));
            const vp = [];
        ...
        kiem: (s) => {
            ...
            for (const id of [...s.substrateLaws.keys()].sort((a, b) => (a < b ? -1 : 1))) {
                ...
                if (ln.trangThai === 'vo_danh' && ln.keHo.length > 0) {
                    vp.push(`Trục "${ln.truc}" còn vô danh mà đã có ${ln.keHo.length} kẽ hở. Chưa ai biết thì chưa ai lách được.`);
                }
                if (ln.trangThai === 'co_ten' && ln.khaiNiemNenId === null) {
                    vp.push(`Trục "${ln.truc}" đã có tên mà không khai khái niệm nền. Luật nền cũng phải tiếp địa (43.3).`);
                }
            }
            return vp;
        },
    ...
    dangKyInvariant({
        ...
        kiem: (s) => {
            ...
            for (const id of [...s.entities.keys()].sort((a, b) => (a < b ? -1 : 1))) {
                const l = s.entities.get(id)?.aspects['lawful'];
                if (l === undefined || l.hieuLuc === undefined)
                    continue;
