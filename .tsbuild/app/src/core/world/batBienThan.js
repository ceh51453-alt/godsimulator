...
let daNap = false;
export function napBatBienTangThan() {
    if (daNap)
        return;
    daNap = true;
...
function dangKyTatCa() {
    ...
    dangKyInvariant({
        ...
        kiem: (s, phamVi) => {
            ...
            for (const id of idCanKiem(s, phamVi)) {
                ...
                const bn = doc(e, 'ban_nga');
                if (!bn)
                    continue;
                for (const d of bn.lichSuLoi) {
                    if (d.eventId === '')
                        xau.push(`'${id}': một lần đổi lõi không ghi eventId`);
                }
    ...
    dangKyInvariant({
        ...
        kiem: (s, phamVi) => {
            const xau = [];
            for (const id of idCanKiem(s, phamVi)) {
                const bn = doc(s.entities.get(id), 'ban_nga');
                if (!bn)
                    continue;
            ...
            for (const id of idCanKiem(s, phamVi)) {
                ...
                for (const [tenAspect, duLieu] of Object.entries(e.aspects)) {
                    if (duLieu === null || typeof duLieu !== 'object')
                        continue;
                    for (const khoa of Object.keys(duLieu)) {
    ...
    dangKyInvariant({
        ...
        kiem: (s, phamVi) => {
            const xau = [];
            for (const id of idCanKiem(s, phamVi)) {
                const dom = doc(s.entities.get(id), 'domain');
                if (!dom)
                    continue;
    ...
    dangKyInvariant({
        ...
        kiem: (s, phamVi) => {
            ...
            for (const id of ids.sort((a, b) => (a < b ? -1 : 1))) {
                ...
                if (!p)
                    continue;
                if (p.goc.ducVongThieu === '')
                    xau.push(`lời cầu '${id}' không nêu dục vọng nào bị chặn`);
    ...
    dangKyInvariant({
        ...
        kiem: (s, phamVi) => {
            ...
            for (const id of idCanKiem(s, phamVi)) {
                const e = s.entities.get(id);
                if (!e || e.kind !== 'covenant')
                    continue;
                const g = doc(e, 'giao_uoc');
                if (!g) {
                    xau.push(`'${id}' là giao ước nhưng thiếu aspect 'giao_uoc'`);
                    continue;
                }
                if (!s.entities.has(g.benAId))
                    xau.push(`giao ước '${id}' có bên A '${g.benAId}' không tồn tại`);
                if (!s.entities.has(g.benBId))
                    xau.push(`giao ước '${id}' có bên B '${g.benBId}' không tồn tại`);
                if (!giaoUocCanBang(g)) {
