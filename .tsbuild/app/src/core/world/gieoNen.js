...
export function thapTuoi(danSo) {
...
export function aspectNen(danSo, rng) {
    const n = Math.max(0, Math.floor(danSo));
 ...
 */
export function patchGieoNen(state, eventId, seed) {
    const rng = taoRng(`${seed}#gieo_nen`);
    const ra = [];
    const noiChon = [...state.entities.keys()]
        .sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))
        .map((id) => state.entities.get(id))
        .filter((e) => e !== undefined && e.kind === 'place' && e.tickDiet === null);
    ...
    for (const id of [...state.entities.keys()].sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))) {
        ...
        if (!e || e.kind !== 'deity' || e.tickDiet !== null)
            continue;
        if (e.aspects['ban_nga'] !== undefined)
            continue;
        ...
        ra.push({
            ...
            value: banNgaTu(soul.banTinh, docAspect(e, 'venerable')),
            sourceEventId: eventId,
        });
    }
 ...
 */
export function eventGieoNen(state, hauTo = '') {
