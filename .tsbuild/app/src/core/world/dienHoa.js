...
// ─────────────────────────────────────────── cấu hình
export const NHIP_DIEN_HOA = ['nien', 'the_dai', 'vinh_kiep'];
...
export const BANG_CAM_DIEN_HOA = Object.freeze(['substrateLaws', 'branches', 'lorebooks']);
/** Đường dẫn cấm ngay cả trên bảng được phép. */
export const DUONG_DAN_CAM_DIEN_HOA = Object.freeze([
 ...
 */
export function locPatchTheoLanRanh(patches, cauHinh, state) {
    const giu = [];
    const bo = [];
    const l = [];
    ...
    for (const p of patches) {
        ...
        if (BANG_CAM_DIEN_HOA.includes(t)) {
            ...
            l.push(loi('patch', 'DIEN_HOA_BANG_CAM', `Bảng "${t}" nằm ngoài quyền của Diễn Hóa.`, { path: p.target.id }));
            continue;
        }
 ...
 */
export function kiemDieuKienDung(nc) {
    const bat = new Set(nc.cauHinh.dieuKienDung);
    const s = nc.state;
    const co = (dk) => bat.has(dk);
    if (co('mach_dat_cao_trao')) {
        const m = [...s.storylines.values()]
            .filter((x) => x.giaiDoan === 'cao_trao' && x.ketCuc === null)
    ...
    if (co('co_che_moi_xuat_hien') && (nc.coCheVuaBat ?? []).length > 0) {
        return {
            ...
            moTa: `Một Cơ Chế Phái Sinh vừa đủ điều kiện tồn tại: ${nc.coCheVuaBat.join(', ')}.`,
            entityIds: [],
        };
    }
    if (co('luat_nen_duoc_dat_ten') && (nc.trucVuaDatTen ?? []).length > 0) {
        return {
            ...
            moTa: `Có kẻ vừa khái niệm hóa được trục ${nc.trucVuaDatTen.join(', ')}.`,
            entityIds: [],
        };
    }
...
export function baoCaoDienHoa(log, truoc, sau) {
    const soNam = Math.max(0, log.tickKetThuc - log.tickBatDau);
    const tieuDe = `DIỄN HÓA · nhịp ${log.tickBatDau} – ${log.tickKetThuc} · ${soNam} nhịp · ` +
    ...
    const dong = [tieuDe, '', `Dừng vì: ${log.lyDoDung}`, ''];
    if (muc.length > 0) {
        dong.push(`Trong khoảng ấy, ${muc.length} điều đáng ghi:`);
        dong.push('');
        for (const m of muc)
            dong.push(`  ${m.tick}  ${m.moTa}`);
        dong.push('');
    }
