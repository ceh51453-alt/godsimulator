...
export const CUA_VAO = ['hu_vo', 'mot_cau', 'day_du'];
...
function tenSinh(rng, so = 2) {
    const phan = [];
    for (let i = 0; i < so; i++)
        phan.push(AM_TIET[rng.nguyen(AM_TIET.length)]);
    return phan.join(' ');
}
/**
 ...
 */
const e = (id, branchId, kind, ten, aspects, moTa = '') => EntitySchema.parse({
    id,
    branchId,
    kind,
    ten,
...
const patchTao = (bang, id, banGhi, eventId) => ({
    op: 'link',
 ...
 */
export function eventGieoTheGioi(ct) {
    ...
    const vungA = e('place_a', b, 'place', tenVungA, {
    ...
    const vungB = e('place_b', b, 'place', tenVungB, {
        spatial: SpatialSchema.parse({
    ...
    const luatNen = e('law_nen', b, 'law', 'Luật Nền: Nhân Quả Không Mất', {
    ...
    const than = e('deity_1', b, 'deity', tenThan, {
    ...
    // ── Phàm nhân ──
    const phamNhan = e('mortal_1', b, 'mortal', tenNguoi, {
        soul: SoulSchema.parse({
            tang: 't2',
    ...
    const lk = (id, tuId, denId, quanHe, trongSo) => patchTao('links', id, LinkSchema.parse({ id, branchId: b, tuId, denId, quanHe, trongSo, tickTao: 0 }), evId);
    patches.push(lk('lk_1', 'concept_o_ue', 'concept_thanh_sach', 'doi_nghich', 100), lk('lk_2', 'concept_thanh_sach', 'concept_o_ue', 'doi_nghich', 100), lk('lk_3', 'concept_o_ue', 'law_thuong', 'sinh_ra_tu', 85), lk('lk_4', 'law_thuong', 'concept_o_ue', 'sinh_ra', 85), lk('lk_5', 'place_a', 'law_nen', 'chiu_luat', 80), lk('lk_6', 'place_b', 'law_nen', 'chiu_luat', 80), lk('lk_7', 'place_a', 'law_thuong', 'dien_giai_sai', 34), lk('lk_8', 'place_b', 'law_thuong', 'dien_giai_sai', 61), lk('lk_9', 'deity_1', 'concept_thanh_sach', 'ket_tinh_tu', 78), lk('lk_10', 'mortal_1', 'place_a', 'cu_tru_tai', 95), lk('lk_11', 'place_a', 'mortal_1', 'la_noi_cu_tru_cua', 95), lk('lk_12', 'deity_1', 'place_b', 'cu_tru_tai', 60), lk('lk_13', 'place_b', 'deity_1', 'la_noi_cu_tru_cua', 60));
    return taoEvent({
        id: evId,
        branchId: b,
        tick: 0,
 ...
 */
export function eventKhaiThienHuVo(ct) {
    return taoEvent({
        id: 'ev_khai_thien',
        branchId: ct.branchId,
        tick: 0,
 ...
 */
export function moTheGioiTrong(ct) {
