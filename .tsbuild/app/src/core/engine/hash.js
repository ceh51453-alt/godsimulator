 ...
 */
export function chuanHoa(v) {
    return viet(v, new WeakSet());
}
function viet(v, daThay) {
    if (v === null)
        return 'n';
    if (v === undefined)
        return 'u';
    ...
    if (v instanceof Map) {
        const cap = [...v.entries()]
            .map(([k, val]) => [viet(k, daThay), viet(val, daThay)])
    ...
    const khoa = Object.keys(v).sort((x, y) => (x < y ? -1 : x > y ? 1 : 0));
    const phan = khoa.map((k) => `${viet(k, daThay)}=${viet(v[k], daThay)}`);
    daThay.delete(o);
    return `o${phan.length}:{${phan.join(',')}}`;
}
/** Hash chính tắc của một giá trị bất kỳ. */
export function hashCua(v) {
 ...
 */
export function hashTap(banGhi) {
    const hs = [];
