...
export function kiemNhanQua(e, log) {
    const errs = [];
    for (const causeId of e.causeEventIds) {
        ...
        if (!cause) {
            errs.push(loi('event', 'CAUSE_KHONG_TON_TAI', `Event '${e.id}' trỏ nguyên nhân '${causeId}' không có trong log.`));
            continue;
        }
        if (cause.tick > e.tick) {
            // [BB] Nguyên nhân không được đến từ tương lai.
            errs.push(loi('event', 'CAUSE_TU_TUONG_LAI', `Event '${e.id}' ở tick ${e.tick} nhận nguyên nhân '${causeId}' ở tick ${cause.tick}.`));
        }
    }
    if (log.theoId(e.id)) {
 ...
 */
export function apDungEvent(state, event, log, tuyChon = {}) {
    ...
    if (e.branchId !== state.world.branchId) {
        return hong([
            loi('event', 'EVENT_SAI_NHANH', `Event '${e.id}' thuộc nhánh '${e.branchId}', state đang ở '${state.world.branchId}'.`),
        ]);
    }
    if (e.tick < state.world.tick) {
        return hong([
            loi('event', 'EVENT_LUI_TICK', `Event '${e.id}' ở tick ${e.tick} nhưng world đã ở tick ${state.world.tick}.`),
        ]);
    }
    const loiNhanQua = kiemNhanQua(e, log);
    if (loiNhanQua.length > 0)
        return hong(loiNhanQua);
    // Mọi patch phải khai đúng Event sinh ra nó.
    const patchLacChu = e.patches.filter((p) => p.sourceEventId !== e.id);
    if (patchLacChu.length > 0) {
        return hong([
            loi('patch', 'PATCH_SAI_NGUON', `${patchLacChu.length} patch trong Event '${e.id}' khai sourceEventId khác.`),
        ]);
    }
...
export function apDungChuoi(state, events, log) {
    for (let i = 0; i < events.length; i++) {
        const r = apDungEvent(state, events[i], log);
        if (!r.ok) {
            return hong([
