...
const entity = (id, extra = {}) => EntitySchema.parse({ id, branchId: 'br', kind: 'concept', ten: id, tickSinh: 0, ...extra });
const p = (op, table, id, path, value, sourceEventId = 'ev1') => ({ op, target: { table, id, path }, value, sourceEventId });
function stateVoi(...ids) {
    const s = taoState(structuredClone(WORLD));
    for (const id of ids)
        s.entities.set(id, entity(id));
    return s;
}
// ─────────────────────────────────────────── RNG
describe('RNG seeded — luật bất biến #7', () => {
    ...
    it('nguyen(n) trong [0, n) và chịu được đầu vào xấu', () => {
        ...
        for (let i = 0; i < 1000; i++)
            expect(r.nguyen(7)).toBeLessThan(7);
...
describe('ExprNode — thay thế eval (ADR-0003)', () => {
    ...
    it('thiếu tham số trả lỗi có cấu trúc, không throw', () => {
        ...
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('EXPR_MISSING_ARG');
    });
    it('cây quá sâu bị chặn, không tràn stack', () => {
        let node = { op: 'literal', value: true, args: [] };
        for (let i = 0; i < DO_SAU_TOI_DA + 5; i++)
            node = { op: 'not', args: [node] };
        const r = tinhExpr(E(node), nguon);
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('EXPR_TOO_DEEP');
    });
    it('cùng biểu thức + cùng nguồn luôn cho cùng kết quả', () => {
        ...
        for (let i = 0; i < 100; i++)
            expect(dieuKienDung(e, nguon)).toBe(true);
    });
});
// ─────────────────────────────────────────── patch
describe('[BB] patch lỗi không để state nửa vời', () => {
    ...
    it('patch làm hỏng schema bị từ chối, state giữ nguyên', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors.some((e) => e.code === 'BAN_GHI_HONG_SAU_PATCH')).toBe(true);
        expect(hashState(s)).toBe(truoc);
    });
    it('mul lên chuỗi bị từ chối — Phần 9.2 kiểm tra 3', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('KIEU_KHONG_HOP');
    });
    ...
    it('link trùng id bị từ chối', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('LINK_TRUNG');
    });
    it('link với bản ghi không hợp lệ bị từ chối', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('LINK_KHONG_HOP_LE');
    });
    ...
    it('bảng lạ bị từ chối', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('BANG_LA');
    });
    it('expectedVersion lệch thì từ chối — optimistic concurrency', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('VERSION_LECH');
    });
...
describe('transaction — cửa duy nhất để state đổi', () => {
    ...
    it('event hợp lệ được commit và vào log', () => {
        ...
        if (r.ok)
            expect(r.value.hashTruoc).not.toBe(r.value.hashSau);
    });
    ...
    it('[BB] vi phạm invariant nặng thì rollback và event không vào log', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.domain).toBe('invariant');
        expect(log.soLuong()).toBe(0);
        expect(hashState(s)).toBe(truoc);
    });
    it('hash event sai bị từ chối — chống sửa log', () => {
        const s = stateVoi('e1');
        const log = taoEventLog();
        const e = { ...ev(), hash: 'gia_mao' };
        const r = apDungEvent(s, e, log);
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('EVENT_HASH_SAI');
    });
    ...
    it('patch khai sai sourceEventId bị từ chối', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('PATCH_SAI_NGUON');
    });
    it('event sai nhánh bị từ chối', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('EVENT_SAI_NHANH');
    });
    it('event lùi tick bị từ chối — tick đơn điệu tăng', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('EVENT_LUI_TICK');
    });
    ...
    it('[BB] log là append-only — id trùng bị từ chối', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors.some((x) => x.code === 'EVENT_TRUNG_ID')).toBe(true);
    });
    it('chuỗi event dừng đúng chỗ và báo vị trí', () => {
        const s = stateVoi('e1');
        const log = taoEventLog();
        const r = apDungChuoi(s, [
            ev({ id: 'a', tick: 1 }),
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.message).toContain('thứ 2/3');
        expect(log.soLuong()).toBe(1);
    });
});
// ─────────────────────────────────────────── invariant
...
describe('[BB] replay deterministic — cổng Phase 1', () => {
    ...
    function sinhChuoi(seed, soBuoc) {
    ...
    it('sinh lại chuỗi từ cùng seed cho cùng event log và cùng hash', () => {
        ...
        if (rx.ok && ry.ok)
            expect(rx.value.hashCuoi).toBe(ry.value.hashCuoi);
    });
    it('seed khác cho hash khác — hash thật sự phản ánh nội dung', () => {
        ...
        expect(rx.ok && ry.ok).toBe(true);
        if (rx.ok && ry.ok)
            expect(rx.value.hashCuoi).not.toBe(ry.value.hashCuoi);
    });
    it('đổi một patch làm hash cuối đổi theo', () => {
        ...
        const goc = events[viTri];
        const sua = [...events];
        sua[viTri] = taoEvent({
            ...goc,
            patches: goc.patches.map((pp) => (pp.op === 'add' ? { ...pp, value: pp.value + 1 } : pp)),
        });
        const b = replay(world, sua);
        expect(a.ok && b.ok).toBe(true);
        if (a.ok && b.ok)
            expect(a.value.hashCuoi).not.toBe(b.value.hashCuoi);
    });
    ...
    it('replay nghiêm ngặt dừng ở event hỏng', () => {
        ...
        const xau = [...events];
        xau[10] = { ...xau[10], hash: 'sai' };
        const r = replay(world, xau, true);
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.errors[0]?.code).toBe('REPLAY_DUT');
    });
