 ...
 */
export function docJsonPatch(raw) {
    const boQua = [];
    ...
    ds.forEach((x, i) => {
        ...
        if (r.success)
            ...
            boQua.push(loi('ai', 'JSON_PATCH_MUC_SAI', `Mục ${i} không hợp lệ: ${r.error.issues[0]?.message ?? 'sai hình dạng'}.`, {
                path: `[${i}]`,
            }));
        }
    });
 ...
 */
export function opEngineCua(op) {
    switch (op) {
        case 'replace':
            return 'set';
        case 'delta':
 ...
 */
export function ghiLorebook(nc) {
    const l = [];
    if (nc.nguonDich !== 'tu_sinh') {
        // [BB] 50.12 chẩn đoán 36 — hỏng NẶNG.
        l.push(loi('schema', 'WORKFLOW_GHI_LOREBOOK_NGUOI_DUNG', `Tác vụ "${nc.taskId}" định ghi vào lorebook nguồn "${nc.nguonDich}". ` +
            'Chỉ được ghi vào lorebook tự sinh (50.10).', { severity: 'fatal', path: nc.lorebookId, recoverable: false }));
        return { ok: false, loi: l };
    }
    if (!nc.target.chongDeQuy) {
        // [BB] 50.7 — bắt buộc, không phải khuyến nghị.
        l.push(loi('schema', 'THIEU_CHONG_DE_QUY', 'Entry do workflow ghi phải bật `chongDeQuy`. Không có nó, entry sẽ kích hoạt keyword của chính nó.', { path: nc.target.tenEntry }));
        return { ok: false, loi: l };
    }
 ...
 */
export function tuKichHoatChinhNo(entry) {
    if (entry.lop === 'loi')
        return false;
...
export function chenVaoCanh(target, noiDung) {
 ...
 */
export function bienTheoLuot(taskId, ten) {
