...
function moiEntity(s) {
    return [...s.entities.keys()]
        .sort((a, b) => (a < b ? -1 : a > b ? 1 : 0))
        .map((id) => s.entities.get(id))
        .filter((e) => e !== undefined);
}
const vai = (entityId, vaiTro, trongSo = 50) => ({
    entityId,
    vaiTro,
    trongSo,
});
function quanHeCua(e, doiTuongId) {
    const soul = aspect(e, 'soul');
    return soul?.quanHe?.[doiTuongId];
}
function moiQuanHe(e) {
    const soul = aspect(e, 'soul');
    const q = soul?.quanHe ?? {};
 ...
 */
const CHU_KY_TROI_DAY = Object.freeze({
    hang_dem: 4,
    theo_mua: 1,
    moi_ky_nguyen: 200,
    chi_o_tan_the: null,
 ...
 */
const DUONG_MAC_DINH = Object.freeze({
    am_i: 4,
    khoi: 8,
    phat_trien: 11,
    cao_trao: 16,
...
const duongChuan = (gd) => DUONG_MAC_DINH[gd];
/** Đường của mạch chậm: mọi delta co lại, nên nó ngồi lâu ở mỗi giai đoạn. */
const duongCham = (gd) => Math.round(DUONG_MAC_DINH[gd] * 0.6);
/** Đường của mạch bùng: cao trào tới nhanh và tới mạnh. */
const duongBung = (gd) => gd === 'cao_trao' ? 26 : Math.round(DUONG_MAC_DINH[gd] * 1.3);
 ...
 */
function nhipTheoMau(m, s, rng, mau, duong, 
 ...
 */
function dauDeLai(m, moTa, camXuc) {
    if (m.giaiDoan !== 'cao_trao' && m.giaiDoan !== 'ha_man')
        return undefined;
    const ra = [];
    const doi = m.nhanVat.find((n) => n.vaiTro === 'doi_dau');
    for (const n of m.nhanVat) {
        // Vai chứng kiến và vai xúc tác không mang ký ức nặng — họ đứng ngoài.
        if (n.vaiTro === 'chung_kien' || n.vaiTro === 'xuc_tac')
            continue;
        ra.push({
            loai: 'ky_uc',
            entityId: n.entityId,
...
const keVi = {
    tienDe(s) {
        const ra = [];
        for (const e of moiEntity(s)) {
            const tc = aspect(e, 'institutional');
            if (!tc)
                continue;
            ...
            for (const cv of tc.chucVu ?? []) {
                if (!cv.nguoiGiuId || !s.entities.has(cv.nguoiGiuId))
                    continue;
...
const chienTranh = {
    tienDe(s) {
        const ra = [];
        for (const e of moiEntity(s)) {
            const an = aspect(e, 'an_ninh');
            const kt = aspect(e, 'kinh_te');
            const thieu = (kt?.thieuHut ?? 0) > 0.2;
            const dangDanh = (an?.xungDot ?? []).filter((x) => x.tickKetThuc === null || x.tickKetThuc === undefined);
            for (const xd of dangDanh) {
                if (!s.entities.has(xd.doiThuId))
                    continue;
                if ((an?.deDoa ?? 0) < 30 && (xd.cuongDo ?? 0) < 30 && !thieu)
                    continue;
                ra.push({
                    loai: 'chien_tranh',
...
const amMuu = {
    tienDe(s) {
        ...
        const ra = [];
        for (const e of moiEntity(s)) {
            const cs = aspect(e, 'ban_nga')?.coreSelf ?? {};
            const tt = ducVong(e)['triThuc'] ?? 0;
            if ((cs['tratTu_phongTung'] ?? 0) >= -40 || tt < 20)
                continue;
            ra.push({
                loai: 'am_muu',
...
const tinhAi = {
    tienDe(s) {
        ...
        const canTro = moiEntity(s).find((x) => x.kind === 'law' && (aspect(x, 'lawful')?.hieuUng ?? []).length > 0);
        if (!canTro)
            return ra;
        for (const e of moiEntity(s)) {
            for (const [doiId, q] of moiQuanHe(e)) {
                if ((q.yeuGhet ?? 0) < 55 || (q.thanSo ?? 0) < 50)
                    continue;
                if (!s.entities.has(doiId))
                    continue;
                ra.push({
    ...
    tienDe(s) {
        ...
        for (const e of moiEntity(s)) {
            ...
            for (const dom of d?.domains ?? []) {
                ...
                const buong = dom.trangThai !== undefined && dom.trangThai !== 'held' && dom.trangThai !== 'contested';
                if (!yeu && !buong)
                    continue;
                ra.push({
                    loai: 'suy_tan',
...
const phatKien = {
    tienDe(s) {
        ...
        if (!luat)
            return ra;
        for (const e of moiEntity(s)) {
            const ky = aspect(e, 'mortal')?.kyNang ?? {};
            if (Math.max(0, ...Object.values(ky)) < 40)
                continue;
            ra.push({
                loai: 'phat_kien',
    ...
    tienDe(s) {
        ...
        for (let i = 0; i < than.length; i++) {
            for (let j = i + 1; j < than.length; j++) {
                ...
                if (!a || !b)
                    continue;
                const da = aspect(a, 'domain')?.domains ?? [];
                const db = aspect(b, 'domain')?.domains ?? [];
...
const datTen = {
    tienDe(s) {
        ...
        for (const ln of voDanh) {
            const kn = nen.find((k) => KHAI_NIEM_NEN_CUA_TRUC[ln.truc].some((h) => k.id.includes(h) || k.tags.includes(h)));
            if (!kn)
                continue;
 ...
 */
export const HANDLER_LOAI_MACH = Object.freeze({
    dat_ten: datTen,
    phuc_hung: phucHung,
    bao_thu: baoThu,
    ke_vi: keVi,
...
export function loaiMachThieuHandler(ids) {
