 ...
 */
export function quetRoRi(payload, bien) {
    const viPham = [];
    const seen = new WeakSet();
    const camKhoa = new Set(KHOA_CAM_RA_NGOAI.filter((k) => {
        // addressPreference được phép vào prompt (xem ma trận).
        if (k === 'addressPreference' && bien === 'prompt')
            return false;
        return true;
    }));
    const di = (v, p) => {
