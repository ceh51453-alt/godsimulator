import { KHAI_NIEM_NEN_CUA_TRUC, PHU_THUOC_TRUC, SubstrateLawSchema, THAM_SO_MAC_DINH, TRUC_NEN, } from './schema.js';
/** Bộ bảy trục mặc định PHÀM TỤC cho một nhánh mới — 43.7. */
export function luatNenMacDinh(branchId) {
    return TRUC_NEN.map((truc) => SubstrateLawSchema.parse({
        id: `ln.${truc}`,
        branchId,
        truc,
        trangThai: 'vo_danh',
...
export function thamSoCua(ds, truc) {
    const ln = ds.find((x) => x.truc === truc);
    if (ln === undefined)
        return THAM_SO_MAC_DINH[truc];
    return { ...THAM_SO_MAC_DINH[truc], ...ln.thamSo };
}
export function daCoTen(ds, truc) {
...
export function datTenTruc(input) {
    ...
    if (kn === undefined || c === undefined) {
        ...
        if (c.giaiDoan === 'hu_danh' || c.giaiDoan === 'manh_nha') {
            loi.push(`Khái niệm "${kn.ten}" mới ở giai đoạn "${c.giaiDoan}". Chưa ai hiểu nó đủ để đặt tên cho một trục nền.`);
        }
        const hopLe = KHAI_NIEM_NEN_CUA_TRUC[input.truc];
        const khop = hopLe.some((h) => input.khaiNiemNenId.includes(h) || kn.tags.includes(h));
        if (!khop) {
            loi.push(`Khái niệm "${kn.ten}" không phải nền của trục "${input.truc}". ` +
    ...
    for (const truoc of PHU_THUOC_TRUC[input.truc]) {
        if (!daCoTen(input.ds, truoc)) {
            loi.push(`Không đặt tên được "${input.truc}" khi "${truoc}" còn vô danh (43.5). ` +
 ...
 */
export function keHoCuaTruc(truc, thamSo) {
    const ra = [];
    switch (truc) {
        case 'khong_gian':
            ...
            if (thamSo['ghiDeCucBo'] === true)
                ra.push('Có thể áp một bộ luật riêng lên một vùng nhỏ.');
            break;
        case 'thoi_gian':
            if (thamSo['quaKhu'] === 'sua_duoc')
                ra.push('Sửa một việc đã xảy ra mà không cần phân nhánh.');
            if (thamSo['tuongLai'] === 'da_ton_tai')
                ra.push('Đọc trước kết cục rồi đi vòng qua nó.');
            if (thamSo['huong'] === 'vong_lap')
                ra.push('Lặp lại một quãng thời gian tới khi ra kết quả muốn có.');
            break;
        case 'nhan_qua':
            if (thamSo['chamTruuTuong'] === true)
                ra.push('Chạm thẳng vào một khái niệm thay vì vào vật mang nó.');
            if (thamSo['thuTu'] === 'long_leo')
                ra.push('Đặt hậu quả trước nguyên nhân trong một khoảng ngắn.');
        ...
        case 'danh_tinh':
            ...
            if (thamSo['saoChep'] === 'hai_nguoi')
                ra.push('Bản sao là người khác — trách nhiệm không đi theo.');
            if (thamSo['lienTuc'] === 'theo_ten_goi')
                ra.push('Đổi tên là đổi người.');
            break;
        case 'sinh_tu':
            if (thamSo['hoiSinh'] !== 'khong_the')
                ra.push('Cái chết trở thành một khoản chi phí, không phải một kết thúc.');
            if (thamSo['ranhGioi'] === 'mo')
                ra.push('Có kẻ nửa sống nửa chết, và luật không biết xếp họ vào đâu.');
            break;
        case 'nhan_thuc':
            if (thamSo['hieuBietLamSuyYeu'] === true)
                ra.push('Giết một vị thần bằng cách giải thích ngài.');
            if (thamSo['tenGoiCoQuyenNang'] === true)
                ra.push('Biết tên thật là khống chế được — tên thành tài sản chiến lược.');
            if (thamSo['quanSatLamDinhHinh'] === true)
                ra.push('Thứ chưa ai nhìn thì còn đổi hình được.');
            break;
        case 'van_menh':
...
function bienNienDatTen(truc, nguoi, tick) {
    ...
    return (`Nhịp ${tick}, ${ten} viết ra câu đầu tiên ${nhan[truc]}. ` +
 ...
 */
export function suaLuatNen(ds, yc) {
    const ln = ds.find((x) => x.truc === yc.truc);
    if (ln === undefined)
        return { ok: false, loi: [`Không có bản ghi luật nền cho trục "${yc.truc}".`] };
 ...
 */
export function laKhoangCachYNghia(ds) {
 ...
 */
export function canhLienKeYNghia(s, ds) {
    if (!laKhoangCachYNghia(ds))
        return [];
    // Nhóm theo vị thần được thờ: mọi cặp trong một nhóm là liền kề.
    const theoThan = new Map();
    for (const l of [...s.links.values()].sort((a, b) => (a.id < b.id ? -1 : 1))) {
        if (l.tickDut !== null)
            continue;
        if (l.quanHe !== 'tho_phung' && l.quanHe !== 'thuoc_ve_than')
            continue;
 ...
 */
export const NHAN_TRUC_NEN = Object.freeze({
...
export function bangLuatNen(ds, s) {
