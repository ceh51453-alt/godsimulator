...
function dungTheGioi() {
    ...
    if (!r.ok)
        throw new Error('gieo lỗi');
    return s;
}
beforeEach(() => {
    state = dungTheGioi();
});
function ngocCanh(mode, chuTheId, cau, i = 0) {
    ...
    it('input được đánh dấu `chan_co_ly_do` bị chặn KÈM lý do cụ thể', () => {
        for (const [mode, chuThe, ds] of boTest) {
            for (const ca of ds.filter((c) => c.mong === 'chan_co_ly_do')) {
                ...
                const plan = lapKeHoach(nc);
                const r = giaiQuyet(nc);
                const coLyDo = plan.blockedBy.length > 0 || plan.unknowns.length > 0 || r.outcome.result !== 'success';
...
describe('[BB] 67.6 — việc đời thường lặp KHÔNG tự thành luật vũ trụ', () => {
    it('lặp một hành động cá nhân 200 lần không đổi luật nào', () => {
        const luatTruoc = JSON.stringify([...state.entities.values()].filter((e) => e.kind === 'law').map((e) => e.aspects['lawful']));
        ...
        const luatSau = JSON.stringify([...state.entities.values()].filter((e) => e.kind === 'law').map((e) => e.aspects['lawful']));
