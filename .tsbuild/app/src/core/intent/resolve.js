...
function laKhongHoanTac(ref, intent) {
    if (KHONG_HOAN_TAC.has(ref))
        return true;
...
export function lapKeHoach(nc) {
    ...
    for (const t of intent.targetRefs) {
        if (!view.entities.has(t.id)) {
            blockedBy.push(chan('KHONG_BIET_MUC_TIEU', `Bạn chưa từng nghe về "${t.label ?? t.id}".`, {
                missingRefs: [t],
            }));
        }
    }
    ...
    if (doan.nguon === 'verb' && intent.mode !== 'sang_the') {
        ...
        blockedBy.push(chan('DONG_TU_SANG_THE', `"${v?.ten ?? doan.ref}" là phép của Sáng Thế Thần. Ở tầng này bạn phải làm điều đó bằng tay người, không bằng ý niệm.`, { recoverable: true }));
    ...
    if (intent.stance.harm === 'allow') {
        for (const l of view.laws) {
            ...
            if (!camGiet)
                continue;
            blockedBy.push(chan('LUAT_CAM', `"${l.ten}" ràng buộc việc này: ${l.dienGiai || l.vanBan || ''}`, {
                lawId: l.id,
                recoverable: true,
            }));
    ...
    const steps = intent.rawText.trim() === ''
        ? []
        : [
            {
                id: `${intent.id}_s1`,
...
export function nenThanhProject(intent) {
    if (laDauHieuProject(intent.rawText))
        return true;
    return intent.horizon === 'season' || intent.horizon === 'year' || intent.horizon === 'era';
}
function phamViProject(intent) {
    if (intent.mode === 'sang_the')
        return 'cosmic';
    if (intent.mode === 'than')
        return 'divine';
    if (intent.horizon === 'era' || intent.horizon === 'year')
        return 'regional';
    return 'personal';
}
/**
 ...
 */
export function taoProject(nc, plan) {
    const { intent } = nc;
    const yeuCau = [];
...
export function giaiQuyet(nc) {
    ...
    const lyDo = plan.blockedBy[0]?.message ??
        plan.unknowns[0] ??
