 ...
 */
const HAI_DAU_TRUC = Object.freeze({
...
function tinhCachThanhLoi(bt) {
    const net = [];
    for (const truc of Object.keys(HAI_DAU_TRUC).sort((a, b) => (a < b ? -1 : 1))) {
        const v = bt[truc];
        if (typeof v !== 'number' || Math.abs(v) < NGUONG_THANH_LOI)
            continue;
...
export function dungChiMuc(s, loreQuery = '') {
    ...
    return Object.freeze(nhap
        .sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
        .map((n) => ChunkSchema.parse({
        id: n.id,
        branchId,
        nguon: n.nguon,
        nguonId: n.nguonId,
