...
import { diemHeuristic, xepHangHeuristic, fusion, mmr, dongGoiTheoToken, locOutputAdapter, catChoAdapter, } from './rerank.js';
import { truyHoi, locTamNhin, dungBaTruyVan, scopeKeyCua, MACH_RERANK_MOI, machSauHong, machSauBoQua, NGUONG_MO_MACH_RERANK, SO_REQUEST_BO_QUA, } from './truyHoi.js';
...
function theGioi(seed = 'retrieval') {
...
const ck = (id, ghiDe = {}) => ChunkSchema.parse({
    id,
    branchId: 'br_goc',
    nguon: 'lorebook',
    nguonId: `nguon_${id}`,
...
const cand = (id, ghiDe = {}) => RerankCandidateSchema.parse({
    chunkId: id,
    sourceType: 'lorebook',
...
describe('[BB] 54.3 — nhãn tầm nhìn gán lúc index', () => {
    it('chunk KHÔNG khai nhãn thì mặc định pham_nhan, và nhãn cao chặn tầng dưới', () => {
        ...
        const nc = { vungIds: new Set(), domainIds: new Set() };
    ...
    it('[BB] 54.2 — diễn giải của vùng A không trả về khi đang kể ở vùng B', () => {
        ...
        expect(chunkDuocThay(dgA, { mucChieu: 'pham_nhan', vungIds: new Set(['place_a']), domainIds: new Set() })).toBe(true);
        expect(chunkDuocThay(dgA, { mucChieu: 'pham_nhan', vungIds: new Set(['place_b']), domainIds: new Set() })).toBe(false);
    });
    it('domain hạn chế: thần chỉ thấy phần trong domain mình giữ', () => {
        ...
        expect(chunkDuocThay(c, { mucChieu: 'than', vungIds: new Set(), domainIds: new Set(['Thanh Sạch']) })).toBe(true);
        expect(chunkDuocThay(c, { mucChieu: 'than', vungIds: new Set(), domainIds: new Set(['Ô Uế']) })).toBe(false);
    });
    ...
    it('bản tính THẬT của thần là chunk `sang_the`; bản tín đồ tin thì `pham_nhan`', () => {
        ...
        if (than) {
            ...
            state.entities.set('deity_1', {
                ...
                aspects: {
                    ...than.aspects,
                    venerable: { ...than.aspects['venerable'], banTinhTinDoTin: { tuBi_tanNhan: -80 } },
                },
            });
        }
        const ds = dungChiMuc(state);
    ...
    it('nét bản tính chưa đủ mạnh thì KHÔNG sinh chunk — dân không kể chuyện nhạt', () => {
        ...
        if (than) {
            state.entities.set('deity_1', {
                ...
                aspects: {
                    ...than.aspects,
                    venerable: { ...than.aspects['venerable'], banTinhTinDoTin: { tuBi_tanNhan: 5 } },
                },
            });
        }
...
describe('ba kênh và RRF — Phần 54.1', () => {
    ...
    it('chunk chưa nhúng (`vector = null`) vẫn là chunk dùng được', () => {
        ...
        expect(rrf([{ ten: 'tu_vung', kq: kenhTuVung([...ds], 'dấu máu'), trongSo: 1 }]).length).toBeGreaterThan(0);
    });
...
describe('mở rộng đồ thị — Phần 6.4', () => {
    it('[BB] chỉ đi qua entity chủ thể ĐƯỢC BIẾT — không có đường rò qua đồ thị', () => {
        ...
        for (const n of mr)
            expect(view.entities.has(n.id)).toBe(true);
        // Không nốt nào nằm trong danh sách `mu`.
        for (const n of mr)
            expect(view.suongMu.mu).not.toContain(n.id);
    });
    ...
    it('kết quả deterministic — cùng view cho cùng thứ tự', () => {
        const { state } = theGioi();
        const view = chieu(state, 'sang_the', null);
        expect(moRong(['law_thuong'], { soHop: 2, view }).map((n) => n.id)).toEqual(moRong(['law_thuong'], { soHop: 2, view }).map((n) => n.id));
    });
...
describe('[BB] 77.4 — heuristic luôn tồn tại và luôn deterministic', () => {
    ...
    it('mỗi task dùng hồ sơ trọng số riêng từ tuning', () => {
        expect(TUNING.rerank.hoSoTask['answer_prayer']?.graph).toBeGreaterThan(TUNING.rerank.hoSoTask['world_report']?.graph ?? 0);
    });
});
// ─────────────────────────────────────────── fusion + MMR
describe('[BB] 77.6 — fusion theo thứ hạng, MMR phạt cả trùng nguồn', () => {
    ...
    it('[BB] hai chunk CÙNG nguonId là trùng MẠNH dù chữ khác hẳn', () => {
        ...
        const xh = [
            { chunkId: 'x1', diem: 1.0, lyDo: [] },
            { chunkId: 'x2', diem: 0.9, lyDo: [] },
            { chunkId: 'y1', diem: 0.75, lyDo: [] },
        ];
        const tra = new Map([
    ...
    it('MMR gắn lý do `diversity` cho chunk được chọn nhờ khác nguồn', () => {
        const xh = [
            { chunkId: 'a', diem: 1, lyDo: [] },
            { chunkId: 'b', diem: 0.9, lyDo: [] },
        ];
        const tra = new Map([
...
describe('[BB] 77.7 — đóng gói theo token, và budget CÓ TRACE', () => {
    const tt = (nguon, nguonId, dai, hop = null) => ({
    ...
    it('hết ngân sách thì DỪNG, và chunk bị bỏ có lý do ghi lại', () => {
        const xh = ['a', 'b', 'c'].map((id, i) => ({ chunkId: id, diem: 1 - i * 0.1, lyDo: [] }));
        const tra = new Map([
    ...
    it('quy tắc 1 + 2 — mỗi nguồn giữ một chỗ, và nhân quả trực tiếp vào trước', () => {
        const xh = [
            { chunkId: 'thap_ma_gan', diem: 0.1, lyDo: [] },
            { chunkId: 'cao_ma_xa', diem: 0.9, lyDo: [] },
            { chunkId: 'nguon_khac', diem: 0.5, lyDo: [] },
        ];
        const tra = new Map([
...
describe('[BB] 77.5 — adapter và ngắt mạch', () => {
    ...
    it('[BB] 77.9 — ba lỗi liên tiếp mở mạch trong hai mươi request', () => {
        ...
        for (let i = 0; i < NGUONG_MO_MACH_RERANK; i++)
            m = machSauHong(m, 'timeout');
        expect(m.moMach).toBe(true);
        expect(m.conBoQua).toBe(SO_REQUEST_BO_QUA);
        for (let i = 0; i < SO_REQUEST_BO_QUA - 1; i++)
            m = machSauBoQua(m);
        expect(m.moMach).toBe(true);
...
describe('[BB] cổng Phase 8 — đường ống truy hồi không rò ba tầng', () => {
    ...
    function dauVao(state, mode, chuTheId) {
    ...
    it('[BB] lọc tầm nhìn chạy TRƯỚC — chunk cấm không tồn tại trong tập đã chiếu', () => {
        ...
        for (const id of CAM)
            expect(daChieu.map((c) => c.id)).not.toContain(id);
    ...
    it('[BB] 77.7 quy tắc 3 — không nguồn nào chiếm quá nửa top-K', async () => {
        ...
        const dem = new Map();
        for (const c of kq.daChon)
            dem.set(c.nguonId, (dem.get(c.nguonId) ?? 0) + 1);
        const tran = Math.max(1, Math.floor(CFG.outputK * TUNING.rerank.tranTyLeMotNguon));
        for (const [nguon, n] of dem)
            expect(n, `nguồn ${nguon} chiếm ${n} chỗ`).toBeLessThanOrEqual(tran);
    });
    it('chunk cùng nguồn bị đẩy xuống dưới chunk khác nguồn có điểm thấp hơn', async () => {
        ...
        if (hai >= 0 && khac >= 0)
            expect(khac).toBeLessThan(hai);
    });
    it('cache: cùng khóa thì tái dùng; đổi scopeKey thì KHÔNG', async () => {
        const { state } = theGioi();
        const kho = new Map();
        const khoaCua = (k) => Object.values(k).join('|');
        const cacheDoc = (k) => Promise.resolve(kho.get(khoaCua(k)));
        const cacheGhi = (k, v) => {
            kho.set(khoaCua(k), v);
            return Promise.resolve();
        };
        const dv = { ...dauVao(state, 'pham_nhan', 'mortal_1'), cacheDoc, cacheGhi };
        ...
        const c = await truyHoi({ ...dauVao(state, 'than', 'deity_1'), cacheDoc, cacheGhi });
...
describe('[BB] 77.10 — bộ đề tự nhãn từ sự thật engine', () => {
    it('dựng được bài thi từ chính luật chống rò rỉ của 18.2', () => {
        ...
        for (const bt of de) {
            ...
            for (const cam of bt.ca.forbiddenChunkIds)
                expect(dung.has(cam)).toBe(false);
        }
    });
