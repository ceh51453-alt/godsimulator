 ...
 */
export const XU_LY_THEO_NHAN = Object.freeze({
 ...
 */
export function phanLoaiNoiDung(text) {
    const ra = [];
    const daCo = new Set();
...
export function cheSecret(s) {
    let ra = s;
    for (const [, re] of MAU_SECRET) {
        ra = ra.replace(new RegExp(re.source, re.flags.includes('g') ? re.flags : `${re.flags}g`), (m) => m.length <= 6 ? '••••••' : `${m.slice(0, 4)}${'•'.repeat(Math.min(12, m.length - 4))}`);
    }
    return ra;
}
 ...
 */
export function quetAnToan(goc) {
    const issues = [];
    const secrets = [];
    const urls = [];
    const nodeBiTuChoi = [];
    ...
    for (const s of secrets) {
        issues.push({
            ...
            severity: 'warning',
            path: s.path,
            message: `Chuỗi ở đây có hình dạng ${s.loai}. Nó sẽ bị che trong giao diện và ` +
    ...
    if (hosts.length > 0) {
        issues.push({
            ...
            severity: 'info',
            path: '',
            message: `File nhắc tới ${hosts.length} host ngoài. Không host nào được gọi khi nhập ` +
 ...
 */
export function locKhoaNguyHiem(goc) {
    const di = (v) => {
        if (Array.isArray(v))
            return v.map(di);
        if (v === null || typeof v !== 'object')
