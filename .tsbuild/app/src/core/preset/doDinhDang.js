...
function dauHieuSillyTavern(o) {
    ...
    if (Array.isArray(prompts) && prompts.length > 0) {
        ...
        if (hopLe.length >= Math.max(1, Math.floor(mau.length * 0.6)))
            ra.push('prompts[]');
    }
    ...
    if (laObj(ext)) {
        if (Array.isArray(ext['regex_scripts']))
            ra.push('extensions.regex_scripts');
        if (laObj(ext['tavern_helper']))
            ra.push('extensions.tavern_helper');
    }
    return ra;
}
/**
 ...
 */
export function doDinhDang(goc, tenNguon = '') {
    const ghiChu = [];
    if (!laObj(goc)) {
        return {
            format: 'unknown_json',
    ...
    if (goc['_format'] === 'thien_dien_bundle_v1' || goc['_format'] === 'thien_dien_lore') {
        const parts = [];
        if (Array.isArray(goc['modules']))
            parts.push('prompt');
        if (laObj(goc['generation']))
            parts.push('generation');
        if (Array.isArray(goc['lorebooks']) || Array.isArray(goc['entries']))
            parts.push('lorebook');
        if (Array.isArray(goc['tasks']))
            parts.push('workflow');
        if (Array.isArray(goc['entries']) && laObj(goc['registry']))
            parts.push('registry');
        return {
    ...
    const laWorldInfo = goc['spec'] === 'lorebook_v3' ||
    ...
    if (diem >= NGUONG_NHAN) {
        ...
        if (coPrompts)
            parts.push('prompt');
        if (dauHieu.some((d) => d.startsWith('extensions.')))
        ...
        if (!coOrder) {
            ghiChu.push({
                ...
                severity: 'warning',
                path: 'prompt_order',
                message: 'File không có prompt_order. Thứ tự và bật/tắt sẽ đọc từ prompts[].enabled — ' +
    ...
    ghiChu.push({
        ...
        severity: 'error',
        path: '',
        message: 'Không nhận ra định dạng. Đây là những khóa ở mức ngoài cùng đã thấy: ' +
