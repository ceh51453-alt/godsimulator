...
function doSau(v, tran, hienTai = 0) {
    if (hienTai > tran)
        return hienTai;
    if (v === null || typeof v !== 'object')
        return hienTai;
    let max = hienTai;
    const ds = Array.isArray(v) ? v : Object.values(v);
    for (const x of ds) {
...
export function nhapPreset(dv) {
    ...
    if (daCoSan !== null) {
        ...
        issues.push({
            ...
            severity: 'info',
            path: '',
            message: `File này đã có trong thư viện (pack "${daCoSan.packId}" bản ${daCoSan.version}). ` +
                'Không tạo bản trùng.',
    ...
    if (kqDo.format !== 'sillytavern_openai_preset') {
        issues.push({
            ...
            severity: 'error',
            path: '',
            message: `Nhận ra định dạng "${kqDo.format}" nhưng đường nhập của nó không nằm ở Xưởng Preset. ` +
    ...
    const soBlock = Array.isArray(sach['prompts']) ? sach['prompts'].length : 0;
     ...
     * sửa đang đọc.
     */
    for (const pl of ['updater', 'evolution', 'workflow_task']) {
 ...
 */
export function tenPackTuNguon(ten, hash) {
    const goc = ten
        .replace(/\.json$/i, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
