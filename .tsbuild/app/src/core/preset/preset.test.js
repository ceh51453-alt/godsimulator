...
import { kichHoat, hoanTac, apActivation, lintTruocKhiBat, diffPack, versionKeTiep, BAC_QUYEN, } from './kichHoat.js';
...
function chayNhap(text, ten, daNhap) {
    return nhapPreset({
        tenNguon: ten,
        noiDung: text,
        tick: 7,
...
describe.each(['A', 'B'])('cổng 1 — fixture %s đúng hash, count và mismatch', (nhan) => {
    ...
    it('hash của file nhập là SHA-256 thật của bytes đã nhận', () => {
        expect(kq.envelope?.sourceHash).toBe(createHash('sha256').update(text, 'utf8').digest('hex').toUpperCase());
    });
    ...
    it('[BB] 63.3 quy tắc 5 — prompt ngoài order được GIỮ nhưng mặc định TẮT', () => {
        ...
        const ngoai = kq.row.pack.modules.filter((m) => m.sourceMeta['trongOrder'] === false);
        expect(ngoai.length).toBe(meta.counts.unordered);
        for (const m of ngoai)
    ...
    it('tổng module bằng tổng prompt — không mục nào bị bỏ im lặng (64.1 mục 2)', () => {
        expect(kq.row.pack.modules.length).toBe(meta.counts.prompts);
    });
    it('Unicode trong tên được giữ nguyên, không bị chuẩn hóa mất dấu', () => {
        ...
        const tenSau = kq.row.pack.modules.map((m) => m.name);
        expect(tenSau).toEqual(tenGoc.map((t, i) => (t === '' ? (goc.prompts[i]?.identifier ?? '') : t)));
    });
});
describe.each(['C', 'D', 'E'])('hồi quy preset người dùng %s (fixture ẩn danh)', (nhan) => {
    const { meta, text } = docFixture(nhan);
    const kq = chayNhap(text, `fixture-${nhan}.json`);
...
describe('cổng 3 — không script nào chạy', () => {
    ...
    it('sanitizer xóa script, iframe, form, handler và URL javascript:', () => {
        const bẩn = '<div onclick="x()"><script>alert(1)</script><iframe src="http://a"></iframe>' +
    ...
    it('regex giữ đúng placement, depth, promptOnly, markdownOnly và disabled', () => {
        const rx = (id, pattern, thayThe, extra = {}) => TransformDefSchema.parse({
            id,
            packId: 'p',
            ten: id,
            pattern,
    ...
    it('HTML preset giữ CSS trong iframe nhưng không giữ script, handler hay URL mạng', () => {
        const kq = taiLieuHtmlCachLy('<style>.x{color:red;background:url(https://x)}</style><div class="x" onclick="x()">ok</div><script>x()</script>');
    ...
    it('script helper được nhận diện thành adapter native và chỉ thị scene được đọc', () => {
        ...
        expect(adapters).toEqual(expect.arrayContaining([expect.objectContaining({ kind: 'scene_switch', batONguon: true })]));
    ...
    it('quét an toàn từ chối node có khóa __proto__ và bản đã lọc không còn nó', () => {
        const doc = JSON.parse('{"a":1,"b":{"__proto__":{"x":1},"c":2}}');
        const q = quetAnToan(doc);
        expect(q.nodeBiTuChoi).toContain('b.__proto__');
        const sach = locKhoaNguyHiem(doc);
...
describe('cổng 5 — preset không đọc hồ sơ riêng, không ghi đè danh tính canon', () => {
    ...
    it('không có tên nào trong `suongMu.mu` lọt vào prompt biên dịch', () => {
        const kq = chayNhap(docFixture('A').text, 'A.json');
        for (const pl of ['narrator', 'updater', 'evolution', 'workflow_task']) {
...
describe('cổng 6 — tắt pack trả về prompt native đúng byte/hash', () => {
    const kq = chayNhap(docFixture('A').text, 'A.json');
    const row = kq.row;
    const bienDich = (pack) => bienDichPromptPreset({
        pack,
        pipeline: 'narrator',
        view: viewGia(),
        scene: sceneGia(),
    ...
    it('tầng 0 luôn có mặt và không module ngoài nào chen lên trước nó', () => {
        ...
        expect(r.messages[0]?.content).toContain(TANG_0[0]);
    ...
    it('injection_order cực lớn hay depth âm KHÔNG đẩy module ngoài lên trên tầng lõi', () => {
        ...
        const m = kq2.row.pack.modules[0];
...
describe('cổng 7 — 100 tick sau import, engine vẫn đúng', () => {
    ...
    function chayTick(state, log, so) {
        for (let i = 0; i < so; i++) {
...
describe('66.5 — nhập lại cùng hash không nhân đôi', () => {
    it('lần nhập thứ hai dừng ở bước 3 và chỉ ra bản đã có', () => {
        ...
        const thuVien = new Map([
            [lan1.envelope?.sourceHash, { packId: lan1.row?.packId, version: 1 }],
        ]);
    ...
    it('cùng pack, hash khác → version mới, KHÔNG ghi đè version cũ', () => {
        const a = chayNhap(docFixture('A').text, 'A.json').row;
        const b = {
            ...chayNhap(docFixture('B').text, 'B.json').row,
            packId: a.packId,
            version: 2,
        };
...
describe('66.5 — export lại vẫn chứa raw phần chưa hỗ trợ', () => {
    it('raw source giữ nguyên từng byte và round-trip lại đúng file gốc', () => {
        ...
        expect(sha256(kq.rawSource?.noiDung)).toBe(kq.envelope?.sourceHash);
    });
    it('script cách ly vẫn giữ hash và độ dài để export lại được', () => {
        const kq = chayNhap(docFixture('A').text, 'A.json');
        for (const s of kq.row.quarantined) {
...
describe('66.5 — prompt output format không phá PatchParser', () => {
    it('bóc tách vẫn nhận khối CapNhat khi output có đầy thẻ legacy', () => {
        const raw = '<thinking>đừng đọc cái này</thinking>' +
...
describe('66.5 — hoàn tác activation không mất lịch sử', () => {
    it('hoàn tác chỉ đổi con trỏ, activation cũ vẫn còn nguyên', () => {
        const row = chayNhap(docFixture('A').text, 'A.json').row;
    ...
    it('bậc quyền đặt "Imported prompt pack" dưới mọi hợp đồng native', () => {
        ...
        expect(BAC_QUYEN.indexOf('Imported prompt pack')).toBeGreaterThan(BAC_QUYEN.indexOf('Native task instruction'));
    });
});
// ─────────────────────────────────────────── macro
describe('macro — AST, không replace chuỗi (63.5)', () => {
    ...
    it('[BB] cycle biến cho ra lỗi CÓ ĐƯỜNG DẪN cycle, không treo', () => {
        ...
        expect((cyc?.details['cycle']).length).toBeGreaterThanOrEqual(2);
    });
...
describe('xung đột và đồ thị phụ thuộc (65.1, 65.2)', () => {
    ...
    it('nhóm exclusive_one và user_choice đòi người chọn; min và native_wins tự giải', () => {
        const kq = chayNhap(docFixture('A').text, 'A.json');
        const nhom = kq.nhom;
        const tuDong = giaiTuDong(nhom, kq.row.pack.modules);
        for (const n of nhom) {
            if (n.canNguoiChon)
                expect(Object.hasOwn(tuDong, n.khoa)).toBe(false);
            else
                expect(Object.hasOwn(tuDong, n.khoa)).toBe(true);
        }
    ...
    it('nhóm xung đột chỉ tính module đang bật', () => {
        const kq = chayNhap(docFixture('B').text, 'B.json');
        const tat = kq.row.pack.modules.map((m) => ({ ...m, enabled: false }));
...
describe('classifier chỉ gắn nhãn, không tự xóa (64.5)', () => {
    ...
    it('nhãn rủi ro được báo nhưng không làm mất prompt đã bật ở nguồn', () => {
        ...
        const mods = kq.row.pack.modules;
        expect(mods[0]?.activation).toBe('adapted');
        expect(mods[1]?.activation).toBe('adapted');
        expect(mods[0]?.sourceMeta['nhanRuiRo']).toEqual(expect.arrayContaining(['visibility_override', 'state_write_claim']));
...
describe('tham số giữ raw, clamp theo profile (62.4)', () => {
    it('top_k = 500 được GIỮ raw và clamp theo model thật', () => {
        ...
        expect(kq.row.pack.generation?.topK).toBe(500);
    });
    ...
    it('profile rộng hơn thì giữ nguyên — clamp là do model, không do schema', () => {
        ...
        const gen = docThamSoNguon(JSON.parse(docFixture('A').text));
...
describe('dò định dạng bằng hình dạng (63.2)', () => {
    it('nhận world info V2 qua bản đồ khóa số', () => {
        expect(doDinhDang({ entries: { '0': { key: ['a'] }, '1': { key: ['b'] } } }).format).toBe('sillytavern_world_info');
    });
...
describe('63.3 quy tắc 3 — không có prompt_order thì mới dùng prompts[].enabled', () => {
    ...
    it('order mồ côi báo ORDER_DANGLING và KHÔNG tạo prompt rỗng', () => {
        ...
        expect(kq.row.pack.modules.length).toBe(1);
    });
    it('identifier trùng KHÔNG gộp — mỗi bản một id riêng', () => {
        ...
        const ids = kq.row.pack.modules.map((m) => m.id);
