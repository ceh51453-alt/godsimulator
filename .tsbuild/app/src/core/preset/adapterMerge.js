 ...
 */
const REGEX_BLOCK_RE = /<regex(?:\s+order\s*=\s*(\d))?\s*>\s*"\/((?:\\.|[^"/\r\n])*)\/([gimsuy]*)"\s*:\s*"((?:\\.|[^"\r\n])*)"\s*<\/regex>/gm;
...
export function apInPromptRegex(text, tuyChon = {}) {
    const errors = [];
    let applied = 0;
    const blocks = docBlocks(text);
    if (blocks.length === 0)
        return { text, applied: 0, errors: [] };
 ...
 */
export function apInPromptRegexMessages(messages, tuyChon = {}) {
    const mutable = (m) => !m.moduleId.startsWith('td:');
    const blocks = messages.flatMap((m) => (mutable(m) ? docBlocks(m.content) : []));
    if (blocks.length === 0)
        return { messages, applied: 0, errors: [] };
    const errors = [];
    let applied = 0;
 ...
 */
export function captureFromOutput(output, rules, existing) {
    const data = { ...existing };
    let changed = false;
 ...
 */
export function replaceTagsInPrompt(prompt, captured) {
    let result = prompt;
