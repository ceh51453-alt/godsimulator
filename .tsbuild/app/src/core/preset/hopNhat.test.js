...
function theGioi() {
    ...
    expect(apDungEvent(state, eventGieoNen(state), log).ok).toBe(true);
    return state;
}
function nguLieu(state, cau = 'Ta ban một điều luật.') {
    return {
...
function packThu(coMarkerLichSu) {
    const mo = (id, kind, lane, content, sourceIdentifier = id) => PromptModuleSchema.parse({
        id: `pk.thu/${id}`,
        packId: 'pk.thu',
        sourceIdentifier,
        name: id,
...
function goi(state, packs, hoTroPrefill = true) {
    return bienSoanLuot({
...
describe('Bật pack không được đánh rơi nội dung native', () => {
    ...
    it('marker rỗng được lắp bằng nội dung native đã chiếu, không gửi ô trống', () => {
        ...
        expect(boiCanh.content.trim().length).toBeGreaterThan(0);
        const lichSu = kq.compiled?.messages.find((m) => m.moduleId === 'pk.thu/lich_su');
        expect(lichSu.content).toContain('sương phủ kín thung lũng');
    });
...
describe('Tương thích thẻ bài MVU — nhận cú pháp, KHÔNG nhận thẩm quyền', () => {
    ...
    it('câu lệnh _.set trỏ ra ngoài thế giới thành BIẾN PACK, không thành patch', () => {
        const kq = bocTach("Nàng cau mày.\n<UpdateVariable>\n_.set('stat_data.hao_cam', 10, 25); // vì ngươi đã giúp nàng\n</UpdateVariable>", { eventId: 'ev1', idHopLe: new Set(['mortal_ly']) });
    ...
    it('câu lệnh trỏ vào thực thể THẬT vẫn phải qua bảng trắng như mọi patch khác', () => {
        ...
        const kq = bocTach("<UpdateVariable>_.set('worlds.w1.playerState.mode', 'pham_nhan', 'sang_the');</UpdateVariable>", { eventId: 'ev1', idHopLe: new Set(['w1']) });
        expect(kq.patches).toHaveLength(0);
    });
    it('[BB] 69.1 — cú pháp MVU không sửa được lõi bản ngã', () => {
        const kq = bocTach("<UpdateVariable>_.set('deity_x.aspects.ban_nga.coreSelf.tuBi', 0, 100);</UpdateVariable>", { eventId: 'ev1', idHopLe: new Set(['deity_x']) });
        expect(kq.patches).toHaveLength(0);
        expect(kq.biTuChoi.some((b) => b.ma === 'DUONG_DAN_CAM')).toBe(true);
    });
    it('bản đồ đường dẫn của 31.7 với {_op:add} thành patch cộng dồn', () => {
        const kq = bocTach('<UpdateVariable>{"concept_x.aspects.conceptual.trongSo": {"_op":"add","_v":45}}</UpdateVariable>', { eventId: 'ev1', idHopLe: new Set(['concept_x']) });
