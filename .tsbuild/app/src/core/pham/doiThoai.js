 ...
 */
export const LOAI_PHAT_NGON = ['loi_hua', 'de_doa', 'thu_nhan', 'giao_keo', 'tin_moi', 'menh_lenh'];
export const NHAN_PHAT_NGON = Object.freeze({
    loi_hua: 'lời hứa',
    de_doa: 'lời đe dọa',
 ...
 */
export function mucHieu(state, phatNgon) {
    ...
    if (vNghe !== null && vNoi !== null && vNghe !== vNoi)
        m -= 0.2;
    const mNghe = docAspect(nghe, 'mortal');
    if (mNghe?.ageBand === 'child')
        m -= 0.35;
    if (mNghe?.ageBand === 'elder')
        m -= 0.05;
    if (phatNgon.loai === 'giao_keo') {
        const kn = docAspect(nghe, 'mortal')?.kyNang ?? {};
 ...
 */
export function noi(state, pn, nc) {
    ...
    patches.push(...datQuanHe(state, pn.nguoiNgheId, pn.nguoiNoiId, {
...
function cauAnTuong(loai, laNoiDoi, hieu) {
    if (hieu < 0.5)
        return 'Ta không rõ người ấy định nói gì.';
    if (loai === 'de_doa')
        return 'Người ấy đã dọa ta một lần.';
    if (loai === 'thu_nhan')
        return 'Người ấy kể ta nghe một chuyện họ giấu người khác.';
    if (loai === 'loi_hua')
        return 'Người ấy hứa với ta một điều.';
    if (loai === 'giao_keo')
        return 'Ta và người ấy có giao kèo.';
    if (laNoiDoi)
        return 'Có gì đó trong lời người ấy không khớp.';
    return 'Người ấy có nói với ta một chuyện.';
}
function cauKeDoiThoai(tenNoi, tenNghe, loai, hieu, soNgheLon) {
 ...
 */
export function xuLyLoiHua(state, nguoiHuaId, boiVuId, giu, nc) {
    ...
    if (bv.toId) {
        patches.push(...datQuanHe(state, bv.toId, nguoiHuaId, {
