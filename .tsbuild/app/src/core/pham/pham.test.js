...
import { canTroLamNghe, doiNghe, kyNangCuaNghe, lamMotNhip, sinhKeCua, truyenNghe, xinHoc, } from './sinhKe.js';
import { lapHo, nhapHo, nuoiHo, tachHo, giaiTheHo, hoCua, hoCuaNguoi, nguoiThuaKe, chuyenThuaKe, } from './ho.js';
 ...
 */
function ap(state, log, id, patchesTho, loai = 'test') {
    if (patchesTho.length === 0)
        return;
...
function themNguoi(state, log, soNguoi, noiId) {
    ...
    const r = vatChatHoa(state, { noiId: noi, soNguoi, eventId: evId, band: 'adult' });
...
describe('[BB] 70.5 — sức khỏe không phải thanh máu', () => {
    it('thương tích có vị trí, nguyên nhân và chặn ĐÚNG việc', () => {
        ...
        const e = state.entities.get(NGUOI);
        expect(e).toBeDefined();
        const r = gayThuongTich(e, { loai: 'gay', viTri: 'chan_trai', nang: 0.7, nguyenNhanEventIds: ['ev_nga_gian_giao'] }, nc(state, 'tt'));
    ...
    it('có người chăm thì lành nhanh hơn hẳn', () => {
        const { state, log } = theGioi();
        const e = state.entities.get(NGUOI);
        const r = gayThuongTich(e, { loai: 'rach', viTri: 'tay_phai', nang: 0.6, nguyenNhanEventIds: [] }, nc(state, 'a'));
        ap(state, log, nc(state, 'a').eventId, r.patches, 'thuong_tich');
        const nangSau = (soBuoc, coCham) => {
            const ban = JSON.parse(JSON.stringify(state.entities.get(NGUOI)));
            const m = phamThan(ban);
            if (coCham && m.thanThe.thuongTich[0])
                m.thanThe.thuongTich[0].nguoiChamId = 'ai_do';
    ...
    it('vết không ai chăm có thể biến chứng; vết đã lành thì hết nặng', () => {
        const { state, log } = theGioi();
        const e = state.entities.get(NGUOI);
        const r = gayThuongTich(e, { loai: 'gay', viTri: 'than', nang: 0.9, nguyenNhanEventIds: [] }, nc(state, 'c'));
        ap(state, log, nc(state, 'c').eventId, r.patches, 'thuong_tich');
        for (let i = 0; i < 40; i++) {
            const cur = state.entities.get(NGUOI);
            const kq = troiThanThe(cur, nc(state, `d${i}`), 1);
        ...
        if (t?.trangThai === 'da_lanh')
            expect(t.nang).toBe(0);
    });
    ...
    it('thân thể kể lại bằng câu, không bằng số — 56.2 quy tắc 1', () => {
        const { state, log } = theGioi();
        const e = state.entities.get(NGUOI);
        const r = gayThuongTich(e, { loai: 'gay', viTri: 'chan_trai', nang: 0.8, nguyenNhanEventIds: [] }, nc(state, 'e'));
...
describe('[BB] 70.2 — sống bằng nghề, học nghề, đổi nghề, truyền nghề', () => {
    it('kỹ năng lên từ việc ĐÃ LÀM, không từ nút bấm', () => {
        ...
        for (let i = 0; i < 6; i++) {
            const e = state.entities.get(NGUOI);
            const r = lamMotNhip(state, e, nc(state, `lam${i}`), 1);
    ...
    it('gãy tay thì KHÔNG tiến bộ nghề mộc — không phải bị phạt, mà là không có buổi làm', () => {
        ...
        expect(canTroLamNghe(e0, 'nghe_moc')).toBeNull();
        const r = gayThuongTich(e0, { loai: 'gay', viTri: 'tay_phai', nang: 0.8, nguyenNhanEventIds: [] }, nc(state, 'f'));
        ap(state, log, nc(state, 'f').eventId, r.patches, 'thuong_tich');
        const e1 = state.entities.get(NGUOI);
        expect(canTroLamNghe(e1, 'nghe_moc')).toBe('Tay chưa làm được việc tinh.');
        const lam = lamMotNhip(state, e1, nc(state, 'g'), 1);
    ...
    it('thợ chưa đủ bậc thì không ai học của họ', () => {
        ...
        const r = xinHoc(state, tro, NGUOI, nc(state, 'h'));
    ...
    it('đủ bậc thì nhận trò, và truyền nghề để lại tiếng tăm cho CẢ HAI', () => {
        ...
        const xh = xinHoc(state, tro, NGUOI, nc(state, 'i'));
        expect(xh.ok, xh.ok ? '' : JSON.stringify(xh.errors)).toBe(true);
        if (!xh.ok)
            return;
        ap(state, log, 'ev_xin_hoc', xh.value.patches, 'nhan_hoc_tro');
        expect(sinhKeCua(state.entities.get(tro))?.thayId).toBe(NGUOI);
        // Trò chưa đủ tay nghề thì chưa ra nghề được.
        expect(truyenNghe(state, NGUOI, tro, nc(state, 'j')).ok).toBe(false);
        ap(state, log, 'ev_tro_gioi', [
            {
                op: 'set',
                target: { table: 'entities', id: tro, path: 'aspects.mortal.kyNang.nghe_chinh' },
                value: 40,
                sourceEventId: 'ev_tro_gioi',
            },
        ], 'test_tro');
        const tn = truyenNghe(state, NGUOI, tro, nc(state, 'k'));
        expect(tn.ok, tn.ok ? '' : JSON.stringify(tn.errors)).toBe(true);
        if (!tn.ok)
            return;
        ap(state, log, 'ev_truyen', tn.value.patches, 'truyen_nghe');
        const ccThay = state.entities.get(NGUOI)?.aspects['can_cuoc'];
        expect(ccThay.tiengTam.some((t) => t.startsWith('Đã truyền nghề'))).toBe(true);
        expect(ccThay.duocNhoBoi).toBeGreaterThan(0);
        expect(sinhKeCua(state.entities.get(tro))?.thayId).toBeNull();
    });
...
describe('[BB] 70.2 — hộ: ăn chung nghĩa là đói chung', () => {
    function lap(state, log, chuId, them = []) {
        const noiO = noiOCua(state, chuId);
        expect(noiO).not.toBeNull();
        const r = lapHo(state, { chuHoId: chuId, thanhVien: them.map((id) => ({ id, vai: 'con' })), noiOId: noiO }, { eventId: `ev_lap_${chuId}`, tick: state.world.tick });
        expect(r.ok, r.ok ? '' : JSON.stringify(r.errors)).toBe(true);
        if (!r.ok)
            throw new Error('không lập được hộ');
    ...
    it('hộ tan thì kho về VÙNG, không bốc hơi', () => {
        const { state, log } = theGioi();
        const hoId = lap(state, log, NGUOI);
        const noiO = hoCua(state.entities.get(hoId))?.noiOId;
        ap(state, log, 'ev_kho2', [
            {
                op: 'set',
                target: { table: 'entities', id: hoId, path: 'aspects.ho.kho.luongThuc' },
                value: 40,
        ...
        const truoc = (state.entities.get(noiO)?.aspects['kinh_te']).kho
            .luongThuc;
        // Người duy nhất trong nhà chết → nhà tan.
        ap(state, log, 'ev_chet_het', [
            {
                op: 'set',
        ...
        const sau = (state.entities.get(noiO)?.aspects['kinh_te']).kho
            .luongThuc;
...
describe('[BB] 70.4 — đối thoại cũng là hành động', () => {
    it('nói một điều CÓ HẬU QUẢ sinh tri thức, quan hệ và nghĩa vụ', () => {
        ...
        const qh = quanHeCua(state, b, NGUOI);
    ...
    it('trẻ con hiểu ít hơn người lớn — mức hiểu không mặc định 100%', () => {
        ...
        ap(state, log, 'ev_tre', [
            {
                op: 'set',
                target: { table: 'entities', id: b, path: 'aspects.mortal.ageBand' },
                value: 'child',
    ...
    it('phá lời hứa KHÔNG xóa nghĩa vụ — nó để lại sẹo', () => {
        ...
        const bvId = (phamThan(state.entities.get(NGUOI))?.boiVu ?? [])[0]?.id;
        ...
        expect(quanHeCua(state, b, NGUOI).anTuong).toContain('không làm');
        const cc = state.entities.get(NGUOI)?.aspects['can_cuoc'];
        expect(cc.tiengTam.some((t) => t.includes('phá một lời hứa'))).toBe(true);
    });
    it('[BB] 11.2 — quan hệ bất đối xứng, không ai đồng bộ hai chiều', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_qh', datQuanHe(state, NGUOI, b, { cong: { yeuGhet: 40 }, anTuong: 'Ta quý người này.' }, 'ev_qh'), 'quan_he');
        expect(quanHeCua(state, NGUOI, b).yeuGhet).toBe(40);
        // Chiều ngược lại KHÔNG bị đụng tới.
        expect(quanHeCua(state, b, NGUOI).yeuGhet).toBe(0);
...
describe('cổng Phase 7 — NPC ngoài cảnh giữ lịch và vị trí', () => {
    ...
    it('NPC không ai nhìn suốt bốn mươi nhịp vẫn ở đúng chỗ và đang làm việc', () => {
        ...
        if (!con || con.tickDiet !== null)
            return; // chết thì không còn lịch, và đó là đúng
    ...
    it('người ốm nặng thì lịch đổi thành nằm, không còn khối làm', () => {
        const { state, log } = theGioi();
        const e = state.entities.get(NGUOI);
        const r = gayThuongTich(e, { loai: 'benh', viTri: 'trong', nang: 0.8, nguyenNhanEventIds: [] }, nc(state, 'r'));
...
describe('[BB] 70.3 — giáng hạng giảm độ phân giải, KHÔNG xóa đời sống', () => {
    it('giáng hạng giữ ba quan hệ mạnh nhất và NÉN phần còn lại có đường khôi phục', () => {
        ...
        them.forEach((id, i) => {
            patches.push(...datQuanHe(state, NGUOI, id, { cong: { thanSo: 90 - i * 15 }, anTuong: `quen ${i}` }, 'ev_qh_nhieu'));
        });
        ...
        const soul = state.entities.get(NGUOI)?.aspects['soul'];
        ...
        const sau = state.entities.get(NGUOI)?.aspects['soul'];
        for (const id of them)
            expect(sau.quanHe[id]).toBeDefined();
    });
...
describe('[BB] 20.3 — chết không Game Over', () => {
    it('thừa kế chia claim đúng, không nhân đôi', () => {
        ...
        expect(nguoiThuaKe(state, NGUOI)
            .map((x) => x.nguoiId)
            .sort()).toEqual([...con].sort());
    ...
    it('luôn có ít nhất đường CHỨNG KIẾN khi thế giới còn người', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_quen', datQuanHe(state, NGUOI, b, { anTuong: 'bạn cũ', cong: { thanSo: 40 } }, 'ev_quen'), 'quan_he');
        const r = chet(state, NGUOI, { eventId: 'ev_chet', tick: state.world.tick });
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
    ...
    it('anh linh hóa thần THÊM aspect, giữ nguyên hồn và quan hệ', () => {
        ...
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_nho', [
            ...datQuanHe(state, NGUOI, b, { anTuong: 'bạn cũ' }, 'ev_nho'),
            {
                op: 'set',
        ...
        expect(quanHeCua(state, NGUOI, b).anTuong).toBe('bạn cũ');
    });
    it('người sinh sau khi ta mất chỉ biết ta như huyền thoại', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_q', datQuanHe(state, b, NGUOI, { anTuong: 'x' }, 'ev_q'), 'quan_he');
        const c = chet(state, NGUOI, { eventId: 'ev_chet', tick: state.world.tick });
        if (!c.ok)
            return;
        ap(state, log, 'ev_chet', c.value.patches, 'nguoi_chet');
        ...
        expect(quanHeCua(state, b, NGUOI).laHuyenThoai).toBe(true);
    });
    it('một đời BÌNH THƯỜNG vẫn để lại event, quan hệ và di sản', () => {
        ...
        const xh = xinHoc(state, tro, NGUOI, nc(state, 's'));
        if (xh.ok)
            ap(state, log, 'ev_hoc', xh.value.patches, 'nhan_hoc_tro');
        chay(state, log, 10);
        const c = chet(state, NGUOI, { eventId: 'ev_chet_cuoi', tick: state.world.tick });
        if (c.ok)
            ap(state, log, 'ev_chet_cuoi', c.value.patches, 'nguoi_chet');
        // Di sản: có event, có người còn nhớ, và có một học trò mang nghề đi tiếp.
        expect(log.soLuong()).toBeGreaterThan(soEventDau);
        expect(sinhKeCua(state.entities.get(tro))?.ngheId).not.toBeNull();
...
describe('cổng Phase 7 — mở một Project nghề nghiệp và một quan hệ', () => {
    it('có cả hai loại ứng viên, mỗi cái có chặng đo được', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_cai_nhau', datQuanHe(state, NGUOI, b, { cong: { tinNgo: -40, yeuGhet: -20 } }, 'ev_cai_nhau'), 'quan_he');
        ...
        const dsMoi = ungVienDuAnNguoi(state, b);
        ...
        for (const u of [...ds, ...dsMoi])
            expect(u.milestones.length).toBe(2);
    });
    ...
    it('[BB] 68.3 — tiến độ nghề đo TỪ THẾ GIỚI, không ai khai được', () => {
        ...
        const ung = ungVienDuAnNguoi(state, ai).find((u) => u.loai === 'ra_nghe');
        expect(ung, 'người có nghề mà chưa lên thợ cả thì phải có việc này').toBeDefined();
        if (!ung)
            return;
        const pj = moDuAnNguoi(state, ai, ung, state.world.tick);
        ...
        for (let i = 0; i < 10; i++) {
            const e = state.entities.get(ai);
            const r = lamMotNhip(state, e, nc(state, `w${i}`), 1);
            expect(r.lyDoNghi).toBeNull();
    ...
    it('Project quan hệ tiến khi bên kia tin lại', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_xau', datQuanHe(state, NGUOI, b, { cong: { tinNgo: -40 } }, 'ev_xau'), 'quan_he');
        ...
        ap(state, log, 'ev_tin_lai', datQuanHe(state, b, NGUOI, { cong: { tinNgo: 30 }, anTuong: 'người ấy quay lại' }, 'ev_tin_lai'), 'quan_he');
...
describe('[BB] 56.1 — Sổ Tay không lộ số engine', () => {
    ...
    it('không khóa engine nào lọt vào trang giấy', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_qh_st', datQuanHe(state, NGUOI, b, { anTuong: 'nàng nghĩ ta giấu chuyện gì đó', xungHo: 'vợ' }, 'ev_qh_st'), 'quan_he');
        const r = gayThuongTich(state.entities.get(NGUOI), { loai: 'gay', viTri: 'chan_trai', nang: 0.5, nguyenNhanEventIds: [] }, nc(state, 't'));
    ...
    it('quan hệ ghi bằng `anTuong`, không bằng bốn trục — quy tắc 4', () => {
        const { state, log } = theGioi();
        const [b] = themNguoi(state, log, 1);
        ap(state, log, 'ev_qh_st2', datQuanHe(state, NGUOI, b, { anTuong: 'ta nợ hắn một mạng', cong: { noOn: 80 } }, 'ev_qh_st2'), 'quan_he');
        const so = soTayCua(state, NGUOI);
    ...
    it('nghi thức vô ích hiện đúng câu của 56.1 — quy tắc 2', () => {
        const { state } = theGioi();
        const so = soTayCua(state, NGUOI);
        expect(so.tin.some((c) => c.includes('Lễ Tẩy Tro') && c.includes('Chưa lần nào thấy khác đi'))).toBe(true);
    });
...
describe('cổng Phase 7 — thế giới vẫn đúng', () => {
    ...
    it('chơi trọn một vòng phàm nhân KHÔNG chạm tầng Thần lần nào', () => {
        ...
        if (r.ok)
            ap(state, log, nc(state, 'v').eventId, r.value.patches, 'doi_thoai');
        const lh = lapHo(state, {
            chuHoId: NGUOI,
            thanhVien: [{ id: ban, vai: 'ban_doi' }],
            noiOId: noiOCua(state, NGUOI),
        }, { eventId: 'ev_nha', tick: state.world.tick });
        expect(lh.ok, lh.ok ? '' : JSON.stringify(lh.errors)).toBe(true);
        if (lh.ok)
            ap(state, log, 'ev_nha', lh.value.patches, 'lap_ho');
        chay(state, log, 30);
