 ...
 */
export function giangHang(state, nguoiId, nc) {
    const e = state.entities.get(nguoiId);
    const s = hon(e);
    if (!e || !s)
        return hong([loi('intent', 'KHONG_CO_HON', 'Thực thể này không có hồn để giáng hạng.')]);
    ...
    if (nen.length > 0) {
        ...
        const tb = (lay) => Math.round(nen.reduce((t, x) => t + lay(x.qh), 0) / nen.length);
 ...
 */
export function thangHang(state, nguoiId, nc) {
    const e = state.entities.get(nguoiId);
    const s = hon(e);
    if (!e || !s)
        return hong([loi('intent', 'KHONG_CO_HON', 'Thực thể này không có hồn để thăng hạng.')]);
    const quanHeMoi = {};
    for (const [id, q] of Object.entries(s.quanHe ?? {})) {
        if (id !== KHOA_NEN)
            quanHeMoi[id] = QuanHeMotChieuSchema.parse(q);
    }
 ...
 */
export function hangNenO(soChang, laChuThe, banKinhGan) {
    if (laChuThe)
        return 't3';
    if (soChang === null)
        return 't1';
