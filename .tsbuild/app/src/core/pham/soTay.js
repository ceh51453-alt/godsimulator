...
const SO_TAY_RONG = Object.freeze({
...
export function doTinTheoChang(hops, nguon) {
    if (hops <= 0)
        return 'ta thấy tận mắt';
    if (hops === 1)
        return `${nguon === '' ? 'có người' : nguon} kể lại`;
    if (hops === 2)
        return 'nghe qua hai miệng';
    if (hops === 3)
        return 'nghe qua ba miệng';
 ...
 */
export function dungSoTay(ng) {
    const { view } = ng;
    const toi = view.chuTheId ? view.entities.get(view.chuTheId) : undefined;
    if (!toi)
        return SO_TAY_RONG;
    const m = doc(toi, 'mortal');
    const sk = doc(toi, 'sinh_ke');
    const cc = doc(toi, 'can_cuoc');
    const soul = doc(toi, 'soul');
    // ── mở đầu ──
    const moDau = [];
    ...
    if (cc && cc.phapLy !== 'tu_do') {
        moDau.push(cc.phapLy === 'dang_bi_truy'
    ...
    if (noChua.length === 1)
        than.push(`Còn một điều chưa làm xong: ${noChua[0]?.description}`);
    else if (noChua.length > 1)
        than.push(`Còn ${noChua.length} điều đã hứa mà chưa làm.`);
    // ── người ta quen ──
    ...
    for (const [id, qhTho] of Object.entries(soul?.quanHe ?? {})) {
        ...
        if (!kia)
            continue; // không thấy được thì không nhớ được
        quen.push({
            ten: kia.ten,
            xungHo: qh.xungHo,
            anTuong: qh.anTuong,
    ...
    for (const nt of ng.nghiThucVoIch) {
        if (nt.soLan <= 0)
            continue;
        tin.push(`Ta đã làm ${nt.ten} ${nt.soLan} lần. ${nt.soLan >= 3 ? 'Chưa lần nào thấy khác đi.' : 'Chưa rõ có ăn thua không.'}`);
    }
    // ── điều ta nghe được ──
    const dsNghe = [];
    ...
    // ── điều ta muốn ──
    const muon = (m?.mucTieuDoiNguoi ?? []).map((g) => ({ noiDung: g, xong: false }));
    if (sk?.thayId) {
 ...
 */
export const KHOA_ENGINE_CAM = Object.freeze([
    'trongSo',
    'thieuHut',
    'tyLeMac',
    'cohort',
