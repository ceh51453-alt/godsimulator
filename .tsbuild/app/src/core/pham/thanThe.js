 ...
 */
export function gayThuongTich(e, yc, nc) {
    const m = phamThan(e);
    ...
    return {
        patches: [
            ...
            dat(e.id, 'aspects.mortal.thanThe.sinhLuc', Math.max(0, Math.round((m?.thanThe.sinhLuc ?? 100) - tt.nang * 30)), nc.eventId),
        ],
        thuongTich: tt,
 ...
 */
export function nhanChamSoc(e, ttId, nguoiChamId, nc) {
    const m = phamThan(e);
 ...
 */
export function troiThanThe(e, nc, soBuocGop = 1) {
    const m = phamThan(e);
    if (!m)
    ...
    if (ds.length === 0 && m.thanThe.dau === 0)
        return { patches: [], suKien: [] };
    const moi = ds.map((t) => ({ ...t }));
    for (const t of moi) {
        if (t.trangThai === 'da_lanh' || t.trangThai === 'di_chung')
            continue;
    ...
    const dau = Math.min(100, Math.round(moi.filter((t) => t.trangThai !== 'da_lanh').reduce((s, t) => s + t.nang * 45, 0)));
...
function cauDiChung(viTri, loai) {
    ...
    if (viTri === 'dau')
        return 'Có những buổi đầu óc không rõ ràng.';
    if (loai === 'bong')
        return 'Vết sẹo không mất đi.';
 ...
 */
export function thanTheKeLai(m) {
    if (!m)
        return [];
    const ra = [];
    for (const t of m.thanThe.thuongTich) {
    ...
    if (m.thanThe.doDoi >= 70)
        ra.push('Bụng đói suốt.');
    else if (m.thanThe.doDoi >= 40)
        ra.push('Bữa cơm mỏng đi.');
    if (m.thanThe.theLuc <= 30)
        ra.push('Làm một lúc là hết hơi.');
    if (m.ageBand === 'elder')
        ra.push('Tuổi rồi, không nhanh như xưa.');
 ...
 */
export function daChet(m) {
    if (!m)
        return { chet: false, chuoiNguyenNhan: [] };
    const chuoi = [];
    if (m.thanThe.sinhLuc <= 0)
        chuoi.push('sinh_luc_can');
    if (m.thanThe.doDoi >= 100)
        chuoi.push('doi_qua_lau');
    const nangNhat = m.thanThe.thuongTich.find((t) => t.trangThai === 'bien_chung' && t.nang >= 0.9);
    if (nangNhat)
        chuoi.push(`bien_chung:${nangNhat.id}`);
    for (const t of m.thanThe.thuongTich) {
        if (t.trangThai !== 'da_lanh' && t.nang >= 0.5)
            chuoi.push(`thuong_tich:${t.id}`);
    }
