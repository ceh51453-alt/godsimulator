 ...
 * biến mất.
 */
export function veSinh(tho, tran = TRAN_HIEN_THI) {
 ...
 */
export function moTaVet(v) {
    const phan = [];
    if (v.soBiDoiChieu > 0)
        phan.push(`${v.soBiDoiChieu} ký tự đảo chiều văn bản`);
    if (v.soVoHinh > 0)
        phan.push(`${v.soVoHinh} ký tự vô hình`);
    if (v.soDieuKhien > 0)
        phan.push(`${v.soDieuKhien} ký tự điều khiển`);
    if (v.daCat)
        phan.push('đoạn quá dài đã bị cắt');
