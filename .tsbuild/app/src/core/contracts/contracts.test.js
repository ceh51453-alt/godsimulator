...
import { ExprNodeSchema, PatchTemplateSchema, EntityRefSchema, BlockReasonSchema, ImportIssueSchema, scopeKeyOf, } from './primitives.js';
...
describe('bốn hợp đồng lõi — Phần 61.3', () => {
    ...
    it('Event bắt buộc có source và hash; strict từ chối trường lạ', () => {
        ...
        expect(EventSchema.safeParse({ id: 'ev1', branchId: 'br', tick: 0, loai: 'x', hash: 'h' }).success).toBe(false);
    ...
    it('Event tick không được âm', () => {
        expect(EventSchema.safeParse({ id: 'e', branchId: 'b', tick: -1, loai: 'x', source: 'engine', hash: 'h' })
            .success).toBe(false);
    });
    ...
    it('PatchOp mang sourceEventId — mọi thay đổi truy được về một Event', () => {
        ...
        expect(PatchOpSchema.safeParse({
            op: 'set',
            target: { table: 'entities', id: 'x', path: 'ten' },
            value: 'y',
            sourceEventId: 'ev1',
...
describe('ExprNode — AST giới hạn, không eval', () => {
    ...
    it('JSON round-trip giữ nguyên nghĩa', () => {
        const e = { op: 'or', args: [{ op: 'literal', value: 1, args: [] }] };
