...
export const GiaTuNhienSchema = z
    ...
    .strict();
const g = (x) => GiaTuNhienSchema.parse(x);
/**
...
export function kenhKhaDung(dieuKien) {
    return KENH_DUNG_SAN.filter((k) => {
        if (k.gia.canLuatNen.some((l) => !dieuKien.luatNenCoSan.includes(l)))
            return false;
        // Không có ai thờ thì nói với ai? Dấu hiệu vẫn gửi được — thế giới vẫn ở đó.
        if (!dieuKien.coTinDo && (k.id === 'giao_ly' || k.id === 'su_gia'))
            return false;
        if (!dieuKien.coTheChe && k.id === 'giao_ly')
            return false;
        if (!dieuKien.coThanKhac && k.id === 'ngoai_giao_than')
            return false;
        return true;
    });
}
