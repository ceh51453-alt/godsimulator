 ...
 */
export function tachPhanThan(state, thanId, nc) {
    ...
    const chia = docAspect(than, 'divisible');
     ...
     */
    patches.push(set(thanId, 'aspects.divisible', DivisibleSchema.parse({
        banTheGocId: chia?.banTheGocId ?? null,
        phanThanIds: [...(chia?.phanThanIds ?? []), ptId],
        doPhanKy: 0,
        nguongHopNhat: chia?.nguongHopNhat ?? 60,
        thamQuyenDuocChia: [...(chia?.thamQuyenDuocChia ?? [])],
    }), nc.eventId), set(thanId, 'aspects.venerable.soTinDoUocLuong', tinDoGoc, nc.eventId), 
    ...
    set(thanId, 'aspects.ban_nga.followerImage', { ...bn.coreSelf }, nc.eventId), set(thanId, 'aspects.ban_nga.currentManifestation', hinhHienTai(bn.coreSelf, bn.coreSelf, ven?.hienThanh ?? 20), nc.eventId), set(thanId, 'aspects.ban_nga.pressure.distortion', 0, nc.eventId), set(thanId, 'aspects.ban_nga.pressure.suppressedTraits', [], nc.eventId), set(thanId, 'aspects.ban_nga.pressure.demandedTraits', [], nc.eventId));
    return dat({
        patches,
        phanThanId: ptId,
        loiKe: `${than.ten} tách làm hai. "${ten}" bước ra mang đúng khuôn mặt mà ${tinDoMoi} người đã dựng nên, ` +
 ...
 */
export function doPhanKy(state, thanId, evId) {
    const goc = state.entities.get(thanId);
    const chia = goc ? docAspect(goc, 'divisible') : undefined;
    const bnGoc = goc ? docAspect(goc, 'ban_nga') : undefined;
    if (!goc || !chia || !bnGoc || chia.phanThanIds.length === 0)
        return [];
    const ra = [];
    let xaNhat = 0;
    for (const ptId of [...chia.phanThanIds].sort()) {
        const pt = state.entities.get(ptId);
        const bnPt = pt ? docAspect(pt, 'ban_nga') : undefined;
        if (!pt || !bnPt || pt.tickDiet !== null)
            continue;
    ...
    if (ra.length > 0)
        ra.push(set(thanId, 'aspects.divisible.doPhanKy', Math.round(xaNhat), evId));
    return ra;
}
/** Hai bản thể còn gần nhau đủ để hợp lại không — 12.3. */
export function hopNhatDuoc(state, gocId, ptId) {
    const goc = state.entities.get(gocId);
    const pt = state.entities.get(ptId);
    const chia = goc ? docAspect(goc, 'divisible') : undefined;
    const bnGoc = goc ? docAspect(goc, 'ban_nga') : undefined;
    const bnPt = pt ? docAspect(pt, 'ban_nga') : undefined;
    if (!goc || !pt || !chia || !bnGoc || !bnPt)
        return false;
