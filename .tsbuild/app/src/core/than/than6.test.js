...
import { boPhieu, boTrongGheDau, ketNap, moNghiQuyet, tiengNoiCua, traoGheDau, trucXuat, ungVienKeVi, } from './hoiDong.js';
...
function ap(state, log, id, patches, loai = 'test') {
    if (patches.length === 0)
        return;
    const ev = taoEvent({
        id,
 ...
 */
function doiLoi(id, giaTri, evId) {
...
describe('bộ chọn chủ thể — đóng lỗi "bấm Thần ra Phàm Nhân"', () => {
    ...
    it('chỉ trả về đúng kind của tầng', () => {
        ...
        for (const u of chonChuThe(state, 'than'))
            expect(state.entities.get(u.id)?.kind).toBe('deity');
        for (const u of chonChuThe(state, 'pham_nhan'))
            expect(state.entities.get(u.id)?.kind).toBe('mortal');
    });
    ...
    it('chủ thể đang nhập luôn đứng đầu — quay lại đúng chỗ mình vừa rời', () => {
        ...
        if (ds.length < 2)
            return; // thế giới hạt giống có thể chỉ có một vị thần
        const cuoi = ds[ds.length - 1]?.id;
        ap(state, log, 'ev_nhap_cuoi', [
            {
                op: 'set',
                target: { table: 'worlds', id: 'worlds', path: 'playerState.chuTheId' },
                value: cuoi,
    ...
    it('thế giới không có thần thì tầng Thần báo không khả dụng, không im lặng', () => {
        ...
        for (const [id, e] of [...state.entities.entries()]) {
            if (e.kind === 'deity')
                state.entities.set(id, { ...e, tickDiet: 0 });
        }
...
describe('[BB] 19.4 — hóa thân hạ tầm nhìn xuống mức phàm nhân', () => {
    function haPham(state, log, mucQuen = 80) {
        const noi = [...state.entities.values()].find((e) => e.kind === 'place');
        const r = hoaThan(state, {
            thanId: THAN,
            thanTheId: null,
            vungId: noi?.id ?? null,
            ten: 'Người Gánh Nước',
        ...
        if (!r.ok)
            throw new Error('không hóa thân được');
    ...
    it('đang hóa thân: KHÔNG đọc được sổ sách tín ngưỡng của chính mình nữa', () => {
        ...
        const truoc = chieu(state, 'than', THAN).entities.get(THAN)?.aspects['venerable'];
        ...
        const sau = chieu(state, 'than', THAN).entities.get(THAN)?.aspects['venerable'];
    ...
    it('quyền năng còn lại do engine suy từ mức quên, không do người chơi khai', () => {
        const { state, log } = theGioi();
        haPham(state, log, 90);
        const av = state.entities.get(THAN)?.aspects['avatar'];
...
describe('[BB] 12.3 — phân thân tách thành entity thật', () => {
    ...
    it('bản thể mới lấy HÌNH ẢNH tín đồ làm lõi; bản gốc giữ lõi thật', () => {
        const { state, log } = theGioi();
        const bnTruoc = state.entities.get(THAN)?.aspects['ban_nga'];
        const ptId = tach(state, log);
        const bnPt = state.entities.get(ptId)?.aspects['ban_nga'];
        const bnGoc = state.entities.get(THAN)?.aspects['ban_nga'];
    ...
    it('tín đồ đi theo hình ảnh — bản gốc mất phần lớn người và toàn bộ đền', () => {
        ...
        const ptId = tach(state, log);
        const goc = state.entities.get(THAN)?.aspects['venerable'];
        const pt = state.entities.get(ptId)?.aspects['venerable'];
    ...
    it('phân thân làm tan áp lực Dị Hóa của bản gốc', () => {
        ...
        tach(state, log);
        const bn = state.entities.get(THAN)?.aspects['ban_nga'];
    ...
    it('hai bản thể nối bằng quan hệ hai chiều — không thực thể mồ côi (6.3)', () => {
        ...
        expect(canh.some((lk) => lk.tuId === ptId && lk.denId === THAN && lk.quanHe === 'phan_than_cua')).toBe(true);
        expect(canh.some((lk) => lk.tuId === THAN && lk.denId === ptId && lk.quanHe === 'co_phan_than')).toBe(true);
        expect(chayInvariantToanBo(state).dat).toBe(true);
    });
    it('phân kỳ đo khoảng cách THẬT, không phải bộ đếm theo thời gian', () => {
        ...
        const d1 = (state.entities.get(ptId)?.aspects['divisible']).doPhanKy;
        ...
        const d2 = (state.entities.get(ptId)?.aspects['divisible']).doPhanKy;
    ...
    it('trôi xa quá ngưỡng thì không hợp nhất lại được', () => {
        ...
        ap(state, log, 'ev_gan', [...doiLoi(ptId, 0, 'ev_gan'), ...doiLoi(THAN, 0, 'ev_gan')], 'than_tu_dinh_nghia');
        expect(hopNhatDuoc(state, THAN, ptId)).toBe(true);
        // Rồi đẩy ra hai đầu — khoảng cách 100 vượt `nguongHopNhat` mặc định 60.
        ap(state, log, 'ev_xa', [...doiLoi(ptId, 100, 'ev_xa'), ...doiLoi(THAN, -100, 'ev_xa')], 'than_tu_dinh_nghia');
...
describe('[BB] 69.3 — hội đồng thần và kế vị', () => {
    ...
    function dungHoiDong(state, log, thanIds) {
    ...
    function baThan(state, log) {
        ...
        if (!b.ok)
            return [THAN, a.value.phanThanId];
    ...
    it('kết nạp cho một vị thần cái ghế, và ghế đó có phiếu', () => {
        ...
        dungHoiDong(state, log, [ds[0]]);
        const r = ketNap(state, HE, ds[1], { eventId: 'ev_ketnap', tick: state.world.tick });
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
    ...
    it('kẻ không có ghế thì không bỏ phiếu được', () => {
        ...
        const ds = baThan(state, log);
        dungHoiDong(state, log, [ds[0]]);
        const r = moNghiQuyet(state, HE, { id: 'nq1', loai: 'ket_nap', noiDung: 'Nhận thêm một vị', veThanIds: [] }, { eventId: 'ev_mo', tick: state.world.tick });
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
        ap(state, log, 'ev_mo', r.value.patches, 'hoi_dong_mo');
        const bp = boPhieu(state, HE, 'nq1', ds[1], 'thuan', {
            eventId: 'ev_bp',
            tick: state.world.tick,
        });
    ...
    it('kẻ bị đuổi không quay lại ngồi ghế đầu', () => {
        ...
        const ds = baThan(state, log);
        dungHoiDong(state, log, ds);
        const tx = trucXuat(state, HE, ds[1], 'phá giao ước', {
            eventId: 'ev_duoi',
            tick: state.world.tick,
        });
        ...
        const trao = traoGheDau(state, HE, ds[1], { eventId: 'ev_trao', tick: state.world.tick });
...
describe('[BB] 69.3 — thần NPC dùng Project chứ không chỉ phản ứng', () => {
    ...
    it('thần NPC tự mở việc trong lúc người chơi vắng', () => {
        const { state, log } = theGioi();
        chay(state, log, 12);
        const du = state.entities.get(THAN)?.aspects['du_an'];
