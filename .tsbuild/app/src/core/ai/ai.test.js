...
import { MACH_MOI, NGUONG_MO_MACH, danhGiaCong, dongMach, machSauKhiHong, machSauKhiThanhCong, tyLeHong, } from './cong.js';
...
describe('[BB] ADR-0028 — không có AI thì không chơi', () => {
    it('cấu hình rỗng: cổng đóng và nói rõ thiếu gì', () => {
        ...
        for (const t of cong.viecCanLam)
            expect(t.thongDiep.length).toBeGreaterThan(10);
    });
...
describe('[BB] 71.5 — LLM không giữ sổ', () => {
    ...
    it('khối <CapNhat> bị cắt khỏi lời kể', () => {
        const r = bocTach('Mưa xuống.\n<CapNhat>{"patches":[{"op":"add","target":{"table":"entities","id":"place_1","path":"aspects.spatial.danSo"},"value":2}]}</CapNhat>', nc);
    ...
    it('patch hợp lệ áp được vào thế giới thật qua Event `ai_validated`', () => {
        ...
        })}</CapNhat>`, { eventId: 'ev_ai_1', idHopLe: new Set(state.entities.keys()) });
        expect(r.patches).toHaveLength(1);
        const ev = taoEvent({
            id: 'ev_ai_1',
...
describe('bốn phương ngữ', () => {
    it('ghép đường không nhân đôi đoạn đã có', () => {
        ...
        expect(ghepDuong('https://x.y/v1/chat/completions', 'chat/completions')).toBe('https://x.y/v1/chat/completions');
        expect(ghepDuong('https://x.y/', 'chat/completions')).toBe('https://x.y/chat/completions');
    });
    ...
    it('[BB] 62.4 — chỉ gửi tham số phương ngữ hỗ trợ', () => {
        ...
        const a = dacTaGoi('anthropic', 'https://x.y', 'k', yc).body;
...
describe('client — mock pass trước network', () => {
    ...
    const traVe = (body, status = 200) => (async () => ({
        ok: status >= 200 && status < 300,
        status,
        text: async () => JSON.stringify(body),
    }));
    it('gọi kể thành công trả về văn bản', async () => {
        const r = await goiKe(ep, { heThong: 'S', nguoiDung: 'U', tang: [], soKyTu: 2, uocToken: 1, vetCat: [], chunkBiCat: [] }, { fetchImpl: traVe({ choices: [{ message: { content: 'Một cảnh.' } }] }) });
        expect(r.ok).toBe(true);
        expect(r.ok && r.vanBan).toBe('Một cảnh.');
    });
    it('model trả rỗng bị tính là HỎNG, không phải "kể xong"', async () => {
        const r = await goiKe(ep, { heThong: 'S', nguoiDung: 'U', tang: [], soKyTu: 2, uocToken: 1, vetCat: [], chunkBiCat: [] }, { fetchImpl: traVe({ choices: [{ message: { content: '' } }] }) });
    ...
    it('proxy trả HTML thì thông điệp nói rõ là sai địa chỉ', async () => {
        ...
        const r = await goiKe(ep, { heThong: '', nguoiDung: '', tang: [], soKyTu: 0, uocToken: 0, vetCat: [], chunkBiCat: [] }, { fetchImpl: html });
        expect(r.ok).toBe(false);
