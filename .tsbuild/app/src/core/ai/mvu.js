...
export const THE_KHOI_CAP_NHAT = ['CapNhat', 'UpdateVariable'];
/** Bắt cả hai thẻ, cả dạng chưa đóng — model bị cắt cụt để lại một thẻ mở. */
export function bieuThucKhoi() {
    return new RegExp(`<(?:${THE_KHOI_CAP_NHAT.join('|')})>([\\s\\S]*?)</(?:${THE_KHOI_CAP_NHAT.join('|')})>`, 'i');
}
export function bieuThucCatKhoi() {
    return new RegExp(`<(?:${THE_KHOI_CAP_NHAT.join('|')})>[\\s\\S]*?</(?:${THE_KHOI_CAP_NHAT.join('|')})>`, 'gi');
}
// ─────────────────────────────────────────── đường dẫn
/**
 ...
 */
export function phanGiaiDuongDan(duong, idHopLe) {
    ...
    for (const bang of BANG_KHAI_TRUOC) {
        if (!sach.startsWith(`${bang}.`))
            continue;
        const con = sach.slice(bang.length + 1);
        const khop = khopId(con, idHopLe);
        if (khop !== null)
            return { table: bang, id: khop.id, path: khop.path };
    ...
    for (const ung of [sach, khongTienTo]) {
        ...
        if (khop !== null && khop.path !== '')
            return { table: 'entities', id: khop.id, path: khop.path };
    }
    return null;
}
function khopId(s, idHopLe) {
    const phan = s.split('.');
    for (let k = phan.length; k >= 1; k--) {
        const ung = phan.slice(0, k).join('.');
        if (idHopLe.has(ung))
            return { id: ung, path: phan.slice(k).join('.') };
    }
    return null;
}
...
function docGiaTri(v) {
    if (v !== null && typeof v === 'object' && !Array.isArray(v)) {
        const o = v;
        if (typeof o['_op'] === 'string')
            return { op: o['_op'], value: o['_v'] };
    }
 ...
 */
export function docKhoiCapNhat(than, idHopLe) {
    const sach = than
        .trim()
        .replace(/^```[a-z]*\n?/i, '')
        .replace(/```$/i, '')
...
function tuJson(doc, idHopLe) {
    if (Array.isArray(doc))
        return { tho: doc, bienPack: [], boQua: [] };
    if (doc !== null && typeof doc === 'object') {
        const o = doc;
        if (Array.isArray(o['patches']))
            return { tho: o['patches'], bienPack: [], boQua: [] };
        const tho = [];
        const bienPack = [];
        const boQua = [];
...
function tuCauLenh(s, idHopLe) {
    ...
    for (const khop of s.matchAll(CAU_LENH)) {
         ...
         */
        const cuoi = doiSo[doiSo.length - 1];
        ...
        const dich = phanGiaiDuongDan(duong, idHopLe);
        if (dich === null) {
            bienPack.push({ duong, giaTri, phep: op, lyDo });
            continue;
        }
