...
export const TuningSchema = z
    ...
    .prefault({});
export const TUNING_MAC_DINH = TuningSchema.parse({});
/**
...
export function tuningTheoHoSo(id) {
    const hs = HO_SO_CAN_BANG[id];
    return TuningSchema.parse(gopSau(TUNING_MAC_DINH, hs.patch));
}
/** Gộp sâu, chỉ trường có mặt mới bị thay — cùng ngữ nghĩa với ghi đè registry (5.2). */
function gopSau(goc, patch) {
    const out = { ...goc };
...
export function moiDuongDanTuning() {
    const out = [];
    const di = (o, tien) => {
