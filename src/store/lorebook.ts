...
async function ghi(ds: readonly MucThuVienLorebook[]): Promise<void> {
...
export type TrangThaiThuVienLorebook = {
  ...
  themTuChuoi(
    noiDung: string,
    ten: string,
    tuyChon?: { dungSan?: boolean; chonChoVanMoi?: boolean },
  ): Promise<boolean>;
  datChonChoVanMoi(id: string, chon: boolean): Promise<void>;
  xoaKhoiThuVien(id: string): Promise<void>;
};

export const useThuVienLorebook = create<TrangThaiThuVienLorebook>((set, get) => ({
  muc: [],
  daNap: false,
  dangXuLy: false,
  loi: '',
  ...
  async themTuChuoi(noiDung, ten, tuyChon = {}) {
    ...
    try {
      ...
      const moi: MucThuVienLorebook = MucThuVienLorebookSchema.parse({
        id,
        ten: tenSach,
      ...
      const ds = [...get().muc.filter((x) => x.id !== id), moi].sort((a, b) =>
