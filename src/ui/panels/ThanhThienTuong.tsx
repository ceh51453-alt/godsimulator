...
const gia: CSSProperties = { fontSize: 13, color: 'var(--tro)' };
 ...
 */
function Delta({ delta, tangLaTot }: { delta: number; tangLaTot: boolean }): JSX.Element | null {
  if (delta === 0) return null;
  const tot = delta > 0 === tangLaTot;
  return (
    <span
      className="chu-so"
      style={{ fontSize: 12, color: tot ? 'var(--ngoc)' : 'var(--hoi)' }}
      title={tot ? 'đang đi theo hướng tốt' : 'đang đi theo hướng xấu'}
    >
      {delta > 0 ? '+' : '−'}
      {Math.abs(delta)}
    </span>
...
export type MucGhimDuoc = { readonly khoa: string; readonly nhan: string };

export function ThanhThienTuong({
  cum,
  ghimDuoc = [],
  dangGhim = [],
  loiGhim = '',
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onMoBang}
        aria-label="Mở Bảng Thiên Diễn"
        title="Mở Bảng Thiên Diễn (phím Tab)"
        style={{
          display: 'flex',
          alignItems: 'center',
          gap: 22,
      ...
      >
        {cum.map((c) => (
          <span
            key={c.id}
            style={{ display: 'inline-flex', alignItems: 'baseline', gap: 7, minWidth: 0 }}
            /*
            ...
            title={coGhim && !CUM_CO_DINH.has(c.id) ? `${c.nhan} — chuột phải để bỏ ghim` : c.nhan}
          >
            <span style={nhan}>{c.nhan}</span>
            <span className="chu-so" style={gia}>
              {c.gia}
            </span>
            {c.delta === null ? null : <Delta delta={c.delta} tangLaTot={c.tangLaTot} />}
          </span>
        ))}
      </button>
      ...
      {coGhim && (
        <button
          type="button"
          aria-expanded={moChon}
          aria-label="Chọn chỉ số ghim lên thanh"
          onClick={() => setMoChon(!moChon)}
          style={{
            ...nhan,
        ...
        >
          Ghim {dangGhim.length > 0 ? `(${dangGhim.length})` : ''}
        </button>
      )}
      <span style={{ ...nhan, flex: '0 0 auto' }}>Tab để mở bảng</span>

      {moChon && coGhim && (
        <div
          role="dialog"
          aria-label="Chỉ số ghim"
          style={{
            position: 'absolute',
            top: '100%',
            right: 22,
        ...
        >
          <span style={nhan}>Ghim lên thanh</span>
          {ghimDuoc.map((m) => {
            ...
            return (
              ...
              >
                <input
                  type="checkbox"
                  checked={daGhim || coDinh}
                  disabled={coDinh}
                  onChange={(e) => (e.target.checked ? onGhim(m.khoa) : onBoGhim(m.khoa))}
                />
                <span>
                  {m.nhan}
                  {coDinh ? ' — luôn hiện' : ''}
          ...
          {loiGhim !== '' && (
            <p role="alert" style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--hoi)' }}>
              {loiGhim}
            </p>
          )}
          <button
