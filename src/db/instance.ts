...
let db: ThienDienDb | null = null;

export function layDb(): ThienDienDb {
  db ??= new ThienDienDb();
  return db;
}

/** Nhánh gốc mặc định cho migration v1 → v2 — save v1 không có cột `branchId`. */
const NHANH_GOC_MAC_DINH = 'br_goc';

let daKhoiDong: Promise<readonly StructuredError[]> | null = null;
 ...
 * xuất dữ liệu ra.
 */
export function khoiDongDb(): Promise<readonly StructuredError[]> {
  daKhoiDong ??= (async () => {
    if (!coIndexedDb()) return [];
    try {
      const r = await chayMoiMigration(layDb(), NHANH_GOC_MAC_DINH, 0);
      return r.ok ? [] : r.errors;
    } catch (e) {
      return [
        loi('migration', 'MIGRATION_NGOAI_Y_MUON', `Không chạy được migration: ${String(e)}`, {
          recoverable: true,
        }),
      ];
    }
...
export function coIndexedDb(): boolean {
  try {
