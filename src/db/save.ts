...
export async function xuatSave(
  db: ThienDienDb,
  state: WorldState,
  events: readonly Event[],
  tuyChon: TuyChonXuat = {},
): Promise<KetQua<SaveExport>> {
  const branchId = state.world.branchId;
  const branches = await db.branches.toArray();

  const goi = {
    dinhDang: 'thien-dien-save' as const,
  ...
  let hoSo: unknown;
  let danhTinh: unknown;
  if (tuyChon.kemHoSoRiengTu === true) {
    const pid = state.world.playerState.playerProfileId;
    if (pid) hoSo = await db.playerProfiles.get(pid);
    const cid = state.world.playerState.creatorIdentityId;
    if (cid) danhTinh = await db.playerIdentities.where('id').equals(cid).first();
  }
 ...
 */
export function nhapSave(tho: unknown): KetQua<KetQuaNhap> {
  ...
  const nap = <T extends { id: string }>(
    ...
    for (const tho2 of ds) {
      ...
      if (!p.success) {
        canhBao.push(
          loi('persistence', 'SAVE_BAN_GHI_HONG', `Bỏ qua một bản ghi '${ten}' không hợp lệ.`, {
            severity: 'warning',
          }),
        );
        continue;
      }
      const val = p.data as T;
      dich.set(val.id, val);
    }
  };
  ...
  for (const e of goi.events) {
    ...
    if (p.success) events.push(p.data);
      ...
      canhBao.push(
        loi('persistence', 'SAVE_EVENT_HONG', 'Bỏ qua một Event không hợp lệ.', { severity: 'warning' }),
      );
  }
  ...
  for (const b of goi.branches) {
    if (!BranchSchema.safeParse(b).success) {
      canhBao.push(
        loi('persistence', 'SAVE_BRANCH_HONG', 'Bỏ qua một bản ghi nhánh không hợp lệ.', {
          severity: 'warning',
        }),
      );
    }
  ...
  if (!hashKhop) {
    canhBao.push(
      loi('persistence', 'SAVE_HASH_LECH', 'Hash trong save không khớp state dựng lại được.', {
        severity: 'warning',
  ...
  if (!inv.dat) {
    return hong(
      [loi('persistence', 'SAVE_VI_PHAM_BAT_BIEN', 'Save vi phạm bất biến, không nạp được.')].concat(
        inv.viPhamNang as StructuredError[],
      ),
      canhBao,
    );
  }
...
export async function luuSnapshot(
  db: ThienDienDb,
  state: WorldState,
  scopeKey: string,
): Promise<SnapshotRow> {
...
export function phucHoiTuSnapshot(row: SnapshotRow): KetQua<WorldState> {
  const d = row.duLieu as {
    world?: unknown;
    entities?: unknown[];
    links?: unknown[];
    gaps?: unknown[];
    metrics?: unknown;
    // Mười bảng Phase 5 – 10. Snapshot cũ (trước bản sửa) không có chúng,
    // nên mỗi trường đều optional và mặc định là mảng rỗng.
    knowledge?: unknown[];
    debts?: unknown[];
    prayers?: unknown[];
    storylines?: unknown[];
    foreshadows?: unknown[];
    substrateLaws?: unknown[];
    coChe?: unknown[];
    lorebooks?: unknown[];
    loreExpectations?: unknown[];
    diBan?: unknown[];
  };
  ...
  for (const e of d.entities ?? []) {
    ...
    if (p.success) s.entities.set(p.data.id, p.data);
  }
  for (const l of d.links ?? []) {
    const p = LinkSchema.safeParse(l);
    if (p.success) s.links.set(p.data.id, p.data);
  }
  for (const g of d.gaps ?? []) {
    const p = GapSchema.safeParse(g);
    if (p.success) s.gaps.set(p.data.id, p.data);
  }
