...
import {
  ...
  VanHoaSchema,
  AnNinhSchema,
} from '../core/schema/aspect/substrate.js';
...
async function docCheckpoint(db: ThienDienDb, id: string): Promise<MigrationCheckpoint | undefined> {
  return db.migrationCheckpoints.get(id);
}

async function ghiCheckpoint(db: ThienDienDb, cp: MigrationCheckpoint): Promise<void> {
 ...
 */
export async function chayMigrationV1V2(
  db: ThienDienDb,
  branchMacDinh: string,
  tick: number,
): Promise<KetQua<KetQuaMigration>> {
  const ID = 'v1_v2';
  ...
  for (const ten of ['entities', 'links'] as const) {
    ...
    for (let i = 0; i < thieu.length; i += CO_LO) {
      ...
      await db.transaction('rw', [bang, db.migrationCheckpoints], async () => {
        for (const r of lo) {
          const khoaCu = [r.branchId ?? '', r.id] as [string, string];
          ...
          await bang.put({ ...r, branchId: branchMacDinh } as never);
          if (khoaCu[0] !== branchMacDinh) await bang.delete(khoaCu as never);
        }
        daXong += lo.length;
  ...
  const conThieu =
    (await db.entities.filter((r) => !r.branchId).count()) +
 ...
 */
export async function chayMigrationV2V3(db: ThienDienDb, tick: number): Promise<KetQua<KetQuaMigration>> {
  const ID = 'v2_v3';
  const canhBao: StructuredError[] = [];
  const cpCu = await docCheckpoint(db, ID);
  if (cpCu?.hoanTat) {
 ...
 */
export async function chayMigrationV3V4(db: ThienDienDb, tick: number): Promise<KetQua<KetQuaMigration>> {
  const ID = 'v3_v4';
  const canhBao: StructuredError[] = [];
  const cpCu = await docCheckpoint(db, ID);
  if (cpCu?.hoanTat) {
  ...
  const canGieo = (await db.entities.toArray()).filter(
    (e) => e.kind === 'place' && e.aspects['dan_cu'] === undefined,
  );
  ...
  const conThieu = (await db.entities.toArray()).filter(
    (e) => e.kind === 'place' && e.aspects['dan_cu'] === undefined,
  ).length;
  if (conThieu > 0) {
    return hong([
 ...
 */
function aspectNenTuSpatial(aspects: Record<string, unknown>): Record<string, unknown> {
  const sp = (aspects['spatial'] ?? {}) as { danSo?: number };
...
export function chiaCohort(danSo: number): { child: number; youth: number; adult: number; elder: number } {
...
export function sinhThaiTheoDan(danSo: number): Record<string, unknown> {
...
export async function chayMoiMigration(
  db: ThienDienDb,
  branchMacDinh: string,
  tick: number,
): Promise<KetQua<KetQuaMigration>> {
 ...
 */
export function kiemPhienBanSave(schemaVersion: number): KetQua<number> {
  ...
  if (schemaVersion > PHIEN_BAN_SCHEMA) {
    return hong([
      loi(
        'persistence',
        'SAVE_MOI_HON_APP',
        `Save này được tạo bởi phiên bản mới hơn (schema v${schemaVersion}); ` +
          `bản đang chạy chỉ đọc được tới v${PHIEN_BAN_SCHEMA}. Hãy cập nhật ứng dụng rồi mở lại.`,
        { recoverable: false, details: { saveVersion: schemaVersion, appVersion: PHIEN_BAN_SCHEMA } },
      ),
    ]);
  }
...
export async function trangThaiMigration(db: ThienDienDb): Promise<readonly MigrationCheckpoint[]> {
