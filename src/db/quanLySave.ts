 ...
 */
export function nhanMacDinh(tick: number, mode: ViewMode): string {
 ...
 */
export async function danhSachSave(db: ThienDienDb): Promise<readonly MucSave[]> {
...
export async function ghiVan(
  ...
  for (const e of events) await kho.themEvent(e);
 ...
 */
export async function ghiVanNhe(db: ThienDienDb, state: WorldState, ten: string): Promise<void> {
  const branchId = state.world.branchId;
  const cu = await db.branches.get(branchId);
...
export async function doiTenVan(db: ThienDienDb, branchId: string, ten: string): Promise<void> {
  const cu = await db.branches.get(branchId);
  if (!cu) return;
  await db.branches.put({ ...cu, ten: ten.trim() });
}
 ...
 */
export async function xoaVan(db: ThienDienDb, branchId: string): Promise<KetQua<number>> {
  ...
  if (con.length > 0) {
    return hong([
      loi(
        'persistence',
        'NHANH_CON_CON_SONG',
        `Ván này là gốc của ${con.length} nhánh khác. Xóa nó sẽ làm hỏng chúng — hãy xóa nhánh con trước.`,
        { details: { con: con.map((c) => c.id) }, recoverable: true },
      ),
    ]);
  }
  ...
  const xoaTheoNhanh = async (t: {
    where: (i: string) => { equals: (v: string) => { delete: () => Promise<number> } };
  }): Promise<void> => {
...
export async function vanGanNhat(db: ThienDienDb): Promise<MucSave | null> {
  const ds = await danhSachSave(db);
  return ds[0] ?? null;
}

/** Hash state hiện tại — dùng cho dòng "đã lưu" trên màn chính. */
export function hashVan(state: WorldState): string {
