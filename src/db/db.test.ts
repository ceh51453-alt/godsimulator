...
const world = (branchId = GOC, over: Partial<World> = {}): World =>
  WorldSchema.parse({
    id: 'w1',
    branchId,
    seed: 'hat-giong',
...
const entity = (id: string, branchId = GOC, ten = id) =>
  EntitySchema.parse({ id, branchId, kind: 'concept', ten, tickSinh: 0 });

const nhanh = (id: string, gocId: string | null, tickTao = 0): Branch =>
  BranchSchema.parse({ id, worldId: 'w1', gocId, tickTao, ten: id });
...
describe('[BB] save round-trip giữ hash', () => {
  it('ghi state rồi đọc lại cho cùng hash', async () => {
    ...
    s.links.set(
      'lk',
      LinkSchema.parse({ id: 'lk', branchId: GOC, tuId: 'e1', denId: 'e2', quanHe: 'nhac_den', tickTao: 0 }),
    );
  ...
  it('napState báo lỗi có cấu trúc khi nhánh không tồn tại', async () => {
    ...
    if (!r.ok) expect(r.errors[0]?.code).toBe('KHONG_CO_NHANH');
  });

  it('event của nhánh con gồm cả lịch sử của cha, sắp xếp deterministic', async () => {
    const ev = (id: string, branchId: string, tick: number): Event =>
      taoEvent({
        id,
        branchId,
        tick,
...
describe('[BB] export không chứa secret hoặc hồ sơ riêng mặc định', () => {
  ...
  it('[BB] không có proxyPassword hay bất kỳ secret nào trong gói xuất', async () => {
    ...
    if (r.ok) {
      ...
      for (const k of KHOA_SECRET) expect(chuoi).not.toContain(`"${k}"`);
    }
  });
  ...
  it('[BB] mặc định KHÔNG kèm hồ sơ riêng tư', async () => {
    ...
    await db.playerProfiles.put(
      PlayerProfileSchema.parse({ id: 'pf1', privateNotes: 'GHI-CHU-RAT-RIENG', createdAt: 0, updatedAt: 0 }),
    );
  ...
  it('chỉ kèm hồ sơ riêng tư khi người dùng chủ động bật', async () => {
    ...
    expect(r.ok).toBe(true);
    if (r.ok) expect(JSON.stringify(r.value)).toContain('GHI-CHU-RAT-RIENG');
  });
  ...
  it('stripSecret khớp không phân biệt hoa thường và bỏ khóa nguy hiểm', () => {
    const sach = stripSecret({ ProxyPassword: 'x', API_KEY: 'y', Token: 'z', ok: 1 }) as Record<
      string,
      unknown
    >;
  ...
  it('nhập file rác trả lỗi có cấu trúc, không throw', () => {
    for (const rac of [null, 42, 'chuỗi', {}, { dinhDang: 'khac' }, []]) {
      ...
      expect(r.ok).toBe(false);
      if (!r.ok) expect(r.errors[0]?.domain).toBe('persistence');
    }
  });
  ...
  it('save vi phạm bất biến bị từ chối nạp', async () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe('SAVE_VI_PHAM_BAT_BIEN');
  });
});
...
describe('[BB] migration tăng dần, có checkpoint, phục hồi được sau crash', () => {
  it('v1→v2 gán branchId cho bản ghi thiếu và đánh dấu hoàn tất', async () => {
    await db.entities.put({ ...entity('e1'), branchId: '' } as never);
    await db.entities.put({ ...entity('e2'), branchId: '' } as never);

    const r = await chayMigrationV1V2(db, GOC, 0);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.value.soBanGhiDiChuyen).toBe(2);
  ...
  it('[BB] crash giữa migration: checkpoint dở dang thì lần sau chạy tiếp và hoàn tất', async () => {
    for (let i = 0; i < 5; i++) await db.entities.put({ ...entity(`e${i}`), branchId: '' } as never);
  ...
  it('[BB] không đánh dấu hoàn tất khi kiểm tra chưa đạt', async () => {
    ...
    await db.entities.put({ ...entity('e9'), branchId: '' } as never);
    const r = await chayMigrationV1V2(db, GOC, 0);
    expect(r.ok).toBe(true);
    expect((await db.entities.toArray()).every((e) => e.branchId === GOC)).toBe(true);
  });
  ...
  it('ghi hash và đếm record trước khi tuyên bố hoàn tất — quy tắc 61.5 #3', async () => {
    ...
    const dem2 = (await db.settings.get('migration.v1_v2.count'))?.value as { soEntity: number };
    expect(dem2.soEntity).toBe(1);
  });

  it('[BB] 78.10 — save cũ mở ra có setupCompleted = true, KHÔNG bị wizard chặn', async () => {
    await db.worlds.put(
      world(GOC, { playerState: { ...world().playerState, setupCompleted: false, setupVersion: 0 } }),
    );
...
describe('autosave và snapshot', () => {
  it('snapshot phục hồi được và giữ hash', async () => {
    ...
    expect(r.ok).toBe(true);
    if (r.ok) expect(hashState(r.value)).toBe(hashState(s));
  });
  ...
  it('snapshot bị sửa thì phát hiện được qua hash', async () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors[0]?.code).toBe('SNAPSHOT_HASH_LECH');
  });
  ...
  it('[BB] Phần 38 — giữ đúng 5 bản autosave gần nhất mỗi nhánh', async () => {
    ...
    const ds = (await db.snapshots.where('branchId').equals(GOC).toArray()).filter(
      (x) => x.scopeKey === scope,
    );
...
describe('[BB] cache rerank không đọc chéo nhánh hay chéo chủ thể', () => {
  ...
  const khoa = (over: Partial<Parameters<KhoRerankCache['doc']>[0]> = {}) => ({
    branchId: GOC,
...
describe('[BB] Phase 8 — save trước v6 vẫn mở được', () => {
  ...
  it('state ghi khi chưa có ba bảng mới đọc lại vẫn đúng hash', async () => {
    ...
    s.entities.set(
      'e1',
      EntitySchema.parse({ id: 'e1', branchId: GOC8, kind: 'place', ten: 'Làng', tickSinh: 0 }),
    );
    await kho.ghiState(s);
  ...
  it('phiên bản schema đã lên v7; save cũ mở được, save mới hơn app bị từ chối tử tế', () => {
    ...
    for (const cu of [1, 2, 3, 4, 5, 6]) expect(kiemPhienBanSave(cu).ok).toBe(true);
  });
});
...
describe('[BB] Phase 9 + 10 — Dexie v8 thêm bảng mà không đụng save cũ', () => {
  ...
  it('save ghi khi chưa có năm bảng mới đọc lại vẫn ĐÚNG HASH', async () => {
    ...
    s.entities.set(
      'e1',
      EntitySchema.parse({ id: 'e1', branchId: GOC10, kind: 'place', ten: 'Làng', tickSinh: 0 }),
    );
    await kho.ghiState(s);
  ...
  it('luật nền, cơ chế và lorebook round-trip qua Dexie, giữ nguyên hash', async () => {
    ...
    for (const ln of luatNenMacDinh(br)) s.substrateLaws.set(ln.id, ln);
    s.coChe.set('than_bi', CoCheRowSchema.parse({ id: 'than_bi', branchId: br, bat: true, tickBat: 5980 }));
    s.lorebooks.set(
      'lb.aicap',
      LorebookSchema.parse({ id: 'lb.aicap', branchId: br, ten: 'Ai Cập', nguon: 'nguoi_dung' }),
    );
    await kho.ghiState(s);
  ...
  it('[BB] copy-on-write vẫn đúng: fork rồi sửa luật nền KHÔNG đè nhánh cha', async () => {
    ...
    for (const ln of luatNenMacDinh(cha)) s.substrateLaws.set(ln.id, ln);
    await kho.ghiState(s);
