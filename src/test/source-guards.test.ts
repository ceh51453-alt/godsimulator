...
function bocVo(src: string): string {
  return src
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    .replace(/(^|[^:])\/\/[^\n]*/g, '$1 ')
    .replace(/'(?:\\.|[^'\\])*'/g, "''")
...
describe('[BB] src/core là TypeScript thuần', () => {
  const CAM_IMPORT = [['react', /from\s+""|require\(\s*""/]] as const;
  void CAM_IMPORT;

  function importCua(src: string): string[] {
    const out: string[] = [];
    const re = /(?:^|\n)\s*(?:import|export)\s[^;]*?from\s+['"]([^'"]+)['"]/g;
    let m: RegExpExecArray | null;
    while ((m = re.exec(src)) !== null) if (m[1]) out.push(m[1]);
    return out;
  }

  it.each(['react', 'react-dom', 'dexie', 'dexie-react-hooks', 'zustand', 'framer-motion', 'ejs'])(
    "không file core nào import '%s'",
    (pkg) => {
      const pham: string[] = [];
      for (const p of FILE_CORE) {
        const imps = importCua(readFileSync(p, 'utf8'));
        if (imps.some((i) => i === pkg || i.startsWith(`${pkg}/`))) pham.push(ten(p));
      }
      expect(pham).toEqual([]);
    },
  );
  ...
  it('không file core nào import từ ui/, db/ hay store/', () => {
    ...
    for (const p of FILE_CORE) {
      const imps = importCua(readFileSync(p, 'utf8'));
      if (imps.some((i) => /(^|\/)(ui|db|store)\//.test(i))) pham.push(ten(p));
    }
    expect(pham).toEqual([]);
  });
...
describe('[BB] mô phỏng deterministic — luật bất biến #7', () => {
  ...
  it('không dùng thời gian máy (Date.now, new Date, performance.now)', () => {
    const pham = NGUON_MO_PHONG.filter((p) =>
      /Date\s*\.\s*now|new\s+Date\b|performance\s*\.\s*now/.test(bocVo(readFileSync(p, 'utf8'))),
    );
    expect(pham.map(ten)).toEqual([]);
  });
  ...
  it('không dùng sort phụ thuộc locale', () => {
    const pham = NGUON_MO_PHONG.filter((p) =>
      /localeCompare|Intl\s*\.\s*Collator|toLocale(?:Date|Time|)String/.test(bocVo(readFileSync(p, 'utf8'))),
    );
    expect(pham.map(ten)).toEqual([]);
  });
});
