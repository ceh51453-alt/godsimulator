...
export const CHUNKS_EVAL: readonly ChunkFixture[] = [
  // ── ĐÚNG cho câu hỏi của phàm nhân về "luật của thế giới này"
  c(
    'ck_dg_thung_lung',
    'nguon_dien_giai_thung_lung',
    'Người già trong thung lũng dạy rằng kẻ mang dấu máu làm ô uế người đứng gần.',
    ['tat_ca'],
    { entityIds: ['law_mau', 'place_thung_lung'], trust: 0.55 },
  ),
  c(
    'ck_dg_bo_song',
    'nguon_dien_giai_bo_song',
    'Bên bờ sông, người ta tin nước rằm rửa được dấu máu nếu lòng thành.',
    ['tat_ca'],
    { entityIds: ['law_mau', 'place_bo_song'], trust: 0.5 },
  ),
  // ── TRÙNG: cùng nguonId với ck_dg_thung_lung, chữ khác
  c(
    'ck_dg_thung_lung_2',
    'nguon_dien_giai_thung_lung',
    'Ở Thung Lũng Tro, ai chạm phải kẻ có dấu thì cũng thành ô uế theo.',
    ['tat_ca'],
    { entityIds: ['law_mau', 'place_thung_lung'], trust: 0.52 },
  ),
  // ── NHIỄU: đúng chủ đề, không trả lời được câu hỏi
  c(
    'ck_nhieu_le_hoi',
    'nguon_bien_nien',
    'Năm 288, lễ hội thuyền hoa bên sông Mê kéo dài chín ngày.',
    ['tat_ca'],
    { sourceType: 'bien_nien', trust: 0.9 },
  ),
  c(
    'ck_nhieu_nghe_ca',
    'nguon_bien_nien',
    'Nghề đan lưới ở thung lũng suy tàn sau khi đàn cá bạc bỏ đi.',
    ['tat_ca'],
    { sourceType: 'bien_nien', trust: 0.9 },
  ),
  ...
  c(
    'ck_cam_van_ban_luat',
    'nguon_luat_goc',
    'Văn bản luật: "Máu đã đổ thì không rửa được." Kích hoạt: gay_chay_mau.',
    ['sang_the'],
    { sourceType: 'luat_goc', trust: 1 },
  ),
  // ── CẤM: bản tính thật của thần. Phàm nhân chỉ được thấy banTinhTinDoTin.
  c(
    'ck_cam_ban_tinh_that',
    'nguon_hon_pho',
    'Bản tính thật của Đấng Tẩy Uế: từ bi, khiêm nhường; ngài chưa từng đòi hỏi trừng phạt.',
    ['sang_the'],
    { sourceType: 'hon_pho', trust: 1 },
  ),
  // ── CẤM: kẽ hở engine đã biết nhưng chưa ai trong thế giới khám phá ra.
  c(
    'ck_cam_ke_ho',
    'nguon_ke_ho',
    'Kẽ hở: bóp cổ không gây chảy máu nên không kích hoạt luật.',
    ['sang_the'],
    { sourceType: 'ke_ho', trust: 1 },
  ),
];

export const CHUNK_CAM: readonly string[] = ['ck_cam_van_ban_luat', 'ck_cam_ban_tinh_that', 'ck_cam_ke_ho'];

export const EVAL_CASES: readonly RetrievalEvalCase[] = [
...
export function duocNhin(chunk: ChunkFixture, mode: string, chuTheId: string | null): boolean {
  if (chunk.aiDuocNhin.includes('tat_ca')) return true;
  if (chunk.aiDuocNhin.includes(mode)) return true;
