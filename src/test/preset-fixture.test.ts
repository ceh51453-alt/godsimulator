...
type Preset = {
  prompts?: { identifier: string; enabled?: boolean; marker?: boolean }[];
  prompt_order?: { order?: { identifier: string; enabled?: boolean }[] }[];
  extensions?: {
    regex_scripts?: { disabled?: boolean }[];
    tavern_helper?: { scripts?: { enabled?: boolean }[] };
  };
};

const doc = (nhan: string): { meta: Meta; preset: Preset } => ({
  meta: JSON.parse(readFileSync(join(THU_MUC, `fixture-${nhan}.meta.json`), 'utf8')) as Meta,
  preset: JSON.parse(readFileSync(join(THU_MUC, `fixture-${nhan}.anon.json`), 'utf8')) as Preset,
});
...
describe.each(['A', 'B'] as const)('fixture preset %s', (nhan) => {
  ...
  it('mọi identifier trong order đều tồn tại trong prompts', () => {
    ...
    const order = preset.prompt_order?.[0]?.order ?? [];
    for (const o of order) expect(ids.has(o.identifier), `order id lạ: ${o.identifier}`).toBe(true);
  });
