...
export type DinhMuc = { readonly inputMin: number; readonly inputMax: number; readonly output: number };

/** Bảng 34.1, nguyên văn. Con số là DỮ LIỆU, không phải hằng rải trong code (7.1). */
export const NGAN_SACH_MAC_DINH: Readonly<Record<LoaiCall, DinhMuc>> = Object.freeze({
...
export function calibMoi(loai: LoaiCall, tyLeToken = 3.2): Calib {
 ...
 */
export function uocLuong(s: string, tyLeToken: number): number {
  return Math.ceil(s.length / Math.max(0.5, tyLeToken));
}

/** Ngân sách INPUT còn dùng được cho một loại call, sau tự hiệu chỉnh. */
export function nganSachInput(loai: LoaiCall, calib: Calib | null): number {
...
export function tuHieuChinh(
  ...
  if (thucTe > 0) {
    ...
    const mau: [number, number][] = [...c.mau, [uoc, thucTe] as [number, number]].slice(-20);
    ...
    if (lech >= 5) {
      ...
      canhBao.push(
        `Ước lượng token lệch quá 12% năm lượt liền cho "${c.loai}"; ` +
  ...
  if (finishReason === 'length') {
    ...
    canhBao.push(
      `Model cắt cụt output ở "${c.loai}" (prompt ${soKyTu} ký tự). ` +
 ...
 */
export function phanBoSauTang(nganSach: number): readonly PhanBoTang[] {
  const p = (x: number): number => Math.max(200, Math.round(nganSach * x));
 ...
 */
export function catTheoTran(
  tang: readonly { so: number; ten: string; noiDung: string }[],
  phanBo: readonly PhanBoTang[],
  tyLeToken: number,
): { tang: readonly { so: number; ten: string; noiDung: string }[]; vetCat: readonly VetCat[] } {
  const vetCat: VetCat[] = [];
  const ra = tang.map((t) => {
    const pb = phanBo.find((p) => p.so === t.so);
    if (!pb) return t;
    const uoc = uocLuong(t.noiDung, tyLeToken);
    if (uoc <= pb.tranToken) return t;
