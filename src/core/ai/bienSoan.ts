 ...
 */
import type { WorldView, ProjectedEntity, ProjectedStoryline } from '../contracts/view.js';
 ...
 */
export const LUAT_KHONG_GIU_SO: readonly string[] = Object.freeze([
...
const MUC_RO_LOI: Readonly<Record<string, string>> = Object.freeze({
  ro: 'thấy rõ',
  mo: 'thấy mờ',
  tin_don: 'chỉ nghe đồn',
});

function motDong(e: ProjectedEntity): string {
...
function tang1(mode: ViewMode): string {
...
function tang2(view: WorldView): string {
  const dong: string[] = [];
  if (laHuVo(view)) return KHOI_HU_VO;
...
function tang3(view: WorldView, tenNguoiChoi: string): string {
  ...
  if (chuThe) {
    dong.push(
      `Nhân vật người chơi: ${chuThe.ten}${chuThe.moTa.trim() === '' ? '' : ` — ${chuThe.moTa.trim()}`}.`,
    );
  } else {
...
function tang4(
  ...
  if (mach) {
    ...
    if (!oChoNguoiChoi) {
      dong.push(
        'Ống kính lượt này KHÔNG ở chỗ người chơi. Không nhắc tới họ, kể cả gián tiếp. ' +
  ...
  if (ro.length > 0) {
    ...
    for (const e of ro) dong.push(motDong(e));
    dong.push('');
  }
  if (xa.length > 0) {
    dong.push('NGHE NÓI CÓ (đừng kể như thể chắc chắn):');
    for (const e of xa) dong.push(motDong(e));
  }
  return dong.join('\n').trimEnd();
}
 ...
 */
function tangTruyHoi(ds: readonly { nguon: string; text: string; daBopMeo: boolean }[]): string {
  if (ds.length === 0) return '';
  const dong: string[] = ['ĐIỀU CHỦ THỂ NHỚ HOẶC BIẾT (đã truy hồi theo tiêu điểm):'];
  for (const c of ds) {
    const nguon =
      c.nguon === 'lorebook'
        ? '[THẦN THOẠI NGUỒN — điểm hút, chưa phải lịch sử đã xảy ra] '
        : c.nguon === 'lorebook_su'
          ? '[SỬ THẾ GIỚI — điều đã xảy ra, thắng mọi Nguồn trái với nó] '
          : c.nguon === 'lorebook_di_san'
            ? '[DI SẢN — ký ức từ vòng trước, thấp hơn Sử và cao hơn Nguồn] '
            : '';
  ...
  if (ds.some((c) => c.nguon === 'lorebook_su') && ds.some((c) => c.nguon === 'lorebook')) {
    dong.push(
      'Khi SỬ THẾ GIỚI và THẦN THOẠI NGUỒN nói khác nhau, giữ Sử và kể Nguồn như một truyền thống, lời tiên tri hoặc dị bản đã không thành. Không trộn hai mệnh đề thành một sự thật thứ ba.',
    );
  }
  return dong.join('\n');
}
...
function tang5(
  ...
  if (banTin && banTin.muc.length > 0) {
    const dong: string[] = ['CHUYỆN VỪA XẢY RA (engine đã quyết — kể lại, đừng phán lại):'];
 ...
 */
function tang6(ng: NguLieuKe): string {
  const dong: string[] = [];
  ...
  if (treo.length > 0) {
    ...
    for (const p of treo) dong.push(`- ${p.noiDung}${p.soNguoi > 1 ? ` (${p.soNguoi} người cùng cầu)` : ''}`);
    dong.push('');
  }
  ...
  if (ng.ketQuaEngine.length > 0) {
    ...
    for (const k of ng.ketQuaEngine) dong.push(`- ${k}`);
  }
  dong.push('');
  ...
  dong.push(
    'Nếu bạn vừa gieo một điều sẽ phải trả về sau (một lời hứa, một điềm báo, một vật lạ, ' +
 ...
 */
export function bienSoanPromptKe(ng: NguLieuKe): PromptGoi {
  const oChoNguoiChoi = ng.ongKinhOChoNguoiChoi ?? true;
  const tho: TangPrompt[] = [
   ...
   */
  const { tang, vetCat } =
    ng.nganSachToken === undefined
      ? { tang: tho, vetCat: [] as readonly VetCat[] }
      : catTheoTran(tho, phanBoSauTang(ng.nganSachToken), ng.tyLeToken);

  const daySo = tang.map((t, i) => ({ ...(tho[i] as TangPrompt), noiDung: t.noiDung }));

  const noi = (loc: (t: TangPrompt) => boolean): string =>
    daySo
      .filter(loc)
      .map((t) => t.noiDung)
      .filter((s) => s.trim() !== '')
...
export function thuDuongDatKhong(traVe: string): boolean {
