...
import type { NguLieuKe } from './bienSoan.js';
...
function theGioi(seed = 'ai8'): { state: WorldState; log: EventLog } {
...
function nguLieu(view: WorldView, them: Partial<NguLieuKe> = {}): NguLieuKe {
  return {
    view,
    banTin: null,
    loiCau: [],
...
describe('[BB] 33.1 — sáu tầng, mạch truyện ở tầng 4', () => {
  ...
  it('[BB] 29.2 quy tắc 5 — ống kính không ở chỗ người chơi thì prompt CẤM nhắc họ', () => {
    ...
    const p = bienSoanPromptKe(
      nguLieu(chieu(state, 'pham_nhan', 'mortal_1'), {
        machDangChieu: MACH,
...
describe('[BB] 30.2 + 54.10 — Foreshadow và Unverified', () => {
  ...
  it('bóc được `<Foreshadow>`, và văn xuôi KHÔNG còn dấu vết khối', () => {
    const raw =
      'Ông lão đặt con dao xuống bàn rồi bỏ đi.\n' +
...
describe('[BB] 46.1 — Cập Nhật Biến có điểm cuối riêng', () => {
  ...
  it('prompt Updater KHÔNG mang bảy quy tắc Narrator — nó không kể chuyện', () => {
    ...
    for (const q of BAY_QUY_TAC_NARRATOR) expect(p.heThong).not.toContain(q);
  ...
  it('Updater vẫn bị `bocTach` duyệt — có điểm cuối riêng không có thẩm quyền riêng', () => {
    const kq = bocTach(
      '<CapNhat>{"patches":[{"op":"set","target":{"table":"entities","id":"mortal_1","path":"aspects.ban_nga.coreSelf.tuBi_tanNhan"},"value":99}]}</CapNhat>',
      { eventId: 'e', idHopLe: new Set(['mortal_1']) },
    );
    expect(kq.patches).toHaveLength(0);
  ...
  it('Updater riêng không làm mất entity Narrator vừa tạo khi trả khối rỗng', () => {
    const goc = bocTach(
      '<CapNhat>{"patches":[{"op":"link","target":{"table":"entities","id":"deity_moi","path":""},"value":{"id":"deity_moi","kind":"deity","ten":"Thần Mới","tickSinh":0}}]}</CapNhat>',
      { eventId: 'e', idHopLe: new Set(), branchId: 'br_goc' },
    );
    ...
    expect(kq.patches).toHaveLength(1);
    expect((kq.patches[0]?.value as { kind: string }).kind).toBe('deity');
  });

  it('link lệch id được ép theo target để không rollback cả lượt', () => {
    const kq = bocTach(
      '<CapNhat>{"patches":[{"op":"link","target":{"table":"entities","id":"deity_dung","path":""},"value":{"id":"deity_sai","kind":"deity","ten":"Thần","tickSinh":0}}]}</CapNhat>',
      { eventId: 'e', idHopLe: new Set(), branchId: 'br_goc' },
    );
    expect(kq.patches).toHaveLength(1);
    expect((kq.patches[0]?.value as { id: string }).id).toBe('deity_dung');
  });
});
