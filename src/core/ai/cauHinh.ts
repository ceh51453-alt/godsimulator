...
export const CAU_HINH_AI_RONG: AiConfig = AiConfigSchema.parse({});
...
const laUrlHopLe = (s: string): boolean => {
  const t = s.trim();
  if (t === '') return false;
 ...
 */
export function thieuGiOEndpoint(ep: AiEndpoint): ThieuSot[] {
  const ra: ThieuSot[] = [];
 ...
 */
export function thieuGiDeChoi(cfg: AiConfig): ThieuSot[] {
  return thieuGiOEndpoint(cfg.narrator);
}

export function congCoMo(cfg: AiConfig): boolean {
 ...
 */
export function cheMatKhau(cfg: AiConfig): AiConfig {
  const che = <T extends AiEndpoint>(ep: T): T => ({
    ...ep,
...
export function saoCauHinh(tu: AiEndpoint, den: AiEndpoint): AiEndpoint {
  return {
    ...den,
