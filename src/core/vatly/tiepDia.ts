...
function docLawful(e: Entity | undefined): Lawful | undefined {
  return e?.aspects['lawful'] as Lawful | undefined;
}
 ...
 */
export function tinhHieuLuc(luat: Entity, s: WorldState, tuning: Tuning): KetQuaHieuLuc {
  const law = docLawful(luat);
  const heSo = tuning.vatLy.heSoVaiTro;

  const manhNoi: ManhNoiTiepDia[] = (law?.tiepDia ?? []).map((t) => {
  ...
  if (nen.length === 0) {
    return {
      ...
      manhNoi,
      loiGiaiThich:
        'Luật này chưa khai khái niệm nền nào. Nó sẽ được ghi vào sổ, nhưng thế giới chưa cảm thấy nó.',
    };
  }

  // [BB] MIN, không phải trung bình.
  let yeuNhat = nen[0] as ManhNoiTiepDia;
  for (const m of nen) if (m.diem < yeuNhat.diem) yeuNhat = m;
 ...
 */
export function kiemTiepDia(luat: Entity, s: WorldState): KetQuaKiemTiepDia {
  const law = docLawful(luat);
  ...
  if (thieu.length === 0) return { dat: true, thieu: [], canTao: [], canhBao, loi: [] };

  if (cheDo === 'chat_che') {
    loi.push(
      `Chế độ chặt chẽ: luật bị từ chối vì thiếu ${thieu.length} khái niệm nền — ${thieu.join(', ')}. ` +
  ...
  canhBao.push(
    `Thiếu ${thieu.length} khái niệm nền. Engine sẽ tạo chúng ở giai đoạn "hư danh", trọng số 0 — ` +
 ...
 */
export function khaiNiemHuDanh(khaiNiemId: string, branchId: string, tick: number, nguong: number): Entity {
  return {
    id: khaiNiemId,
    branchId,
    kind: 'concept',
 ...
 */
export function suyKhaiNiemSanCo(canId: string, s: WorldState): string | null {
  const chuan = (x: string): string =>
    x
      .toLowerCase()
      .normalize('NFD')
      .replace(/[̀-ͯ]/g, '')
  ...
  for (const e of ungVien) {
    if (chuan(e.id) === muc || chuan(e.ten) === muc) return e.id;
    if (e.aliases.some((a) => chuan(a) === muc)) return e.id;
  }
  return null;
}

// ─────────────────────────────────────────── 42.5 đánh vào khái niệm
 ...
 */
export function anhHuongKhiKhaiNiemYeuDi(
  s: WorldState,
  khaiNiemId: string,
  trongSoMoi: number,
  tuning: Tuning,
): AnhHuongKhaiNiem[] {
  const ra: AnhHuongKhaiNiem[] = [];
  const goc = s.entities.get(khaiNiemId);
  if (goc === undefined) return ra;
  const c = docConcept(goc);
  if (c === undefined) return ra;

  const gia: WorldState = {
    ...s,
  ...
  for (const e of [...s.entities.values()].sort((a, b) => (a.id < b.id ? -1 : 1))) {
    ...
    if (!law.tiepDia.some((t) => t.khaiNiemId === khaiNiemId)) continue;
    const truoc = tinhHieuLuc(e, s, tuning).hieuLuc;
    const sau = tinhHieuLuc(e, gia, tuning).hieuLuc;
    if (truoc !== sau) ra.push({ luatId: e.id, ten: e.ten, hieuLucTruoc: truoc, hieuLucSau: sau });
  }
  return ra;
}
 ...
 */
export function coTheHoiSinh(luat: Entity, s: WorldState, tuning: Tuning): boolean {
  const kq = tinhHieuLuc(luat, s, tuning);
  if (kq.hieuLuc > 0) return false;
...
export function bangTiepDia(kq: KetQuaHieuLuc): string {
  const thanh = (m: ManhNoiTiepDia): string => {
