 ...
 */
export function docDuongDan(goc: unknown, duongDan: string): unknown {
  if (duongDan === '') return goc;
  let hienTai: unknown = goc;
  for (const phan of duongDan.split('.')) {
    if (phan === '__proto__' || phan === 'constructor' || phan === 'prototype') return undefined;
    if (hienTai === null || hienTai === undefined) return undefined;
    if (typeof hienTai !== 'object') return undefined;
    if (Array.isArray(hienTai)) {
      const i = Number(phan);
      if (!Number.isInteger(i) || i < 0 || i >= hienTai.length) return undefined;
      hienTai = hienTai[i];
      continue;
    }
    ...
    if (!Object.prototype.hasOwnProperty.call(hienTai, phan)) return undefined;
    hienTai = (hienTai as Record<string, unknown>)[phan];
  }
  return hienTai;
}
...
function soSanh(a: unknown, b: unknown): number | null {
...
function laThat(v: unknown): boolean {
  return v === true;
}
 ...
 */
export function tinhExpr(node: ExprNode, nguon: NguonDoc, doSau = 0): KetQua<unknown> {
  ...
  const tinhCon = (i: number): KetQua<unknown> => {
    const con = args[i];
    if (!con) {
  ...
  const tinhHet = (): KetQua<unknown[]> => {
    const ra: unknown[] = [];
    const canhBao: StructuredError[] = [];
    for (let i = 0; i < args.length; i++) {
      const r = tinhCon(i);
      if (!r.ok) return hong(r.errors, [...canhBao, ...r.warnings]);
  ...
  switch (node.op) {
    ...
    case 'and': {
      ...
      for (let i = 0; i < args.length; i++) {
        const r = tinhCon(i);
        if (!r.ok) return r;
        if (!laThat(r.value)) return dat(false);
      }
      return dat(true);
    }
    ...
    case 'or': {
      for (let i = 0; i < args.length; i++) {
        const r = tinhCon(i);
        if (!r.ok) return r;
        if (laThat(r.value)) return dat(true);
      }
      return dat(false);
    }
    ...
    case 'in': {
      ...
      if (Array.isArray(dong))
        return dat(
          dong.some((x) => bang(x, kim)),
          r.warnings,
        );
      if (dong instanceof Set) {
        for (const x of dong) if (bang(x, kim)) return dat(true, r.warnings);
        return dat(false, r.warnings);
      }
      if (typeof dong === 'string' && typeof kim === 'string') return dat(dong.includes(kim), r.warnings);
      return dat(false, [
        ...r.warnings,
    ...
    default: {
      ...
      const opLa = (node as { op: string }).op;
      return hong([
...
export function dieuKienDung(node: ExprNode, nguon: NguonDoc): boolean {
