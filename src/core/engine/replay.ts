...
export function replay(
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    tuChoi++;
    canhBao.push(
      loi('persistence', 'REPLAY_BO_QUA_EVENT', `Bỏ qua Event '${e.id}': ${r.errors[0]?.message ?? 'lỗi'}`, {
        severity: 'warning',
      }),
    );
  }
 ...
 */
export function kiemDeterminism(worldBanDau: World, events: readonly Event[]): KetQua<string> {
  const a = replay(worldBanDau, events);
  if (!a.ok) return hong(a.errors, a.warnings);
  const b = replay(worldBanDau, events);
  if (!b.ok) return hong(b.errors, b.warnings);
