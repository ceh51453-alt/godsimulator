...
import type { DanCu, KinhTe } from '../schema/aspect/substrate.js';
...
describe('bằng chứng Phase 5', () => {
  it('in số liệu cho tài liệu sống', () => {
    ...
    const evNen = eventGieoNen(state) as Event;
    apDungEvent(state, evNen, log);

    const danSoDau = [...state.entities.keys()].reduce(
      (t, id) => t + tongCohort(docAspect<DanCu>(state.entities.get(id), 'dan_cu')?.cohort),
      0,
    );
    ...
    const danSoCuoi = [...state.entities.keys()].reduce(
      (t, id) => t + tongCohort(docAspect<DanCu>(state.entities.get(id), 'dan_cu')?.cohort),
      0,
    );
    const kho = [...state.entities.keys()].reduce(
      (t, id) => t + (docAspect<KinhTe>(state.entities.get(id), 'kinh_te')?.kho.luongThuc ?? 0),
      0,
    );
