/**
 * `population_household` và `health_disease` — Phần 71.2.
 *
 * Hai tiến trình này viết vào cùng một thứ mà cả thế giới nhìn: số người còn sống.
 * Vì vậy chúng tuân ba quy tắc cứng:
 *
 *   1. Mọi thay đổi dân số đi qua `add` trên `cohort` VÀ `add` khớp hệt trên
 *      `spatial.danSo`. Nhờ vậy hai con số không bao giờ trôi khỏi nhau, và
 *      `conflict_security` cùng ghi trong một stage vẫn gộp được (71.4 quy tắc 1).
 *   2. Phần lẻ được giữ ở `dan_cu.du`. Không có nó, làm tròn mỗi tick sẽ âm thầm
 *      nuốt hoặc đẻ ra người.
 *   3. Không tiến trình nào được lấy quá nửa một nhóm tuổi trong một lần chạy.
 *      Trần này để hai tiến trình cùng trừ trong một stage không thể đẩy dân số
 *      xuống âm.
 */
...
const TICK_QUA_BAND = { child: 60, youth: 40, adult: 140 } as const;

/** Tử suất nền mỗi tick theo nhóm tuổi, thế giới tiền công nghiệp. */
const TU_SUAT_NEN = { child: 0.006, youth: 0.002, adult: 0.0025, elder: 0.02 } as const;
...
function tachNguyen(tichLuy: number): { n: number; du: number } {
...
export function chayDanSo(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
  ...
  const delta = new Map<string, Cohort>();
  const soCai = new Map<string, SoCaiDan>();
  const duMoi = new Map<string, DanCu['du']>();
  const danHienTai = new Map<string, Cohort>();

  const themDelta = (id: string, band: keyof Cohort, n: number): void => {
  ...
  for (const { id, e } of noiChon) {
    ...
    // ── già đi ──
    const chuyen: [keyof Cohort, keyof Cohort, keyof DanCu['du'], number][] = [
    ...
    const kyVong: Record<keyof Cohort, number> = { child: 0, youth: 0, adult: 0, elder: 0 };
    const nguyenNhan = { tuNhien: 0, doi: 0, benh: 0 };

    for (const band of ['child', 'youth', 'adult', 'elder'] as const) {
      const dan = c[band];
      if (dan <= 0) continue;
    ...
    if (soChet > 0 && tongKyVong > 0) {
      ...
      const chia: Record<keyof Cohort, number> = { child: 0, youth: 0, adult: 0, elder: 0 };
      let daChia = 0;
      let bandLon: keyof Cohort = 'adult';
      for (const band of ['child', 'youth', 'adult', 'elder'] as const) {
        chia[band] = Math.floor((soChet * kyVong[band]) / tongKyVong);
        daChia += chia[band];
        if (kyVong[band] > kyVong[bandLon]) bandLon = band;
      }
      chia[bandLon] += soChet - daChia;

      for (const band of ['child', 'youth', 'adult', 'elder'] as const) {
        const thatSu = Math.min(chia[band], Math.floor(c[band] * TRAN_LAY_MOT_LAN));
        if (thatSu > 0) themDelta(id, band, -thatSu);
        chia[band] = thatSu;
      }
  ...
  for (const { id, e } of noiChon) {
    ...
    if (scTu) scTu.xuatCu += thatSu;
    if (scDen) scDen.nhapCu += thatSu;
  ...
  for (const { id, e } of noiChon) {
    ...
    patches.push(
      dat(nc, id, 'aspects.dan_cu.soCai', {
        ...sc,
        tuDoXungDot: 0,
        vatChatHoa: 0,
      }),
    ...
    if (soHo !== dc.soHo) patches.push(dat(nc, id, 'aspects.dan_cu.soHo', soHo));
...
export function chaySucKhoe(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
  const patches: PatchOp[] = [];
  const suKien: UngVienSuKien[] = [];
   ...
   */
  const layMoi = new Map<string, { dichId: string; tuId: string; duongId: string }>();
  const tuBung = new Set<string>();
  ...
  for (const { id, e } of moiNoiChon(nc.state)) {
    ...
    if (dan <= 0) {
      if (yt.dichId !== null) patches.push(dat(nc, id, 'aspects.y_te.dichId', null));
      if (yt.tyLeMac > 0) patches.push(dat(nc, id, 'aspects.y_te.tyLeMac', 0));
      continue;
    }
    ...
    if (yt.dichId === null) {
      ...
      if (rng.co(gopTyLe(p, n))) {
        ...
        if (lam(moi) !== lam(yt.mienDich)) patches.push(dat(nc, id, 'aspects.y_te.mienDich', lam(moi)));
      }
      continue;
    }
    ...
    if (macMoi > 0.08) {
      for (const lg of langGieng(nc.state, id)) {
        const ytb = docAspect<YTe>(nc.state.entities.get(lg.noiId), 'y_te');
        if (!ytb || ytb.dichId !== null) continue;
        if (layMoi.has(lg.noiId)) continue;
        ...
        if (!rng.co(gopTyLe(pLay, n))) continue;
  ...
  for (const [noiId, lay] of [...layMoi.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1))) {
    ...
    suKien.push({
      loai: 'dich_lay_lan',
      mucDo: 'lon',
      moTa: `Bệnh theo đường từ ${nc.state.entities.get(lay.tuId)?.ten ?? lay.tuId} sang ${
        nc.state.entities.get(noiId)?.ten ?? noiId
      }.`,
      tienTrinhId: 'health_disease',
