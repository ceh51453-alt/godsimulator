...
export function chayMachTruyen(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
  const kq: KetQuaTienTrinh = { patches: [], suKien: [] };
  const s = nc.state;
   ...
   */
  const quet =
    nc.tick % CHU_KY_QUET_TIEN_DE === 0 ? quetMachTruyen(s, nct) : { patches: [], machMoi: [], soBiTran: 0 };
