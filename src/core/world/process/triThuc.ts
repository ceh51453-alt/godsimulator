/**
 * `travel_communication` và `knowledge_technology` — Phần 71.2.
 *
 * Đây là hai tiến trình quan trọng nhất với vòng chat, vì chúng quyết định
 * **NPC biết gì khi người chơi hỏi**. Một tin xảy ra ở làng bên chỉ tới miệng
 * người đối diện sau khi có ai đó thật sự đi hết con đường đó.
 *
 * [BB] 71.4 — "tri thức không xuất hiện ở chủ thể nếu thiếu đường truyền".
 * Mọi dòng `knowledge` có `hops > 0` phải trỏ về một dòng nguồn có thật, học
 * trước đó đủ số tick của tuyến. Bất biến `khong_tri_thuc_teleport` kiểm lại.
 */
...
import type { DanCu, KinhTe, VanHoa } from '../../schema/aspect/substrate.js';
...
type ChiSoTriThuc = {
  /** factId → dòng của từng vùng. */
  theoFact: Map<string, KnowledgeRow[]>;
  /** `knowerId|factId` → dòng. */
  theoCap: Map<string, KnowledgeRow>;
  /** knowerId → số dòng đang giữ. */
  demTheoNguoi: Map<string, number>;
};

function lapChiSo(nc: NgocCanhTienTrinh): ChiSoTriThuc {
  const theoFact = new Map<string, KnowledgeRow[]>();
  const theoCap = new Map<string, KnowledgeRow>();
  const demTheoNguoi = new Map<string, number>();
...
export function chayLienLac(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
  ...
  for (const { id } of moiNoiChon(nc.state)) {
    const vhA = docAspect<VanHoa>(nc.state.entities.get(id), 'van_hoa');

    for (const lg of langGieng(nc.state, id)) {
      if (!nc.state.entities.has(lg.noiId)) continue;
      const vhB = docAspect<VanHoa>(nc.state.entities.get(lg.noiId), 'van_hoa');
      ...
      if (coNguoiDi) luuLuong.set(lg.duongId, (luuLuong.get(lg.duongId) ?? 0) + 1);
    }
  }
...
export function chayKyThuat(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
  const patches: PatchOp[] = [];
  const suKien: UngVienSuKien[] = [];
  const chiSo = lapChiSo(nc);
  ...
  for (const { id, e } of moiNoiChon(nc.state)) {
    ...
    if (lam(moi) !== lam(kt.kyThuat)) patches.push(dat(nc, id, 'aspects.kinh_te.kyThuat', lam(moi)));
    ...
    if (dan < 40 && soBiet > 0) {
      ...
      if (rngQuen.co(gopTyLe(0.06, n))) {
        ...
        const cua = [...nc.state.knowledge.values()]
          ...
          .sort((a, b) =>
            a.confidence !== b.confidence ? a.confidence - b.confidence : a.id < b.id ? -1 : 1,
          );
        const bo = cua[0];
        if (bo) {
          patches.push({
