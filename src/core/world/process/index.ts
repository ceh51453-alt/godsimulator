...
import type { HandlerTienTrinh, TienTrinhNen } from './types.js';
...
const HANDLER: Readonly<Record<string, HandlerTienTrinh>> = Object.freeze({
...
let cache: readonly TienTrinhNen[] | null = null;

/**
 * Mười hai tiến trình đã nối handler, sắp xếp theo id để mọi lần chạy đều
 * duyệt cùng thứ tự (luật bất biến #7).
 */
export function moiTienTrinh(): readonly TienTrinhNen[] {
  if (cache) return cache;
  napDungSan();
  const ra: TienTrinhNen[] = [];
  for (const def of R.worldProcess.tatCa() as readonly WorldProcessDef[]) {
...
export function datLaiTienTrinh(): void {
  cache = null;
}

/** Id tiến trình chưa có handler — cổng Phase 5 đòi danh sách này rỗng. */
export function tienTrinhThieuHandler(): readonly string[] {
  napDungSan();
  return (R.worldProcess.tatCa() as readonly WorldProcessDef[])
