/**
 * Hai tiến trình tầng Phàm Nhân — Phần 70.2, 70.3, 71.2.
 *
 * Cổng Phase 7 có một dòng mà cách sai rất hấp dẫn:
 *
 * > "NPC ngoài cảnh giữ lịch và vị trí."
 *
 * Cách sai: mô phỏng từng người từng nhịp. Với vài nghìn dân thì nó chết máy, và
 * với một trăm năm tua nhanh thì nó không bao giờ về.
 *
 * Cách ở đây: **lịch là hàm thuần của hoàn cảnh** (`pham/lich.ts`), nên không ai
 * phải mô phỏng nó. Tiến trình dưới đây chỉ lo những thứ THẬT SỰ tích lũy:
 * thân thể, tay nghề, kho của hộ, và vòng đời hộ. Người ngoài cảnh vẫn "đang ở
 * ngoài ruộng" mà không tốn một phép tính nào.
 */
...
function moiNguoi(nc: NgocCanhTienTrinh): { id: string; e: Entity; m: Mortal }[] {
  const ra: { id: string; e: Entity; m: Mortal }[] = [];
 ...
 */
function tran(nc: NgocCanhTienTrinh): number {
 ...
 */
export function chayDoiNguoi(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
    ...
    if (sk?.ngheId) {
      ...
      if (r.sanLuong > 0) {
        ...
        if (dich && nc.state.entities.has(dich)) {
          const duong =
            nc.state.entities.get(dich)?.kind === 'household' ? 'aspects.ho.kho' : 'aspects.kinh_te.kho';
          patches.push({
            op: 'add',
 ...
 */
export function chayVongDoiHo(nc: NgocCanhTienTrinh): KetQuaTienTrinh {
  const patches: PatchOp[] = [];
  const suKien: UngVienSuKien[] = [];
  const n = nc.soBuocGop;
  let dem = 0;
  ...
  for (const { id, e, ho } of moiHo(nc.state)) {
    ...
    if (ho.thanhVien.length >= 4) {
      const ungVien = ho.thanhVien
        ...
        .filter((x): x is Entity => x !== undefined && x.tickDiet === null)
        .filter((x) => {
          const mm = phamThan(x);
          const kk = docAspect<SinhKe>(x, 'sinh_ke');
 ...
 */
function duAnCuaNguoi(nc: NgocCanhTienTrinh, id: string, e: Entity, suKien: UngVienSuKien[]): PatchOp[] {
  const patches: PatchOp[] = [];
  ...
  if (dangChay.length === 0) {
    const ung = ungVienDuAnNguoi(nc.state, id).filter(
...
export function tranXuLy(nc: NgocCanhTienTrinh): number {
  return tran(nc);
}

/** Hộ của một người, đọc nhanh — dùng ở `soTay` và UI. */
export function khoCuaHo(nc: NgocCanhTienTrinh, hoId: string): number {
