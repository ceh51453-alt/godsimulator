...
import type { Ho } from '../schema/aspect/pham.js';
...
export function napBatBienTangPham(): void {
  if (daNap) return;
  daNap = true;
...
function dangKyTatCa(): void {
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        const m = doc<Mortal>(s.entities.get(id), 'mortal');
        if (!m) continue;
        ...
        if (Math.abs(m.thanThe.dau - dauNen) > 12) {
          xau.push(
            `'${id}': đau = ${m.thanThe.dau} nhưng thương tích đang mang chỉ giải thích được ${dauNen}`,
          );
        }
  ...
  dangKyInvariant({
    ...
    kiem: (s) => {
      ...
      for (const e of s.entities.values()) {
        if (e.tickDiet !== null) continue;
        const m = doc<Mortal>(e, 'mortal');
      ...
      for (const [target, t] of [...tong.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1))) {
        ...
        if (t > 1.001) xau.push(`'${target}': tổng phần sở hữu = ${Math.round(t * 1000) / 1000}`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      const xau: string[] = [];
      for (const id of idCanKiem(s, phamVi)) {
        const h = doc<Ho>(s.entities.get(id), 'ho');
        if (!h) continue;
        if (h.kho.luongThuc < -0.001) xau.push(`'${id}': lương thực = ${h.kho.luongThuc}`);
        if (h.kho.vatLieu < -0.001) xau.push(`'${id}': vật liệu = ${h.kho.vatLieu}`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        ...
        const h = doc<Ho>(e, 'ho');
        if (!h || h.tickTan !== null) continue;
        for (const t of h.thanhVien) {
          const x = s.entities.get(t.id);
          if (!x) xau.push(`'${id}': thành viên '${t.id}' không tồn tại`);
          else if (x.tickDiet !== null) xau.push(`'${id}': thành viên '${t.id}' đã chết mà vẫn trong sổ`);
        }
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        ...
        const m = doc<Mortal>(e, 'mortal');
        const soul = doc<{ tang?: string; kyUc?: unknown[] }>(e, 'soul');
        if (!m || soul?.tang !== 't1') continue;
