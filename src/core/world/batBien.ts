...
import type { AnNinh, DanCu, Duong, KinhTe, SinhThai } from '../schema/aspect/substrate.js';
 ...
 */
function noiCua(s: WorldState, id: string): string | null {
  const e = s.entities.get(id);
  if (!e) return null;
  if (e.kind === 'place' || e.kind === 'realm') return id;
 ...
 */
export function napBatBienTheGioiSong(): void {
  if (daNap) return;
  daNap = true;
...
function dangKyTatCa(): void {
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        ...
        for (const band of ['child', 'youth', 'adult', 'elder'] as const) {
          ...
          if (v < 0) xau.push(`'${id}' có ${band} = ${v}`);
        }
        if (dc.soHo < 0) xau.push(`'${id}' có soHo = ${dc.soHo}`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        ...
        if (!dc || !sp || typeof sp.danSo !== 'number') continue;
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      const xau: string[] = [];
      for (const id of idCanKiem(s, phamVi)) {
        const kt = docAspect<KinhTe>(s.entities.get(id), 'kinh_te');
        if (!kt) continue;
        if (kt.kho.luongThuc < -1e-6) xau.push(`'${id}' có luongThuc = ${lam4(kt.kho.luongThuc)}`);
        if (kt.kho.vatLieu < -1e-6) xau.push(`'${id}' có vatLieu = ${lam4(kt.kho.vatLieu)}`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      const xau: string[] = [];
      for (const id of idCanKiem(s, phamVi)) {
        const st = docAspect<SinhThai>(s.entities.get(id), 'sinh_thai');
        if (!st) continue;
        for (const loai of ['rung', 'thu', 'ca', 'dat'] as const) {
          const v = st.taiNguyen[loai];
          const K = st.sucChua[loai];
          if (v < -1e-6) xau.push(`'${id}' có ${loai} = ${lam4(v)}`);
          // Vượt sức chứa nghĩa là có ai đó đang cộng vào từ hư không.
          else if (K > 0 && v > K + 1e-3) xau.push(`'${id}' có ${loai} = ${lam4(v)} > sức chứa ${lam4(K)}`);
        }
      }
      return xau;
    },
  ...
  dangKyInvariant({
    ...
    kiem: (s) => {
      ...
      const theoCap = new Map<string, { learnedAtTick: number }>();
      for (const r of s.knowledge.values()) theoCap.set(`${r.knowerId}|${r.factId}`, r);
      ...
      for (const e of s.entities.values()) {
        ...
        for (const k of [`${d.tuId}|${d.denId}`, `${d.denId}|${d.tuId}`]) {
          ...
          if (cu === undefined || d.doDai < cu) tuyen.set(k, d.doDai);
        }
      }
      ...
      for (const id of ids) {
         ...
         */
        if (noiCua(s, nguonId) !== null && noiCua(s, nguonId) === noiCua(s, r.knowerId)) continue;
        ...
        if (r.learnedAtTick - nguon.learnedAtTick < doDai) {
          xau.push(
            `'${id}': tin đi từ '${nguonId}' cần ${doDai} tick, nhưng tới sau ` +
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        const e = s.entities.get(id);
        if (!e || e.kind !== 'route') continue;
        const d = docAspect<Duong>(e, 'duong');
        if (!d) {
          xau.push(`'${id}' là tuyến đường nhưng thiếu aspect 'duong'`);
          continue;
        }
        if (!s.entities.has(d.tuId)) xau.push(`tuyến '${id}' bắt đầu ở '${d.tuId}' không tồn tại`);
        if (!s.entities.has(d.denId)) xau.push(`tuyến '${id}' kết thúc ở '${d.denId}' không tồn tại`);
        if (d.tuId === d.denId) xau.push(`tuyến '${id}' nối một nơi với chính nó`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s) => {
      ...
      for (const lk of s.links.values()) {
        if (lk.tickDut !== null || lk.quanHe !== 'so_huu') continue;
        const vat = s.entities.get(lk.denId);
        if (!vat || vat.kind !== 'artifact') continue;
      ...
      for (const vatId of [...chu.keys()].sort((a, b) => (a < b ? -1 : 1))) {
        ...
        if (ds.length > 1) xau.push(`'${vatId}' đang có ${ds.length} chủ: ${ds.join(', ')}`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      const xau: string[] = [];
      for (const id of idCanKiem(s, phamVi)) {
        const ins = docAspect<Institutional>(s.entities.get(id), 'institutional');
        if (!ins) continue;
        for (const c of ins.chucVu) {
          if (c.nguoiGiuId === null) continue;
          const nguoi = s.entities.get(c.nguoiGiuId);
          if (!nguoi || nguoi.tickDiet === null) continue;
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of ids.sort((a, b) => (a < b ? -1 : 1))) {
        ...
        if (!no) continue;
        if (no.amount < 0) xau.push(`nợ '${id}' có amount = ${no.amount}`);
        if (!s.entities.has(no.creditorId)) xau.push(`nợ '${id}' có chủ nợ '${no.creditorId}' không tồn tại`);
        if (!s.entities.has(no.debtorId)) xau.push(`nợ '${id}' có con nợ '${no.debtorId}' không tồn tại`);
        if (no.creditorId === no.debtorId) xau.push(`nợ '${id}' tự nợ chính mình`);
      }
      return xau;
    },
  });
  ...
  dangKyInvariant({
    ...
    kiem: (s, phamVi) => {
      ...
      for (const id of idCanKiem(s, phamVi)) {
        ...
        if (!an || !dc) continue;
        const tong = dc.cohort.child + dc.cohort.youth + dc.cohort.adult + dc.cohort.elder;
        if (an.thuongVongKy < 0) xau.push(`'${id}' có thương vong âm`);
        if (tong < 0) xau.push(`'${id}' còn ${tong} người sau thương vong`);
      }
      return xau;
    },
  });
