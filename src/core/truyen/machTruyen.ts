...
import type { Beat, BienDoiTuSu, UngVienMach } from './loaiMach.js';
import { hashCua } from '../engine/hash.js';

/** Thứ tự giai đoạn khi mạch tiến — `chet_yeu` KHÔNG nằm trên đường này. */
const DUONG_DI: readonly GiaiDoanMach[] = ['am_i', 'khoi', 'phat_trien', 'cao_trao', 'ha_man', 'du_am'];

function giaiDoanKe(gd: GiaiDoanMach): GiaiDoanMach {
  const i = DUONG_DI.indexOf(gd);
  if (i < 0 || i === DUONG_DI.length - 1) return gd;
 ...
 */
export function idMach(branchId: string, loai: string, nhanVatIds: readonly string[]): string {
...
function nhanVatChinh(uv: UngVienMach): readonly string[] {
 ...
 */
export function quetMachTruyen(s: WorldState, nc: NgocCanhTruyen): KetQuaQuetMach {
   ...
   */
  const chuaCo = ungVien.filter(
    (uv) => !s.storylines.has(idMach(s.world.branchId, uv.loai, nhanVatChinh(uv))),
  );
  ...
  const coNguoiChoi = (uv: UngVienMach): boolean =>
    nc.nguoiChoiId !== null && uv.nhanVat.some((n) => n.entityId === nc.nguoiChoiId);

  const diem = (uv: UngVienMach): number => uv.cangThangDau + (coNguoiChoi(uv) ? 10 : 0);
  const sapXep = (a: UngVienMach, b: UngVienMach): number => {
  ...
  const chon: UngVienMach[] = [];
  chon.push(...vang.slice(0, Math.max(chatVang, 0)));
  for (const uv of co) {
    if (chon.length >= conCho) break;
    chon.push(uv);
  }
...
function nhipMacDinhCua(loai: string): number {
 ...
 */
const NHIP_MAC_DINH: Readonly<Record<string, number>> = Object.freeze({
  bao_thu: 50,
  ke_vi: 40,
  chien_tranh: 20,
  ly_giao: 60,
 ...
 */
export function nhipMachTruyen(s: WorldState, nc: NgocCanhTruyen): KetQuaNhipMach {
  const patches: PatchOp[] = [];
  const daChay: NhipDaChay[] = [];

  const ids = [...s.storylines.keys()].sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
  for (const id of ids) {
    const m = s.storylines.get(id);
    if (!m || m.tickKet !== null) continue;
     ...
     */
    let gdMoi: GiaiDoanMach = beat.giaiDoanMoi ?? m.giaiDoan;
    if (beat.giaiDoanMoi === undefined) {
      if (cangThang >= NGUONG_TIEN[m.giaiDoan]) gdMoi = giaiDoanKe(m.giaiDoan);
      const boBe = nc.tick - (m.tickChieuCuoi ?? m.tickSinh);
      if (m.giaiDoan === 'phat_trien' && boBe > m.nhipMoi * NGUONG_CHET_YEU) gdMoi = 'chet_yeu';
    }

    set('soNhip', soNhip);
    set('cangThang', cangThang);
    set('dongHo', m.nhipMoi);
    if (gdMoi !== m.giaiDoan) set('giaiDoan', gdMoi);
 ...
 */
function apBienDoiTuSu(
  s: WorldState,
  ds: readonly BienDoiTuSu[],
  machId: string,
  nc: NgocCanhTruyen,
): readonly PatchOp[] {
  const ra: PatchOp[] = [];
  ...
  for (const [i, b] of ds.entries()) {
    ...
    const soul = e.aspects['soul'] as { kyUc?: unknown[]; tamTrang?: unknown[] } | undefined;
    if (!soul) continue;

    if (b.loai === 'ky_uc') {
      if ((soul.kyUc ?? []).length >= TRAN_KY_UC_MOI_NGUOI) continue;
      ra.push({
        op: 'push',
    ...
    if ((soul.tamTrang ?? []).length >= TRAN_CAM_XUC) continue;
    ra.push({
      op: 'push',
...
const NGUONG_TIEN: Readonly<Record<GiaiDoanMach, number>> = Object.freeze({
  am_i: 20,
  khoi: 40,
  phat_trien: 65,
  cao_trao: 85,
 ...
 */
export function hanNgachVangMat(canh: readonly { coNguoiChoi: boolean }[]): DoVangMat {
  ...
  const thongDiep = dat
    ...
    : !duTyLe
      ? `Chỉ ${vang}/${soCanh} cảnh không có người chơi — dưới ngưỡng ${Math.round(
          TI_LE_VANG_MAT.mucTieu * 100,
        )}%. Ống kính kỷ nguyên sau sẽ ưu tiên mạch ở xa.`
...
export function machNguoiChoiBiet(s: WorldState): readonly Storyline[] {
