...
import type { StoryKindDef } from '../registry/types.js';
...
function theGioi(seed = 'cong-phase-8'): { state: WorldState; log: EventLog } {
...
function chay(state: WorldState, log: EventLog, soTick: number): void {
  for (let i = 0; i < soTick; i++) {
    ...
    for (const ev of r.events) {
      ...
      if (!ok.ok) throw new Error(ok.errors.map((e) => e.message).join('; '));
    }
  }
}
 ...
 * bao giờ nhìn thấy.
 */
function theGioiDong(seed = 'cong-phase-8-dong'): { state: WorldState; log: EventLog } {
  const { state, log } = theGioi(seed);
  const evId = 'ev_them_nguoi';

  const nguoi = (id: string, ten: string, quanHe: Record<string, unknown>, ducVong: Record<string, number>) =>
    EntitySchema.parse({
      id,
  ...
  const cast: { id: string; ten: string; qh: Record<string, unknown>; dv: Record<string, number> }[] = [
  ...
  const patches: PatchOp[] = cast.map((x) => ({
    op: 'link' as const,
...
function machTay(state: WorldState, ghiDe: Partial<Storyline> = {}): Storyline {
...
describe('mạch truyện đa dạng — Phần 28.3', () => {
  it('mười bốn loại đều có handler, không loại nào ở trạng thái can_adapter', () => {
    napDungSan();
    const ids = (R.storyKind.tatCa() as readonly StoryKindDef[]).map((d) => d.id);
  ...
  it('mười loại của bảng 28.3 đều có mặt, kể cả loại khai thác khe hở bốn trục', () => {
    napDungSan();
    const ids = new Set((R.storyKind.tatCa() as readonly StoryKindDef[]).map((d) => d.id));
    // `bao_thu` mang tiền đề của `phuc_thu`, `troi_day` mang tiền đề của `cuu_the` (ADR-0037).
    for (const x of ['bao_thu', 'ke_vi', 'chien_tranh', 'ly_giao', 'am_muu']) expect(ids.has(x)).toBe(true);
    for (const x of ['tinh_ai', 'kham_pha', 'troi_day', 'suy_tan', 'phan_boi']) expect(ids.has(x)).toBe(true);
  });
...
describe('nhịp mạch truyện chạy bằng engine — Phần 28.5', () => {
  ...
  it('mạch đã đóng KHÔNG bị sinh lại dưới cùng id', () => {
    ...
    for (const m of kq1.machMoi) state.storylines.set(m.id, { ...m, giaiDoan: 'du_am', tickKet: 9 });
    ...
    for (const m of kq2.machMoi) expect(kq1.machMoi.some((x) => x.id === m.id)).toBe(false);
  });
...
describe('[BB] Sổ Phục Bút — Phần 30.2: thứ đã gieo không biến mất', () => {
  ...
  it('[BB] quá hạn GẤP ĐÔI → thành gap nhan_qua, tức một bí ẩn — KHÔNG bị xóa', () => {
    ...
    expect((gap?.value as { loai: string }).loai).toBe('nhan_qua');
    expect((gap?.value as { trangThai: string }).trangThai).toBe('thanh_bi_an');
...
describe('[BB] nén có hình dạng truyện — Phần 30.3', () => {
  function machDayDu(state: WorldState): Storyline {
  ...
  it('phục bút ĐÃ TRẢ không còn chiếm chỗ trong bản nén', () => {
    ...
    if (pb) state.foreshadows.set('pb_1', { ...pb, daTra: true, cachTra: 'Nó gãy thật.' });
...
describe('[BB] 28.5 — nhịp truyện áp bienDoiTrangThai vào world', () => {
  it('cao trào để lại KÝ ỨC trong hồn từng người trong cuộc', () => {
    ...
    expect((kyUc[0]?.value as { lienQuan: string[] }).lienQuan).toEqual([m.id]);
  });
  ...
  it('cảm xúc luôn có ĐỐI TƯỢNG và nguồn gốc, không phải một con số trôi nổi', () => {
    ...
    const v = cx?.value as { loai: string; doiTuongId: string; nguonGocKyUcId: string };
  ...
  it('[BB] nhịp KHÔNG chạm vật chất — mười hai tiến trình nền giữ nguyên quyền', () => {
    ...
    for (const p of kq.patches) {
      if (p.target.table !== 'entities') continue;
...
describe('[BB] 30.3 — mốc kỷ nguyên là chỗ duy nhất phép nén chạy', () => {
  ...
  it('qua mốc kỷ nguyên: kyUcMach dày lên và bộ đệm được dọn', () => {
    ...
    for (const m of coNhip) expect(m.nhipGanDay).toEqual([]);
