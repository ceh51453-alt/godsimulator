...
import type { AnNinh, DanCu, KinhTe, YTe } from '../schema/aspect/substrate.js';
 ...
 */
export function quetBeTac(state: WorldState, noiId: string): readonly BeTac[] {
  const e = state.entities.get(noiId);
  const dc = doc<DanCu>(e, 'dan_cu');
  if (!e || !dc) return [];

  const dan = dc.cohort.child + dc.cohort.youth + dc.cohort.adult + dc.cohort.elder;
  if (dan <= 0) return [];

  const kt = doc<KinhTe>(e, 'kinh_te');
  const yt = doc<YTe>(e, 'y_te');
  const an = doc<AnNinh>(e, 'an_ninh');
  const ra: BeTac[] = [];
 ...
 */
export function thanDuocGoi(state: WorldState, noiId: string, rng: Rng): string | null {
  const ungVien: { id: string; mat: number }[] = [];
  for (const id of [...state.entities.keys()].sort((a, b) => (a < b ? -1 : 1))) {
    ...
    if (mat > 0) ungVien.push({ id, mat });
  }
  if (ungVien.length === 0) return null;
  const i = rng.softmax(
    ungVien.map((x) => x.mat * 100),
    35,
  );
  return ungVien[i]?.id ?? (ungVien[0] as { id: string }).id;
}

/** [BB] 22.2 — chỉ sinh khi `khaThi` dưới ngưỡng. Trên ngưỡng thì họ tự làm được. */
export const NGUONG_KHA_THI = 0.45;

export function sinhLoiCau(
  state: WorldState,
  bt: BeTac,
  ctx: { tick: number; eventId: string; rng: Rng },
): { prayer: Prayer; patch: PatchOp } | null {
  if (bt.khaThi >= NGUONG_KHA_THI) return null;

  const thanId = thanDuocGoi(state, bt.noiId, ctx.rng);
  const id = `cau_${ctx.tick}_${bt.noiId}_${bt.ducVongThieu}`;
  if (state.prayers.has(id)) return null;
 ...
 */
export function traLoiCau(
  state: WorldState,
  prayer: Prayer,
  cach: Exclude<CachTraLoi, 'chua'>,
  ctx: { tick: number; eventId: string },
): KetQuaTraLoi {
  ...
  const loiKe: Record<Exclude<CachTraLoi, 'chua'>, string> = {
...
export function loiCauQuaHan(state: WorldState, tick: number): readonly Prayer[] {
  const ra: Prayer[] = [];
  for (const id of [...state.prayers.keys()].sort((a, b) => (a < b ? -1 : 1))) {
    const p = state.prayers.get(id);
    if (!p || p.daTraLoi) continue;
    if (p.hanChot !== null && tick > p.hanChot) ra.push(p);
  }
  return ra;
}

/** Lời cầu đang chờ một vị thần cụ thể, sắp theo 22.4. */
export function loiCauCho(state: WorldState, thanId: string | null, tick: number): readonly Prayer[] {
  const ra: Prayer[] = [];
  for (const p of state.prayers.values()) {
    if (p.daTraLoi) continue;
    if (p.hanChot !== null && tick > p.hanChot) continue;
    if (thanId !== null && p.thanNhanId !== null && p.thanNhanId !== thanId) continue;
    ra.push(p);
  }
