...
function theGioi(seed = 'phase6'): { state: WorldState; log: EventLog } {
...
function nhapThan(state: WorldState, log: EventLog, thanId = THAN): void {
  const ev = taoEvent({
    id: `ev_nhap_${thanId}`,
...
function chay(state: WorldState, log: EventLog, soTick: number): string[] {
  const loi: string[] = [];
  for (let i = 0; i < soTick; i++) {
    ...
    for (const c of r.chanDoan) if (c.muc === 'loi') loi.push(`${c.ma}: ${c.thongDiep}`);
  }
  return loi;
}
 ...
 */
function epDoi(state: WorldState, ...noiIds: string[]): void {
  for (const id of noiIds) {
    const e = state.entities.get(id) as Entity;
    e.aspects['kinh_te'] = {
      ...(e.aspects['kinh_te'] as object),
      kho: { luongThuc: 0, vatLieu: 0 },
      thieuHut: 1,
    };
    e.aspects['sinh_thai'] = {
      ...(e.aspects['sinh_thai'] as object),
      taiNguyen: { rung: 0, thu: 0, ca: 0, dat: 0 },
    };
  }
}

const banNga = (s: WorldState, id = THAN): DivineIdentity =>
  s.entities.get(id)?.aspects['ban_nga'] as DivineIdentity;
...
describe('bốn lớp bản ngã — Phần 69.1', () => {
  it('thế giới mới gieo đủ bản ngã cho mọi vị thần', () => {
    ...
    const soul = state.entities.get(THAN)?.aspects['soul'] as { banTinh: Record<string, number> };
  ...
  it('đo áp lực chỉ ra trục lệch nặng nhất và nét bị đè', () => {
    const bn = banNgaTu(
      { tuBi_tanNhan: 60, kieuNgao_khiemNhuong: 0 } as never,
      { banTinhTinDoTin: { tuBi_tanNhan: -70 }, hienThanh: 20 } as never,
    );
    const ap = doApLuc(bn);
  ...
  it('[BB] lõi CHỈ đổi qua Event, và mỗi lần đổi đều ghi eventId', () => {
    ...
    const than = state.entities.get(THAN) as Entity;
    const bn = banNga(state);
    const r = dapDiHoa(
      than,
      { ...bn, followerImage: { ...bn.followerImage, tuBi_tanNhan: 90 } },
      'tuBi_tanNhan',
      'chap_nhan',
      {
        eventId: 'ev_test',
    ...
    expect((ls?.value as { eventId: string }).eventId).toBe('ev_test');
  });
  ...
  it('bất biến BẮT được lõi bị sửa lén', () => {
    ...
    const e = state.entities.get(THAN) as Entity;
    const bn = e.aspects['ban_nga'] as DivineIdentity;
...
describe('[BB] cổng Phase 6 — không mana, không cooldown', () => {
  ...
  it('bất biến BẮT được ai đó thêm thanh mana', () => {
    ...
    const e = state.entities.get(THAN) as Entity;
    e.aspects['domain'] = { ...(e.aspects['domain'] as object), mana: 100 };
    const inv = chayInvariantToanBo(state);
    expect(inv.dat).toBe(false);
...
describe('cầu nguyện — Phần 22', () => {
  it('[BB] lời cầu sinh từ bế tắc THẬT, không từ hư không', () => {
    ...
    const e = state.entities.get('place_a') as Entity;
    e.aspects['kinh_te'] = { ...(e.aspects['kinh_te'] as object), thieuHut: 0.6 };
  ...
  it('vùng đói vừa phải thì tự lo, không sinh lời cầu', () => {
    ...
    const e = state.entities.get('place_a') as Entity;
    e.aspects['kinh_te'] = { ...(e.aspects['kinh_te'] as object), thieuHut: 0.2 };
    const bt = quetBeTac(state, 'place_a').find((x) => x.ducVongThieu === 'anToan');
    expect(bt).toBeDefined();
    expect(bt?.khaThi).toBeGreaterThanOrEqual(NGUONG_KHA_THI);
    expect(
      sinhLoiCau(state, bt as never, { tick: 4, eventId: 'e', rng: rngCuaTick('s', 1, 'x') }),
    ).toBeNull();
  });
  ...
  it('[BB] bốn cách trả lời đều có hậu quả — làm ngơ KHÔNG phải không có gì', () => {
    for (const [cach, hq] of Object.entries(HAU_QUA_TRA_LOI)) {
      const coHauQua =
        hq.phuThuoc !== 0 || hq.thatVong !== 0 || hq.soHai !== 0 || hq.yeuGhet !== 0 || hq.tinNguong !== 0;
  ...
  it('ban phước GỠ đúng thứ đã sinh ra lời cầu', () => {
    ...
    const r = traLoiCau(state, cau as never, 'ban_phuoc', { tick: state.world.tick, eventId: evId });
    const ev = taoEvent({
      id: evId,
      branchId: state.world.branchId,
      tick: state.world.tick,
    ...
    const kt = state.entities.get('place_a')?.aspects['kinh_te'] as { thieuHut: number };
    expect(kt.thieuHut).toBe(0);
    // Và cái giá: vùng lệ thuộc hơn.
    const sp = state.entities.get('place_a')?.aspects['spatial'] as { doPhuThuoc: number };
...
describe('tranh đoạt domain bằng quy kết — Phần 19.2', () => {
  it('không HP, không sát thương — chỉ có ai được tin', () => {
    ...
    const r = giaiQuyKet(
      ...
      [{ thanId: THAN, domainTen: 'tay_ue', cuongDo: 60 }],
      { eventId: 'ev', tick: 4, tuning: TUNING, rng: rngCuaTick('s', 4, 'qk') },
    );
  ...
  it('rà soát domain cắt neo đã chết và đổi trạng thái theo', () => {
    ...
    const e = state.entities.get(THAN) as Entity;
    const dom = e.aspects['domain'] as { domains: Record<string, unknown>[] };
    dom.domains[0] = {
      ...dom.domains[0],
      suc: 0,
...
describe('[BB] cổng Phase 6 — thần NPC tiếp tục sống khi vắng', () => {
  it('ba tiến trình tầng Thần có mặt và nối handler', () => {
    ...
    for (const id of WORLD_PROCESS_IDS_THAN) expect(co.has(id), `thiếu '${id}'`).toBe(true);
  });
...
describe('[BB] cổng Phase 6 — ba mục tiêu ngoài tranh domain', () => {
  ...
  it('chơi trọn một vòng ở tầng Thần: trả lời cầu, đáp Dị Hóa, lập giao ước', () => {
    ...
    const r1 = traLoiCau(state, cau as never, 'dau_hieu', { tick: state.world.tick, eventId: ev1 });
    expect(
      apDungEvent(
        state,
        taoEvent({
          ...
          actorIds: [THAN],
          targetIds: [(cau as never as { nguoiCauId: string }).nguoiCauId],
          causeEventIds: [],
          locationId: null,
          patches: [...r1.patches],
          visibility: 'cong_khai',
    ...
    const idx = banNga(state).pressure.tinhHuongMo.findIndex((t) => t.id === (th as { id: string }).id);
    const ev2 = 'ev_muc_tieu_2';
    const r2 = dapDiHoa(
      state.entities.get(THAN) as Entity,
      banNga(state),
      (th as { truc: string }).truc as never,
      'mac_ca',
      { eventId: ev2, tick: state.world.tick, tuning: TUNING, rng: rngCuaTick(state.world.seed, 1, 'd') },
    );
    expect(
      apDungEvent(
        state,
