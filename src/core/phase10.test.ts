...
import {
  ...
  khaiNiemHuDanh,
  suyKhaiNiemSanCo,
  anhHuongKhiKhaiNiemYeuDi,
  coTheHoiSinh,
  bangTiepDia,
} from './vatly/tiepDia.js';
import {
  luatNenMacDinh,
  datTenTruc,
  daCoTen,
...
import type { PhanHoiGoi, YeuCauGoi } from './workflow/chay.js';
...
import type { CheDoBenchmark } from './retrieval/benchmark.js';
...
function chayTick(s: WorldState, log: ReturnType<typeof taoEventLog>, so: number): void {
  for (let i = 0; i < so; i++) {
...
const khaiNiem = (
  ...
  ten: string,
  trongSo: number,
  giaiDoan: 'hu_danh' | 'manh_nha' | 'thanh_hinh' | 'ket_tinh',
  tags: string[] = [],
): Entity =>
  EntitySchema.parse({
    id,
...
const luat = (
  ...
  tiepDia: {
    khaiNiemId: string;
    vaiTro: 'chu_the' | 'doi_tuong' | 'tac_dong' | 'trang_thai' | 'pham_tru';
    batBuoc?: boolean;
  }[],
): Entity =>
  EntitySchema.parse({
...
const entry = (over: Partial<LorebookEntry> & { id: string; ten: string; order: number }): LorebookEntry =>
  LorebookEntrySchema.parse(over);
...
describe('42.4 [BB] — Hiệu Lực dùng MIN, không dùng trung bình', () => {
  function theGioiCoLuat() {
    ...
    state.entities.set(
      'luat.mau',
      luat('luat.mau', 'Máu đã đổ thì không rửa được', [
  ...
  it('một khái niệm trọng số 0 kéo cả câu luật về 0 — dù ba cái kia rất mạnh', () => {
    ...
    const kq = tinhHieuLuc(s.entities.get('luat.mau') as Entity, s, TUNING);
  ...
  it('nuôi khái niệm yếu nhất lên thì luật mạnh dần — cùng câu chữ, sau vài kỷ nguyên có răng', () => {
    const s = theGioiCoLuat();
    const truoc = tinhHieuLuc(s.entities.get('luat.mau') as Entity, s, TUNING).hieuLuc;
    s.entities.set('kn.tay_ue', khaiNiem('kn.tay_ue', 'Tẩy Uế', 900, 'thanh_hinh'));
    const sau = tinhHieuLuc(s.entities.get('luat.mau') as Entity, s, TUNING).hieuLuc;
    expect(truoc).toBe(0);
    expect(sau).toBeGreaterThan(0);
    // Giờ mắt xích yếu nhất đổi sang cái khác — đó là dấu hiệu min hoạt động đúng.
    expect(tinhHieuLuc(s.entities.get('luat.mau') as Entity, s, TUNING).matXichYeuNhat?.ten).not.toBe(
      'Tẩy Uế',
    );
  });
...
describe('42.3, 42.6 — ba chế độ tiếp địa; hai chế độ mềm KHÔNG BAO GIỜ trượt', () => {
  ...
  it('chat_che TỪ CHỐI và nêu rõ khái niệm nào còn thiếu', () => {
    const kq = kiemTiepDia(s0('chat_che').entities.get('luat.x') as Entity, s0('chat_che'));
  ...
  it('[BB] tu_tiep_dia NHẬN luật, tạo khái niệm hư danh, và nói rõ nó yếu', () => {
    ...
    const kq = kiemTiepDia(s.entities.get('luat.x') as Entity, s);
    ...
    expect((kn.aspects['conceptual'] as { trongSo: number }).trongSo).toBe(0);
    s.entities.set(kn.id, kn);
    expect(tinhHieuLuc(s.entities.get('luat.x') as Entity, s, TUNING).hieuLuc).toBe(0);
  });
...
describe('42.5 [BB] — đánh vào khái niệm thay vì đánh vào luật', () => {
  it('làm khái niệm nền yếu đi thì luật tụt hiệu lực mà KHÔNG bị bãi bỏ', () => {
    ...
    state.entities.set(
      'luat.mau',
      luat('luat.mau', 'Máu không rửa được', [{ khaiNiemId: 'kn.tay_ue', vaiTro: 'tac_dong' }]),
    );
  ...
  it('[BB] hồi sinh được luật đã chết nếu khái niệm nền vẫn còn tồn tại', () => {
    ...
    expect(coTheHoiSinh(state.entities.get('luat.mau') as Entity, state, TUNING)).toBe(true);

    // Khái niệm chết hẳn thì không hồi sinh được nữa.
    state.entities.set('kn.tay_ue', { ...(state.entities.get('kn.tay_ue') as Entity), tickDiet: 5 });
    expect(coTheHoiSinh(state.entities.get('luat.mau') as Entity, state, TUNING)).toBe(false);
  });
...
describe('43 [BB] — bảy trục, vô danh và có tên', () => {
  ...
  it('[BB] 43.3 — khái niệm nền phải CÓ THẬT và ít nhất thành hình', () => {
    ...
    expect(kq.ok).toBe(false);
    if (!kq.ok) expect(kq.loi.join(' ')).toContain('manh_nha');
  });
  ...
  it('đặt tên thành công thì SINH KẼ HỞ và có dòng biên niên bằng giọng kể', () => {
    ...
    state.entities.set(
      'kn.truoc_sau',
      khaiNiem('kn.truoc_sau', 'Trước Sau', 1200, 'ket_tinh', ['truoc_sau']),
    );
  ...
  it('[BB] 43.6 — sửa luật nền LUÔN bắt buộc phân nhánh, và bất khả nghịch thì từ chối', () => {
    ...
    expect(kq.ok).toBe(true);
    if (kq.ok) expect(kq.batBuocPhanNhanh).toBe(true);
  });
  ...
  it('[BB] 43.4 — khoangCach = "y_nghia" làm hai đền cùng thần thành LIỀN KỀ', () => {
    ...
    for (const id of ['den.a', 'den.b']) {
      state.entities.set(
        id,
        EntitySchema.parse({ id, branchId: 'br_goc', kind: 'place', ten: id, tickSinh: 0, aspects: {} }),
      );
    }
    state.entities.set(
      'than.x',
...
describe('44 [BB] — cơ chế phái sinh là HỆ QUẢ, không phải tính năng', () => {
  it('[BB] 44.2 — mọi cơ chế PHẢI có `moTaKhiKhong` khác rỗng', () => {
    for (const id of Object.keys(CO_CHE) as (keyof typeof CO_CHE)[]) {
  ...
  it('bật hieuBietLamSuyYeu thì Thần Bí TỰ xuất hiện kèm công bố giọng biên niên', () => {
    ...
    const ds = luatNenMacDinh('br_goc').map((x) =>
      x.truc === 'nhan_thuc'
        ? SubstrateLawSchema.parse({
            ...x,
            trangThai: 'co_ten',
  ...
  it('Nguyên Điểm kéo bản tính về một trục, và sụp hết thì đề xuất đổi kind', () => {
    let bt: Record<string, number> = { tan_bao: 20, khoan_dung: 60, cham_chi: 50 };
    let doi = false;
    ...
    expect(bt['tan_bao']).toBeGreaterThan(bt['khoan_dung'] as number);
  });
  ...
  it('[BB] Vũ Khí Khái Niệm CHỈ TÍNH hậu quả, không áp gì — và hậu quả rất nặng', () => {
    ...
    state.entities.set(
      'luat.rong',
      luat('luat.rong', 'Luật về Rồng', [{ khaiNiemId: 'kn.rong', vaiTro: 'doi_tuong' }]),
    );
    state.links.set('l.r', {
      id: 'l.r',
      branchId: 'br_goc',
...
describe('35.3 — nhập lorebook ba định dạng, tự dò', () => {
  ...
  it('map đúng key → keys, insertion_order → order, constant → lop "loi"', () => {
    ...
    expect(kq.ok).toBe(true);
    const e = kq.lorebook?.entries[0] as LorebookEntry;
  ...
  it('[BB] 51.5 — order ngoài dải bị DỒN về dải của nguồn, giữ thứ tự tương đối', () => {
    ...
    expect((ds[0]?.order as number) < (ds[1]?.order as number)).toBe(true);
...
describe('51 [BB] — Sử thắng Nguồn, và che KHÔNG phải xóa', () => {
  ...
  it('51.7 — bảng đối soát gom đủ bốn loại', () => {
    const b = bangDoiSoat(
      doiSoatEntry({ entry: moi, lorebookId: 'lb.su', nguon: 'tu_sinh' }, [
...
describe('52 [BB] — bảng quyền: AI không bao giờ sửa entry người dùng', () => {
  it('bảng quyền khớp đúng 52.2', () => {
    for (const op of ['sua', 'gop', 'tach', 'doi_key', 'xoa'] as const) {
  ...
  function ctx(entries: LorebookEntry[], nguon: Record<string, 'nguoi_dung' | 'tu_sinh' | 'di_san'>) {
    return {
      entries: new Map(entries.map((e) => [e.id, e])),
      nguonCua: new Map(Object.entries(nguon)),
      nguonDich: 'tu_sinh' as const,
  ...
  it('op `sua` trên entry người dùng bị TỪ CHỐI với lý do cụ thể', () => {
    ...
    const kq = apMotOp(
      { op: 'sua', id: 'u1', truong: 'noiDung', noiDungMoi: 'bị sửa', lyDo: '' },
      ctx([e], { u1: 'nguoi_dung' }),
    );
    expect(kq.ok).toBe(false);
  ...
  it('op `che` trên entry người dùng ĐƯỢC PHÉP và giữ nguyên nội dung', () => {
    ...
    const kq = apMotOp(
      { op: 'che', id: 'u1', boiId: 's1', lyDo: 'Ra bị thu hồi' },
  ...
  it('[BB] 52.3 — xóa là XÓA MỀM, và thùng rác giữ ba kỷ nguyên', () => {
    ...
    const daXoa = kq.sua[0] as LorebookEntry;
  ...
  it('52.5 — lịch sử phiên bản không vượt 20 mục', () => {
    ...
    for (let i = 0; i < 30; i++) {
      const kq = apMotOp(
        { op: 'doi_key', id: 's1', keys: [`k${i}`] },
        { ...c, entries: new Map([[e.id, e]]) },
      );
      e = kq.sua[0] as LorebookEntry;
    }
...
describe('51.6 + 53 [BB] — văn bản không bao giờ tự chứng minh được chính nó', () => {
  const ev = (
    ...
    actors: string[],
    patches: number,
    vis: 'cong_khai' | 'bi_mat' = 'cong_khai',
    cause: string[] = [],
  ): [string, Event] => [
    id,
    {
...
describe('35.4, 35.5 [BB] — lorebook là lực hấp dẫn, và Dị Bản khi thế giới đi lối khác', () => {
  function lorebookAiCap(lucHapDan = 100) {
    ...
    const lb = { ...(kq.lorebook as NonNullable<typeof kq.lorebook>), lucHapDan, bat: true };
    return lb;
  }
  ...
  it('bật Lorebook hiện thực hóa neo nhân vật rõ ràng và kỳ vọng nhận đúng thực thể', () => {
    ...
    const lorebook = { ...(kq.lorebook as NonNullable<typeof kq.lorebook>), bat: true };

    expect(docNeoLore(lorebook.entries[0] as LorebookEntry)?.ten).toBe('Thanh My');
  ...
  it('[BB] 35.5 — thu hồi thần mặt trời sinh Dị Bản đủ bốn thứ bắt buộc + một gap', () => {
    ...
    expect(aiThoa(kv[0]?.dieuKien as never, state)).toBe('e.ra');
    ...
    const db = b2.diBanMoi[0] as NonNullable<(typeof b2.diBanMoi)[number]>;
...
describe('50.4 [BB] — lịch thời gian truyện là chế độ quan trọng nhất', () => {
  ...
  it('[BB] không đọc được thời gian thì BỎ LƯỢT, không chạy bừa', () => {
    ...
    const qd = quyetDinhChay(
      tv,
      trangThaiLichMoi(),
      { luot: 0, tick: 10, suKien: [], vanBan: 'không có thẻ nào' },
      5,
    );
  ...
  it('bốn chế độ lịch đều chạy đúng', () => {
    ...
    expect(
      quyetDinhChay(theoSk, trangThaiLichMoi(), { luot: 1, tick: 1, suKien: ['het_ky_nguyen'] }, 5).chay,
    ).toBe(true);
  });
});

describe('50.3 [BB] — ba mươi nhân vật thì ba mươi call', () => {
  const goiThanh: (log: YeuCauGoi[]) => (yc: YeuCauGoi) => Promise<PhanHoiGoi> = (log) => async (yc) => {
    log.push(yc);
    return { ok: true, text: `xong ${yc.mucId ?? 'chung'}` };
  };

  it('30 mục → 30 call, chạy theo lô 5, và một cái hỏng không kéo sập 29 cái kia', async () => {
    const goiLog: YeuCauGoi[] = [];
    ...
    const kq = await chayDuongOng({
      ...
      goi: async (yc) => {
        goiLog.push(yc);
        // Một mục hỏng.
        if (yc.mucId === 'e.npc7') return { ok: false, maLoi: 'HTTP_500', thongDiep: 'proxy lỗi' };
  ...
  it('giai đoạn sau CHỜ giai đoạn trước và thấy output của nó', async () => {
    ...
    const kq = await chayDuongOng({
      ...
      dungPrompt: (t, _m, nguCanhTruoc) => {
        if (t.id === 'c') thay.push(nguCanhTruoc);
...
describe('50.6, 50.7, 50.10 [BB] — đích ghi và lằn ranh lorebook người dùng', () => {
  ...
  it('[BB] 50.7 — thiếu chống đệ quy thì TỪ CHỐI ghi', () => {
    const t = {
      loai: 'ghi_lorebook' as const,
      mauChen: '',
      lorebookNguon: 'the_gioi' as const,
      lorebookId: 'lb.su',
      tenEntry: 'X',
      loaiEntry: 'constant' as const,
      keys: '',
      viTri: { position: 'x', depth: 1, order: 1 },
      chongDeQuy: false,
      tachTheoThuocTinh: false,
    ...
    expect(kq.ok).toBe(false);
    if (!kq.ok) expect(kq.loi[0]?.code).toBe('THIEU_CHONG_DE_QUY');
  });
...
describe('50.8, 50.9, 50.12 — bảy tác vụ dựng sẵn, năm preset, sáu chẩn đoán', () => {
  ...
  it('[BB] 50.2 — nhomPrompt là MẢNG có tên và vai trò, không phải một chuỗi lớn', () => {
    for (const t of TAC_VU_DUNG_SAN) {
      ...
      for (const n of t.nhomPrompt) expect(n.ten.trim()).not.toBe('');
    }
  });
  ...
  it('năm preset dựng sẵn, và preset chuẩn qua được kiểm lằn ranh', () => {
    ...
    expect(kiemLanRanh(PRESET_WORKFLOW.engine_hau_truong as never).dat).toBe(true);
  });
  ...
  it('[BB] 50.8 — xuất một file JSON, nhập lại ở save khác, chạy y hệt', () => {
    const goc = PRESET_WORKFLOW.chi_npc as NonNullable<typeof PRESET_WORKFLOW.chi_npc>;
...
describe('26.3 — merge CÓ conflict report', () => {
  ...
  it('[BB] thiếu một quyết định thì KHÔNG hợp — không dùng đề xuất thay người chơi', () => {
    ...
    expect(kq.ok).toBe(false);
    if (!kq.ok) expect(kq.chuaQuyetDinh).toContain('e.chung');
  });
  ...
  it('patch hợp nhánh áp được thật vào một state đích', () => {
    ...
    dich.entities.clear();
    dich.links.clear();
    const r = apPatch(dich, kq.patches as PatchOp[]);
...
describe('47 [BB] — Diễn Hóa: lằn ranh cứng và điểm dừng thông minh', () => {
  ...
  it('[BB] 47.4 — không sửa Luật Nền, không tạo nhánh, không ghi lorebook', () => {
    ...
    const p = (table: string, path = ''): PatchOp => ({
      op: 'set',
      target: { table, id: 'x', path },
      value: 1,
      sourceEventId: 'ev',
    });
    const kq = locPatchTheoLanRanh(
      [p('substrateLaws'), p('lorebooks'), p('entities', 'ten')],
      cauHinh,
      state,
    );
...
describe('cổng — imported registry KHÔNG chứa code', () => {
  ...
  it.each([
    ...
    const ban = JSON.parse(banJson) as Record<string, unknown>;
    const xau = {
      ...packSach,
      pack: {
        ...packSach.pack,
    ...
    expect(kq.ok).toBe(false);
    expect(kq.issues.some((i) => i.code === 'PACK_CO_DAU_VET_CODE' || i.code === 'PACK_SAI_SCHEMA')).toBe(
      true,
    );
  });
  ...
  it('xuất rồi nhập lại cho cùng pack', () => {
    const kq = nhapWorldPack(JSON.parse(xuatWorldPack(nhapWorldPack(packSach).pack as never)));
    expect(kq.ok).toBe(true);
...
describe('cổng — rerank tăng hoặc giữ nDCG, KHÔNG đánh đổi forbidden recall = 0', () => {
  ...
  const chay =
    (ketQua: Record<CheDoBenchmark, Record<string, string[]>>) =>
    async (ca: (typeof boDe)[number], mode: CheDoBenchmark) => ({
      kq: {
        caseId: ca.id,
        ...
        daFallback: false,
        tokenSauRerank: 500,
        modeUsed: mode === 'heuristic' ? ('heuristic' as const) : ('proxy_cross_encoder' as const),
      },
      latencyMs: mode === 'heuristic' ? 20 : 90,
    });
  ...
  it('ngang bằng thì nói rõ latency — semantic không sai nhưng chưa đáng tiền', () => {
    const base = {
      ...
      tongKet: { ndcgAt10: 0.5, recallAt20: 0.6, forbiddenRecall: 0, p95LatencyMs: 20 } as never,
      cong: [],
    };
    const moi = {
      mode: 'semantic' as const,
      tongKet: { ndcgAt10: 0.5, recallAt20: 0.6, forbiddenRecall: 0, p95LatencyMs: 200 } as never,
      cong: [],
    };
...
describe('cổng — tắt feature KHÔNG làm core hỏng', () => {
  ...
  it('bật đủ luật nền + cơ chế + lorebook rồi TẮT lại cho đúng hash như chưa từng bật', () => {
    ...
    for (const ln of luatNenMacDinh('br_goc')) a.state.substrateLaws.set(ln.id, ln);
    ...
    if (lb) a.state.lorebooks.set(lb.id, lb);
  ...
  it('năm bảng mới của Phase 10 đều patch được qua transaction, không có đường ghi tắt', () => {
    ...
    const ln = luatNenMacDinh('br_goc')[0] as ReturnType<typeof luatNenMacDinh>[number];
...
describe('bất biến Phase 10 — bắt lỗi ở lượt thứ bốn trăm, không chỉ ở lượt đầu', () => {
  ...
  function voiLorebook(nguon: 'nguoi_dung' | 'tu_sinh', e: LorebookEntry) {
  ...
  it('[BB] 51.6 — doTinCay cao mà không có sự kiện chống lưng thì cảnh báo', () => {
    const s = voiLorebook(
      'tu_sinh',
      entry({ id: 's1', ten: 'X', order: 10_001, doTinCay: 90, suKienChongLung: [] }),
    );
    expect(chayInvariantToanBo(s).canhBao.some((v) => v.code === 'ENTRY_KHONG_CHONG_LUNG_KHONG_NAP')).toBe(
      true,
    );
  });
  ...
  it('[BB] 43.5 — trục có tên khi trục phụ thuộc còn vô danh là vi phạm NẶNG', () => {
    ...
    for (const ln of luatNenMacDinh('br_goc')) {
      state.substrateLaws.set(
        ln.id,
        ln.truc === 'van_menh' ? { ...ln, trangThai: 'co_ten', khaiNiemNenId: 'kn.tat_yeu' } : ln,
      );
    }
  ...
  it('[BB] 43.2 — trục vô danh mà đã có kẽ hở là vi phạm NẶNG', () => {
    ...
    for (const ln of luatNenMacDinh('br_goc')) {
      state.substrateLaws.set(
        ln.id,
        ln.truc === 'thoi_gian' ? { ...ln, keHo: [{ moTa: 'lách được', daBiKhaiThac: false }] } : ln,
      );
    }
    expect(chayInvariantToanBo(state).viPhamNang.some((v) => v.code === 'KE_HO_CHI_CO_KHI_CO_TEN')).toBe(
      true,
    );
  });
