 ...
 */
export function dungChiMuc(s: WorldState, loreQuery = ''): readonly Chunk[] {
  const nhap: Nhap[] = [];
  ...
  for (const lbId of [...s.lorebooks.keys()].sort((a, b) => (a < b ? -1 : 1))) {
    ...
    for (const entry of entries) {
      ...
      for (const entity of s.entities.values()) {
        if (
          (ten !== '' && entity.ten.toLocaleLowerCase() === ten.toLocaleLowerCase()) ||
          keys.some(
            (k) =>
              entity.ten.toLocaleLowerCase() === k.toLocaleLowerCase() ||
