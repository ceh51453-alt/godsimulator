...
import type { AdapterSemantic, ThongTinChunk } from './rerank.js';
...
export const MACH_RERANK_MOI: MachRerank = Object.freeze({
  hongLienTiep: 0,
  moMach: false,
  conBoQua: 0,
  lyDoCuoi: '',
...
export function machSauHong(m: MachRerank, lyDo: string): MachRerank {
...
export function machSauThanhCong(): MachRerank {
  return MACH_RERANK_MOI;
}

/** Một request đi qua trong lúc mạch mở. Về 0 thì probe một batch nhỏ. */
export function machSauBoQua(m: MachRerank): MachRerank {
  if (!m.moMach) return m;
...
export function dungBaTruyVan(
  view: WorldView,
  nc: { tieuDiemIds: readonly string[]; loiNguoiChoi: string; machDangChieuId: string | null },
): BaTruyVan {
 ...
 */
export function locTamNhin(ds: readonly Chunk[], nc: NgocCanhLoc): readonly ChunkDaChieu[] {
  const ra: ChunkDaChieu[] = [];
  for (const c of ds) {
    if (c.branchId !== nc.view.branchId) continue;
    ...
    if (c.entityIds.length > 0 && !c.entityIds.some((id) => nc.view.entities.has(id))) continue;

    let text = c.noiDung;
    let daBopMeo = false;
    if (c.tamNhin.laTinDon) {
      const chang = typeof c.meta['chang'] === 'number' ? (c.meta['chang'] as number) : 1;
...
export function scopeKeyCua(view: WorldView): string {
...
export async function truyHoi(
  dv: DauVaoTruyHoi,
  chunkCam: ReadonlySet<string> = new Set(),
): Promise<KetQuaTruyHoi> {
  const now = dv.dongHo ?? ((): number => 0);
  const bat = now();
  ...
  for (const id of dv.tieuDiemIds) diemEntity.set(id, 1);
  ...
  const candidates: RerankCandidate[] = hopNhat.map((m, i) => {
  ...
  const query: RerankQuery = RerankQuerySchema.parse({
  ...
  if (daCache) {
    const goiLai = daCache.orderedChunkIds
      ...
      .filter((c): c is ChunkDaChieu => c !== undefined);
    return {
      query,
      candidates,
      ketQua: daCache,
  ...
  let hangSemantic: Map<string, number> | null = null;
  let fallbackReason = '';
  let modeUsed = dv.config.endpoint.mode;

  const muonSemantic =
    dv.config.bat && dv.adapter != null && dv.config.endpoint.mode !== 'heuristic' && candidates.length > 1;
  ...
  // ── 7. MMR ──
  const traMmr = new Map(
    candidates.map((c) => [c.chunkId, { projectedText: c.projectedText, nguonId: c.nguonId }]),
  );
  ...
  const traGoi = new Map<string, ThongTinChunk>(
    candidates.map((c) => [
      c.chunkId,
      {
        projectedText: c.projectedText,
   ...
   */
  if (fallbackReason === '') await dv.cacheGhi?.(khoa, ketQua);

  const daChon = goi.chon.map((m) => traCuu.get(m.chunkId)).filter((c): c is ChunkDaChieu => c !== undefined);
