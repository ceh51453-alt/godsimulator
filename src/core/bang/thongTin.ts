...
import type { NguonSinh } from '../schema/aspect/provenance.js';

// ─────────────────────────────────────────── tab

export const TAB_THONG_TIN = ['tong_quan', 'quy_luat', 'tao_vat', 'than_he', 'mach_truyen', 'ta'] as const;
 ...
 */
export function nhanTab(tab: TabThongTin, mode: ViewMode): string {
  if (tab === 'ta') {
    return mode === 'sang_the' ? 'Ta' : mode === 'than' ? 'Thần Vị Của Ta' : 'Đời Ta';
  }
  const bang: Readonly<Record<Exclude<TabThongTin, 'ta'>, string>> = {
    tong_quan: 'Tổng quan',
    quy_luat: 'Quy luật',
    tao_vat: 'Tạo vật',
    than_he: 'Thần hệ',
  ...
  return bang[tab as Exclude<TabThongTin, 'ta'>];
}

/** [BB] 58.13 — không dùng "Không có dữ liệu". Mỗi tab có câu rỗng của thế giới. */
export const CAU_RONG: Readonly<Record<TabThongTin, string>> = Object.freeze({
...
export type TabTa = Readonly<{
  ...
  dauAn: readonly Readonly<{ ten: string; loai: string; tick: number }>[];
...
export type BangThongTin = Readonly<{
  ...
  chipLoai: readonly Readonly<{ kindId: string; nhan: string; so: number }>[];
}>;
...
const NHAN_TANG: Readonly<Record<ViewMode, string>> = Object.freeze({
 ...
 */
const NHAN_PHAM_VI: Readonly<Record<string, string>> = Object.freeze({
  vu_tru: 'toàn vũ trụ',
  coi: 'một cõi',
  vung: 'một vùng',
...
function nhanPhamVi(pv: string): string {
  return NHAN_PHAM_VI[pv] ?? pv.replace(/_/g, ' ');
}

function dongDau(s: string): string {
  const t = s.trim();
  if (t === '') return '';
  const cat = t.indexOf('\n');
...
function nhomCua(nguon: NguonSinh): string {
  for (const [nhom, ds] of Object.entries(NHOM_NGUON)) {
    if ((ds as readonly string[]).includes(nguon)) return nhom;
  }
  return 'tu_sinh';
}
 ...
 */
function dungQuyLuat(view: WorldView): readonly HangLuat[] {
  const ra: HangLuat[] = [];

  for (const l of view.laws) {
    const e = view.entities.get(l.id);
    const aspect = e?.aspects['lawful'] as
      { hieuLuc?: number; xungDot?: unknown[]; ngoaiLe?: unknown[]; keHo?: unknown[] } | undefined;
    const hieuLuc = typeof aspect?.hieuLuc === 'number' ? aspect.hieuLuc : null;
    const soVanDe =
      (Array.isArray(aspect?.xungDot) ? aspect.xungDot.length : 0) +
      (Array.isArray(aspect?.ngoaiLe) ? aspect.ngoaiLe.length : 0);

    const trangThai =
      hieuLuc === null ? 'chưa rõ' : hieuLuc >= 50 ? 'hiệu lực' : hieuLuc > 0 ? 'yếu' : 'treo';
  ...
  const bac = (h: HangLuat): number => {
    if (h.soVanDe > 0) return 0;
    if (h.trangThai === 'hiệu lực') return 2;
    if (h.trangThai === 'yếu') return 1;
    if (h.trangThai === 'treo') return 3;
    return 4;
  };
  ra.sort((a, b) =>
    bac(a) !== bac(b) ? bac(a) - bac(b) : (b.hieuLuc ?? 0) - (a.hieuLuc ?? 0) || (a.id < b.id ? -1 : 1),
  );
...
function dungTaoVat(view: WorldView): readonly HangTaoVat[] {
  const bac = new Map<string, { den: string; quanHe: string; trongSo: number }[]>();
  ...
  for (const e of view.entities.values()) {
    if (KHONG_LA_TAO_VAT.has(e.kind)) continue;
    ...
    const dom = e.aspects['domain'] as { domains?: readonly { ten?: string }[] } | undefined;
    const car = e.aspects['carrier'] as { khaiNiemIds?: readonly string[] } | undefined;
    const anhHuong = [
      ...(dom?.domains ?? []).map((d) => d.ten ?? '').filter((s) => s !== ''),
      ...(car?.khaiNiemIds ?? []).map((id) => view.entities.get(id)?.ten ?? '').filter((s) => s !== ''),
    ].slice(0, 3);

    const sp = e.aspects['spatial'] as { viTriId?: string } | undefined;
...
function dungThanHe(view: WorldView): readonly HangThanHe[] {
  const ra: HangThanHe[] = [];
  for (const e of view.entities.values()) {
    if (e.kind !== 'pantheon') continue;
    const inst = e.aspects['institutional'] as
      { moHinhCaiTri?: string; thanhVienIds?: readonly string[]; nguoiDungDauId?: string | null } | undefined;
...
function dungMach(view: WorldView, tc: TuyChonBang): readonly HangMach[] {
  const ghim = new Set(tc.theoDoiMachIds ?? []);
  const ra = view.machTruyen.map((m) =>
    Object.freeze({
      id: m.id,
      ten: m.ten,
      loai: m.loai,
  ...
  const bac = (h: HangMach): number => (h.dangTheoDoi ? 0 : h.giaiDoan === 'cao_trao' ? 1 : 2);
  return Object.freeze(
    [...ra].sort((a, b) =>
      bac(a) !== bac(b) ? bac(a) - bac(b) : b.cangThang - a.cangThang || (a.id < b.id ? -1 : 1),
    ),
  );
}
 ...
 */
function dungTa(view: WorldView): TabTa {
  const chuThe = view.chuTheId === null ? undefined : view.entities.get(view.chuTheId);

  const dauAn: { ten: string; loai: string; tick: number }[] = [];
  for (const e of view.entities.values()) {
    const ng = docNguonGoc(e.aspects);
    const laCuaTa = ng.nguon === 'nguoi_choi' || (view.chuTheId !== null && ng.actorId === view.chuTheId);
    if (!laCuaTa) continue;
    if (e.id === view.chuTheId) continue;
    dauAn.push({ ten: e.ten, loai: tenKind(e.kind), tick: ng.tick });
  }
  dauAn.sort((a, b) => (b.tick !== a.tick ? b.tick - a.tick : a.ten < b.ten ? -1 : 1));

  const ven = chuThe?.aspects['venerable'] as
    { banTinhTinDoTin?: Readonly<Record<string, unknown>>; danhXungKhac?: readonly string[] } | undefined;

  const heQua: ChuoiHeQua[] = [];
  if (view.chuTheId !== null) {
    const toi = new Map<string, { den: string; quanHe: string }[]>();
    for (const l of view.links) {
      if (l.daDut) continue;
    ...
    for (const goc of dauAn.slice(0, 3)) {
      ...
      if (batDau === undefined) continue;
      const buoc: string[] = [];
      const daQua = new Set<string>([batDau.id]);
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (canh === undefined) break;
        const den = view.entities.get(canh.den);
        if (den === undefined) break;
        buoc.push(`${canh.quanHe} → ${den.ten}`);
        daQua.add(canh.den);
        hienTai = canh.den;
      }
      if (buoc.length > 0) heQua.push(Object.freeze({ moc: goc.ten, cacBuoc: Object.freeze(buoc) }));
    }
  }
  ...
  return Object.freeze({
    ...
    theGianGoi: Object.freeze(
      ven?.danhXungKhac !== undefined && ven.danhXungKhac.length > 0
        ? [...ven.danhXungKhac]
...
export function tinhBangThongTin(view: WorldView, tc: TuyChonBang = {}): BangThongTin {
  ...
  const demChip = new Map<string, number>();
  for (const t of taoVat) demChip.set(t.kindId, (demChip.get(t.kindId) ?? 0) + 1);
 ...
 */
export function timTrongBang(bang: BangThongTin, q: string): readonly KetQuaTim[] {
  const tu = q.trim().toLowerCase();
  if (tu === '') return Object.freeze([]);
  const ra: KetQuaTim[] = [];
  const co = (...ds: readonly (string | undefined)[]): boolean =>
    ds.some((s) => typeof s === 'string' && s.toLowerCase().includes(tu));

  for (const l of bang.quyLuat) {
    if (co(l.ten, l.cau)) ra.push({ tab: 'quy_luat', id: l.id, ten: l.ten, vi: l.trangThai });
  }
