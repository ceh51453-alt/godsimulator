...
import type { DonViThoiGian, WorkflowSchedule, WorkflowTask } from './schema.js';
 ...
 */
export const TICK_MOI_DON_VI: Readonly<Record<DonViThoiGian, number>> = Object.freeze({
  gio: 1,
  ngay: 1,
  tuan: 1,
  thang: 1,
...
export function trangThaiLichMoi(): TrangThaiLich {
...
export function quyetDinhChay(
  ...
  tt: TrangThaiLich,
  nc: NgocCanhLich,
  nguongParseLoiLienTiep: number,
): QuyetDinhLich {
  if (!task.bat) {
    return { chay: false, soLan: 0, lyDo: 'tác vụ đang tắt', trangThaiSau: tt };
  }
  const lich: WorkflowSchedule | null = task.lich;
  ...
  const tickHienTai =
    tg.nguonThoiGian.loai === 'tick_engine'
      ? nc.tick
 ...
 */
export function docTickTuVanBan(vanBan: string, tenThe: readonly string[]): number | null {
