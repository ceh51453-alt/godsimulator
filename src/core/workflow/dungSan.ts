...
const nhom = (ten: string, vaiTro: 'system' | 'user' | 'assistant', noiDung: string) => ({
  ten,
  vaiTro,
  noiDung,
  bat: true,
 ...
 */
const moiDinhDang = (mo: string) => nhom('mồi định dạng', 'assistant', mo);

export const TAC_VU_DUNG_SAN: readonly WorkflowTask[] = Object.freeze([
  WorkflowTaskSchema.parse({
    ...
    nhomPrompt: [
      ...
      nhom(
        'khuôn',
        'system',
        'Trả về JSON: [{"id":"...","lyDo":"..."}]. Không rào markdown, không thêm lời nào.',
      ),
      moiDinhDang('['),
    ],
  ...
  WorkflowTaskSchema.parse({
    ...
    nhomPrompt: [
      ...
      nhom(
        'giới hạn',
        'system',
        'Engine giữ sổ. Không bịa số dân, số của cải hay tên riêng chưa có trong dữ liệu.',
      ),
    ],
    cheDoCoNhau: 'json_patch',
    cheDoNguCanh: 'rieng',
...
const lay = (...ids: readonly string[]): WorkflowTask[] =>
  ids.map((id) => TAC_VU_DUNG_SAN.find((t) => t.id === id)).filter((t): t is WorkflowTask => t !== undefined);

/** Năm preset dựng sẵn — 50.8. */
export const PRESET_WORKFLOW: Readonly<Record<string, WorkflowPreset>> = Object.freeze({
 ...
 */
export const DUONG_DAN_CAM_WORKFLOW: readonly string[] = Object.freeze([
  'substrateLaws',
  'tuning',
  'branches',
  'aiConfigs',
...
export type KetQuaKiemLanRanh = { readonly dat: boolean; readonly loi: readonly StructuredError[] };

/** Kiểm một preset workflow trước khi nạp — chặn sớm thay vì chặn lúc chạy. */
export function kiemLanRanh(preset: WorkflowPreset): KetQuaKiemLanRanh {
  ...
  for (const t of preset.tasks) {
    for (const dg of t.dichGhi) {
      if (dg.loai === 'ghi_lorebook' && !dg.chongDeQuy) {
        l.push(
          loi('schema', 'THIEU_CHONG_DE_QUY', `Tác vụ "${t.ten}" ghi lorebook mà không bật chống đệ quy.`, {
            path: t.id,
          }),
        );
      }
      if (dg.loai === 'patch_world') {
        for (const cam of DUONG_DAN_CAM_WORKFLOW) {
          if (dg.mauChen.includes(cam)) {
            l.push(
              loi('schema', 'CHAM_DUONG_DAN_CAM', `Tác vụ "${t.ten}" chạm bảng cấm "${cam}" (50.10).`, {
                path: t.id,
                severity: 'fatal',
                recoverable: false,
              }),
    ...
    if (t.id === 'hanh_dong_npc' && !t.hoBanSao.bat) {
      l.push(
        loi(
          'schema',
          'STAGE2_THIEU_HO_BAN_SAO',
          'Tác vụ hành động NPC bắt buộc bật họ bản sao (50.9 [BB]).',
          {
            path: t.id,
          },
        ),
    ...
    if (t.id === 'thoi_cuc_the_gioi' && t.lich?.cheDo !== 'theo_thoi_gian_truyen') {
      l.push(
        loi('schema', 'STAGE4_SAI_LICH', 'Tác vụ thời cục phải dùng lịch thời gian truyện (50.9 [BB]).', {
          path: t.id,
        }),
      );
    }
    if (t.nhomPrompt.length === 0) {
      l.push(
        loi(
          'schema',
          'THIEU_NHOM_PROMPT',
          `Tác vụ "${t.ten}" không có nhóm prompt nào — không gỡ lỗi được.`,
          {
            path: t.id,
            severity: 'warning',
          },
...
export function chanDoanWorkflow(
  sl: SoLieuChanDoan,
  nguong: {
...
export function xuatPreset(p: WorkflowPreset): string {
  return JSON.stringify({ _format: 'thien_dien_workflow_v1', preset: p }, null, 2);
}

export function nhapPresetWorkflow(
  text: string,
): { ok: true; preset: WorkflowPreset } | { ok: false; loi: string } {
  let cay: unknown;
  try {
    cay = JSON.parse(text);
  } catch {
  ...
  const goc = cay as { _format?: unknown; preset?: unknown };
  ...
  if (!r.success) return { ok: false, loi: r.error.issues.map((i) => i.message).join('; ') };
