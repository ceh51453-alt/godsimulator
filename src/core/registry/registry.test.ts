...
describe('mười hai registry', () => {
  ...
  it('mỗi registry đều có mục dựng sẵn — không registry nào rỗng', () => {
    ...
    for (const id of REGISTRY_IDS) {
      const reg = R[id] as Registry<{ id: string }>;
  ...
  it('aspect và kind nền của Phase 5 nằm ngoài bảng 4.2/4.3, không chồng lấn', () => {
    ...
    for (const id of ASPECT_IDS_NEN) expect(ASPECT_IDS_42).not.toContain(id);
    for (const id of KIND_IDS_NEN) expect(KIND_IDS_43).not.toContain(id);
...
describe('quetDauVetCode chặn pack độc hại', () => {
  ...
  it('bắt được prototype pollution qua khóa __proto__', () => {
    const doc = JSON.parse('{"config":{"__proto__":{"x":1}}}') as unknown;
...
describe('ba tầng nạp — Phần 5.2', () => {
  const dinhNghiaHopLe = (id: string) => ({
    id,
    ten: 'Thử',
  ...
  it('pack ghi đè dựng sẵn theo id', () => {
    ...
    const reg = R.kind as unknown as Registry<{ id: string; ten: string }>;
    reg.napPack('pack_thu', [{ ...(truoc as object), ten: 'Thượng Đế' } as never]);
  ...
  it('[BB] ghi đè mục không tồn tại thì cảnh báo, giữ tầng dưới, KHÔNG crash', () => {
    ...
    const loi = R.kind.ghiDe('khong_ton_tai', { ten: 'X' } as never);
  ...
  it('[BB] ghi đè KHÔNG hợp lệ bị bỏ, giữ giá trị tầng dưới, không throw', () => {
    ...
    const reg = createRegistry<{ id: string; ten: string }>('kind', (d) => {
      const x = d as { id?: string; ten?: string };
      if (!x.id || !x.ten) return { ok: false as const, errors: ['thiếu id hoặc ten'] };
      return { ok: true as const, value: { id: x.id, ten: x.ten } };
    });
  ...
  it('co() là cách kiểm tính hợp lệ của kind chuỗi — Phần 4.1', () => {
    ...
    R.kind.napPack('pack_tinh_linh', [dinhNghiaHopLe('tinh_linh') as never]);
