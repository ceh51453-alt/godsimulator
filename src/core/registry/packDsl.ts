 ...
 */
export function nhapWorldPack(goc: unknown): KetQuaNhapPack {
  const issues: ImportIssue[] = [];
  const rong: KetQuaNhapPack = {
    ok: false,
    pack: null,
    muc: [],
    tuning: null,
  ...
  if (g._format !== 'thien_dien_world_pack_v1') {
    issues.push({
      ...
      path: '_format',
      message:
        'Thiếu `_format: "thien_dien_world_pack_v1"`. World pack phải khai tường minh — ' +
  ...
  for (const m of pack.entries) {
    if (!(REGISTRY_IDS as readonly string[]).includes(m.registry)) {
    ...
    if (m.handlerId !== '' && !coHandler(m.handlerId)) {
      muc.push({
        ...
        lyDo:
          `handlerId "${m.handlerId}" chưa có trong HandlerCatalog. Mục được LƯU nguyên vẹn nhưng ` +
  ...
  const dem = (tt: TrangThaiMuc): number => muc.filter((m) => m.trangThai === tt).length;
  return {
...
export function xuatWorldPack(pack: RegistryPack, tuning?: Tuning): string {
  return JSON.stringify(
    tuning === undefined
      ? { _format: 'thien_dien_world_pack_v1', pack }
      : { _format: 'thien_dien_world_pack_v1', pack, tuning },
    null,
    2,
  );
}
