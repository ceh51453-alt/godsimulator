/**
 * Mười bốn kind dựng sẵn — Phần 4.3 [MR].
 *
 * [BB] `phanChieu` là DỮ LIỆU. Quy tắc chiếu (Phần 18.2) đọc từ đây, không hardcode.
 * Người dùng thêm loại thực thể mới bằng JSON, không sửa code.
 */
import type { KindDef, PhanChieu } from './types.js';
import { manifestCua } from './define.js';

type Nhap = Omit<KindDef, 'manifest' | 'moTa'> & { moTa: string };

const P = (than: PhanChieu['than'], phamNhan: PhanChieu['phamNhan']): PhanChieu => ({
  sangThe: 'day_du',
  than,
  phamNhan,
});
...
const nguon: readonly Nhap[] = [...nguon43, ...nguonNen, ...nguonThan];

export const KINDS_DUNG_SAN: readonly KindDef[] = nguon.map((n) => ({
  ...n,
