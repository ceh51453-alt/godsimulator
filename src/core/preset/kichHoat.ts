...
export const BAC_QUYEN: readonly string[] = Object.freeze([
  'Product safety',
 ...
 */
export function lintTruocKhiBat(
  row: PresetPackRow,
  chon: {
  ...
  const dangBat: PromptModule[] = row.pack.modules.map((m) => ({ ...m, enabled: daChon.has(m.id) }));
  ...
  for (const n of nhomXungDot(dangBat)) {
    ...
    if (!Object.hasOwn(chon.conflictResolutions, n.khoa)) {
      issues.push({
        ...
        path: n.khoa,
        message:
          `Nhóm "${n.khoa}" có ${n.moduleIds.length} module cùng tác động. ` +
...
export type KetQuaKichHoat =
  | { readonly ok: true; readonly activation: PresetActivation; readonly issues: readonly ImportIssue[] }
 ...
 */
export function hoanTac(hienTai: PresetActivation | null): {
  readonly veActivationId: string | null;
  readonly veNative: boolean;
} {
  if (hienTai === null) return { veActivationId: null, veNative: true };
  return {
 ...
 */
export function apActivation(row: PresetPackRow, act: PresetActivation | null): NormalizedPresetPack {
...
export type DiffPack = {
  ...
  readonly thamSoDoi: readonly { readonly truong: string; readonly truoc: unknown; readonly sau: unknown }[];
...
export function diffPack(cu: PresetPackRow, moi: PresetPackRow): DiffPack {
  ...
  for (const [src, m] of b) {
    ...
    if (ha !== hb) moduleDiff.push({ moduleId: m.id, loai: 'doi_noi_dung', truoc: ha, sau: hb });
    const ma = truoc.macroRefs.join(',');
    const mb = m.macroRefs.join(',');
    if (ma !== mb) moduleDiff.push({ moduleId: m.id, loai: 'doi_macro', truoc: ma, sau: mb });
  }
  for (const [src, m] of a) {
    if (!b.has(src)) moduleDiff.push({ moduleId: m.id, loai: 'bo', truoc: m.name, sau: '' });
  }

  const thamSoDoi: { truong: string; truoc: unknown; sau: unknown }[] = [];
  const ta = new Map(cu.thamSo.map((t) => [t.truong, t.raw]));
  const tb = new Map(moi.thamSo.map((t) => [t.truong, t.raw]));
  for (const [k, v] of tb) {
    const truoc = ta.get(k);
    if (JSON.stringify(truoc) !== JSON.stringify(v)) thamSoDoi.push({ truong: k, truoc, sau: v });
  }
  ...
  moduleDiff.sort((x, y) =>
    x.moduleId !== y.moduleId ? (x.moduleId < y.moduleId ? -1 : 1) : x.loai < y.loai ? -1 : 1,
  );
 ...
 */
export function versionKeTiep(daCo: readonly PresetPackRow[]): number {
