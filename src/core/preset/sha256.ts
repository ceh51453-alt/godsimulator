...
export function utf8Bytes(s: string): Uint8Array {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c < 0x80) ra.push(c);
    else if (c < 0x800) ra.push(0xc0 | (c >> 6), 0x80 | (c & 0x3f));
    else if (c < 0x10000) ra.push(0xe0 | (c >> 12), 0x80 | ((c >> 6) & 0x3f), 0x80 | (c & 0x3f));
    else {
...
export function soByteUtf8(s: string): number {
  return utf8Bytes(s).length;
}

const xoay = (x: number, n: number): number => ((x >>> n) | (x << (32 - n))) >>> 0;
