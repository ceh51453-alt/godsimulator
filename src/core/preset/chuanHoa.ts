...
function chonActivation(
  nhan: readonly string[],
  macroLa: readonly string[],
  laMarker: boolean,
): ActivationState {
  for (const n of nhan) {
    if (XU_LY_THEO_NHAN[n as keyof typeof XU_LY_THEO_NHAN] === 'quarantine') return 'quarantined';
  }
  for (const n of nhan) {
    if (XU_LY_THEO_NHAN[n as keyof typeof XU_LY_THEO_NHAN] === 'disable') return 'disabled';
  }
  if (macroLa.length > 0) return 'needs_adapter';
...
export function chuanHoaSillyTavern(input: {
  ...
  if (quarantined.length > 0) {
    issues.push({
      ...
      message:
        `${quarantined.length} script trợ giúp đã được LƯU nguyên vẹn và KHÔNG chạy. ` +
