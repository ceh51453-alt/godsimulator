...
const XuongRegistry = lazy(() =>
  import('./ui/screens/XuongRegistry.js').then((m) => ({ default: m.XuongRegistry })),
);
 ...
 * hỏng.
 */
function DangTaiMan(): JSX.Element {
  return (
    <p style={{ padding: '32px 24px', color: 'var(--mo)', fontSize: 13 }} role="status">
      Đang mở màn…
    </p>
  );
}
...
export function App() {
  ...
  const [loiKhoiDong, setLoiKhoiDong] = useState<readonly string[]>([]);
  ...
  useEffect(() => {
    void khoiDongDb().then((ds) => {
      if (ds.length > 0) setLoiKhoiDong(ds.map((e) => `${e.code}: ${e.message}`));
    });
  }, []);
   ...
   */
  const bangLoiKhoiDong =
    loiKhoiDong.length === 0 ? null : (
      <div
        role="alert"
        style={{
          padding: '10px 20px',
      ...
      >
        <strong>Nâng cấp dữ liệu chưa xong.</strong> Đừng tạo ván mới trước khi xuất bản sao:{' '}
        {loiKhoiDong.join(' · ')}
      </div>
    );
  ...
  if (!daVao) {
    ...
    return (
      <>
        {bangLoiKhoiDong}
        {dangTao ? (
          <KhoiNguyen onQuayLai={() => setDangTao(false)} />
        ) : (
          <ManChinh onBatDau={() => setDangTao(true)} />
        )}
      </>
    );
  }
 ...
 */
function ManCaiDatTheoId({ man, onVeSanh }: { man: string; onVeSanh?: () => void }): JSX.Element {
  switch (man) {
    case 'cai_dat':
      return <CaiDat />;
    case 'cai_dat_ai':
      return <CaiDat tabDau="proxy" />;
    case 'xuong_preset':
      return <CaiDat tabDau="preset" />;
    case 'lorebook':
      return <CaiDat tabDau="lorebook" />;
    case 'xuong_workflow':
      return <CaiDat tabDau="workflow" />;
    case 'chan_doan':
      return <ChanDoan />;
    case 'xuong_registry':
      return <XuongRegistry />;
    case 'ban_do_nhanh':
      return <BanDoNhanh />;
    case 'vat_ly':
      return <VatLy />;
    default:
      ...
      return <ManLa ten={man} onVeSanh={onVeSanh} />;
  }
}

function ManLa({ ten, onVeSanh }: { ten: string; onVeSanh?: () => void }): JSX.Element {
  return (
    <main style={{ maxWidth: 620, margin: '0 auto', padding: '48px 22px' }}>
      <h1 style={{ fontFamily: 'var(--chu-hien)', fontSize: 28, margin: 0 }}>Không có màn nào tên vậy</h1>
      <p style={{ color: 'var(--tro)', fontSize: 14 }}>
        Trạng thái giao diện đang trỏ tới <code style={{ fontFamily: 'var(--chu-so)' }}>{ten}</code>, một màn
        không còn tồn tại. Thường là vì save này được ghi bởi một bản cũ hơn. Quay về Sảnh là an toàn — không
        có dữ liệu nào của ván bị ảnh hưởng.
      </p>
      {onVeSanh !== undefined && (
        <button
          type="button"
